...
# tighter, path-specific limit). Shipped in PREVIEW (log-only) so you can watch
# real traffic before enforcing — frontline-worker clinics often sit behind one
# shared NAT, so calibrate the threshold against `enforcedAction`/preview logs
# before dropping `--preview` (gcloud compute security-policies rules update 2000
