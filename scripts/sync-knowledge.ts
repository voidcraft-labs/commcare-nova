...
import type { PipelineConfig, DiscoveryResult, TriageResult } from './knowledge/types.js'
...
import { reorganize, reorgPlan, reorgExecute } from './knowledge/phase-reorganize.js'
...
function parseArgs(): { phase: string | null; skipConfirmation: boolean } {
  ...
  if (phase && !['discover', 'crawl', 'triage', 'distill', 'reorganize', 'reorg-plan', 'reorg-execute'].includes(phase)) {
    console.error(`Unknown phase: ${phase}. Valid phases: discover, crawl, triage, distill, reorganize, reorg-plan, reorg-execute`)
    process.exit(1)
  }
...
function loadConfig(skipConfirmation: boolean): PipelineConfig {
  ...
  if (!confluenceEmail || !confluenceApiToken) {
    console.log('No CONFLUENCE_EMAIL/CONFLUENCE_API_TOKEN set — using anonymous access.')
  }
