"""Regenerate actual HQ no-matches actions, registration XForms and return frames from admitted Nova apps."""
import argparse
import hashlib
import json
import os
...
results=[]
for source in sorted(args.exports.glob("no-matches-*.json")):
    ...
    app._id="no-matches-evidence";app.version=1;app.custom_base_url="https://www.commcarehq.org"
    with ExitStack() as scope:
        ...
        form=app.get_module(len(app.modules)-1).get_form(0)
    ...
    for selector in ["detail/action","entry/stack","menu"]:
        assert [shape(e) for e in local.findall(selector)]==[shape(e) for e in native.findall(selector)],(source.stem,selector)
