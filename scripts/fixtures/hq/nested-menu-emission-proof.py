"""Regenerate actual HQ nested-menu actions, registration XForms and return frames from admitted Nova apps."""
import argparse
import hashlib
import json
import os
...
results=[]
mismatches=[]
for source in sorted(args.exports.glob("nested-menu-*.json")):
    ...
    app._id="nested-menu-evidence";app.version=1;app.custom_base_url="https://www.commcarehq.org"
    with ExitStack() as scope:
        ...
        xform=XForm(form.source,domain="test-domain");xform.add_case_and_meta(form);xform.strip_vellum_ns_attributes()
    ...
    for selector in ["entry","menu"]:
        def comparable(root):
                ...
                for node in element.iter():
                    if node.tag != "instance":
                        for value in node.attrib.values():
                            referenced.update(re.findall(r"instance\(['\"]([^'\"]+)['\"]\)",value))
            ...
            return items
        if comparable(local)!=comparable(native):mismatches.append((source.stem,selector))
...
refusals=json.loads((args.exports / "hq-nested-target-refusals.json").read_text())
assert {scenario for scenario, findings in refusals.items() if findings} == {"parent-multiple", "same-smaller"}, refusals
assert mismatches == [("nested-menu-parent-multiple", "entry"), ("nested-menu-same-smaller", "entry")], mismatches
print("Two deliberately bypassed HQ target refusals reproduce their native entry differences: " + str(mismatches))
print(json.dumps({"results":results,"hqCommit":subprocess.check_output(["git","-C",str(hq_root),"rev-parse","HEAD"],text=True).strip(),"limits":"Actual imported HQ model, native case/meta/Vellum XForm processing and suite contributors. Complete entry and menu trees retain all text/attributes/children. Supplied default build2.53/origin; enabled advanced search and follow-up case list forms. No real search/claim HTTP, whole HQ build or resource installation."},indent=2))
