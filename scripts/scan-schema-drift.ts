...
program
	...
	.addHelpText(
		...
		"\nDatabase:\n" +
			...
			"  Postgres, or the Cloud SQL connector env (see scripts/lib/prodDb.ts\n" +
			"  for the per-developer prod-read setup).\n" +
			"\nExamples:\n" +
			"  $ npx tsx scripts/scan-schema-drift.ts\n",
	);
program.parse();

function driftLines(drift: CaseTypeDrift): string {
	const parts: string[] = [];
	if (drift.missingRow)
	...
	if (drift.refined.length > 0)
		parts.push(`    spec refined (re-sync): ${drift.refined.join(", ")}`);
	...
	if (drift.unresolvable.length > 0)
		parts.push(
			`    ✗ UNRESOLVABLE stored spec (needs owner): ${drift.unresolvable.join(", ")}`,
		);
	return parts.join("\n");
}
