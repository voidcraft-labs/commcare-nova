...
import { z } from 'zod'
import type { PipelineConfig, DiscoveryResult, TriageResult, CrawledPage } from './types.js'
...
const tagClusterSchema = z.object({
  clusters: z.array(z.object({
    ...
    description: z.string().describe('1-2 sentence summary of what knowledge this cluster covers'),
    tags: z.array(z.string()).describe('Every tag string that belongs in this cluster'),
  })),
})
...
function assignPagesToClusters(
  ...
  if (orphaned > 0) {
    log('Distill', `  ${orphaned} pages had no matching tags — assigned to catch-all cluster`)
  }

  return clusterPages
}

export async function distill(config: PipelineConfig, discovery: DiscoveryResult, triageResult: TriageResult): Promise<void> {
  ...
  log('Distill', `${tagArray.length} unique tags across ${relevantEntries.length} pages`)
  ...
  const clusteringCost = (clusteringInputTokens / 1_000_000) * SONNET_INPUT_COST + (clusteringOutputTokens / 1_000_000) * SONNET_OUTPUT_COST

  const totalContentChars = relevantPages.reduce((sum, p) => sum + p.content.length, 0)
  const distillInputTokens = estimateTokens(totalContentChars)
  const distillOutputTokens = 12 * 3000
  const distillCost = (distillInputTokens / 1_000_000) * SONNET_INPUT_COST + (distillOutputTokens / 1_000_000) * SONNET_OUTPUT_COST

  const estCost = clusteringCost + distillCost

  log('Distill', 'Cost estimate:')
  log('Distill', `  Step 1 — Tag clustering (${tagArray.length} tag strings, 1 API call): ~$${clusteringCost.toFixed(2)} (~${clusteringInputTokens.toLocaleString()} input tokens)`)
  log('Distill', `  Step 2 — Distillation (full page content, ~12 separate API calls): ~$${distillCost.toFixed(2)} (~${distillInputTokens.toLocaleString()} input tokens)`)
  ...
  log('Distill', `  Sending tag array (~${estimateTokens(tagsInputChars).toLocaleString()} tokens) to Sonnet...`)
...
- EVERY tag in the input array MUST appear in exactly one cluster's tags array`,
  ...
  for await (const partial of clusterStream.partialOutputStream) {
    ...
    for (let c = 0; c < arr.length; c++) {
      ...
      const status = c < arr.length - 1 ? '\x1b[32m✓\x1b[0m' : '\x1b[33m⟳\x1b[0m'
      lines.push(`  ${status} ${(c + 1).toString().padStart(2)}. ${name.padEnd(50).slice(0, 50)} ${String(tags).padStart(4)} tags`)
    }
  ...
  totalCost += logCost('Distill', '  Clustering done', clusterUsage.inputTokens ?? 0, clusterUsage.outputTokens ?? 0, SONNET_INPUT_COST, SONNET_OUTPUT_COST)

  // Log clusters
  const assignedTags = new Set(tagClusters.flatMap(c => c.tags))
  const missingTags = tagArray.filter(t => !assignedTags.has(t))
  log('Distill', `  ${tagClusters.length} clusters defined, ${assignedTags.size}/${tagArray.length} tags assigned`)
  if (missingTags.length > 0) {
    log('Distill', `  WARNING: ${missingTags.length} tags not assigned: ${missingTags.slice(0, 10).join(', ')}${missingTags.length > 10 ? '...' : ''}`)
  }
  ...
  for (let i = 0; i < tagClusters.length; i++) {
    ...
    if (sourcePages.length === 0) {
      log('Distill', `  [${i + 1}/${tagClusters.length}] ${cluster.name} — no pages, skipping`)
      continue
    }
    ...
    for (const page of sourcePages) {
      const pageTokens = estimateTokens(page.content.length + page.title.length + 50)
      if (currentTokens + pageTokens > MAX_INPUT_TOKENS && currentBatch.length > 0) {
    ...
    log('Distill', `  [${i + 1}/${tagClusters.length}] ${cluster.name} — ${sourcePages.length} pages in ${batches.length} batch${batches.length > 1 ? 'es' : ''}`)
...
Format as clean markdown. Start with a level-1 heading matching the topic name. Use level-2 and level-3 headings to organize sections. Include code blocks for XPath patterns, XML snippets, and configuration examples where they add clarity.`
    ...
    for (let b = 0; b < batches.length; b++) {
      ...
      const sourceContent = batch.map(p =>
        `=== ${p.title} (${p.spaceKey}) ===\n\n${p.content}`
      ).join('\n\n---\n\n')

      const inputTokenEst = estimateTokens(sourceContent.length)
      const batchLabel = batches.length > 1 ? ` (batch ${b + 1}/${batches.length})` : ''
      log('Distill', `    ${batchLabel} ${batch.length} pages, ~${inputTokenEst.toLocaleString()} input tokens — sending...`)
      ...
      try {
        ...
        const batchCost = logCost('Distill', `    ${cluster.name}${batchLabel}`, usage.inputTokens ?? 0, usage.outputTokens ?? 0, SONNET_INPUT_COST, SONNET_OUTPUT_COST)
        totalCost += batchCost
    ...
    if (combined.length > 0) {
      ...
      log('Distill', `    Saved: ${outputPath} (${combined.length.toLocaleString()} chars)`)
    }
  }
  ...
  // Summary
  const generatedFiles = tagClusters.filter(c => fs.existsSync(path.join(DISTILL_DIR, `${c.filename}.md`)))
  ...
  logSummary('Distill', [
    ...
    '',
    'Files:',
    ...generatedFiles.map(c => `  ${c.filename}.md — ${c.name} (${clusterPages.get(c.name)?.length ?? 0} pages)`),
    '',
