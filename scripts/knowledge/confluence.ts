...
export class ConfluenceClient {
  ...
  constructor(baseUrl: string, email?: string, apiToken?: string, rateLimitMs = 100) {
  ...
  private async request<T>(url: string): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Confluence API ${res.status}: ${url} — ${text.slice(0, 200)}`)
    }
  ...
  async getPageContent(pageId: string): Promise<{ body: string; lastModified: string }> {
    await this.delay()
