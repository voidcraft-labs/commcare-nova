...
import { z } from 'zod'
import type { PipelineConfig, DiscoveryResult, CrawledPage, TriageEntry, TriageResult } from './types.js'
...
export async function triage(config: PipelineConfig, discovery: DiscoveryResult): Promise<TriageResult> {
  // Load crawled pages
  ...
  if (fs.existsSync(TRIAGE_PATH)) {
    const cached = JSON.parse(fs.readFileSync(TRIAGE_PATH, 'utf-8')) as TriageResult
    // If it was a complete run, return it
    if (cached.totalPages >= allPages.length) {
      log('Triage', `Found complete triage: ${cached.totalPages} pages, ${cached.passedPages} passed (6+)`)
      return cached
    }
    ...
    log('Triage', `Resuming: ${existingEntries.length}/${allPages.length} already triaged`)
  }
  ...
  // Cost estimate
  const totalContentChars = pages.reduce((sum, p) => sum + Math.min(p.content.length, MAX_CONTENT_CHARS), 0)
  ...
  const estCost = (estInputTokens / 1_000_000) * HAIKU_INPUT_COST + (estOutputTokens / 1_000_000) * HAIKU_OUTPUT_COST

  log('Triage', `Estimated cost: ~$${estCost.toFixed(2)} (${estInputTokens.toLocaleString()} input + ${estOutputTokens.toLocaleString()} output tokens)`)

  if (!config.skipConfirmation) {
    const ok = await confirm(`[Triage] Proceed with triaging ${pages.length} pages (~$${estCost.toFixed(2)})?`)
    if (!ok) {
...
- **reasoning**: 1 sentence explaining the score.`,
      ...
      for (const page of batch) {
        const classification = result.object.pages.find(c => c.pageId === page.id)
        if (classification) {
          entries.push({
            ...classification,
            title: page.title,
      ...
      totalCost += logCost('Triage', `Batch ${batchNum}/${totalBatches}`, usage.inputTokens ?? 0, usage.outputTokens ?? 0, HAIKU_INPUT_COST, HAIKU_OUTPUT_COST)
  ...
  for (const entry of entries) {
    relevanceDist.set(entry.relevance, (relevanceDist.get(entry.relevance) ?? 0) + 1)
  }
  ...
  logSummary('Triage', [
    ...
    '',
    'Relevance distribution:',
    ...[...relevanceDist.entries()].sort((a, b) => a[0] - b[0]).map(([score, count]) => `  ${score}: ${count} pages`),
    '',
    ...
    '',
    'Top-scoring pages:',
    ...entries.filter(e => e.relevance >= 8).sort((a, b) => b.relevance - a.relevance).slice(0, 15)
      .map(e => `  [${e.relevance}] ${e.title} (${e.spaceKey}) — ${e.topicTags.join(', ')}`),
  ])
