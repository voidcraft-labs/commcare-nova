...
import type { PipelineConfig, DiscoveryResult, SpaceInfo, PageMeta } from './types.js'
...
export async function discover(config: PipelineConfig): Promise<DiscoveryResult> {
  ...
  if (fs.existsSync(discoveryPath)) {
    ...
    const cached = JSON.parse(fs.readFileSync(discoveryPath, 'utf-8')) as DiscoveryResult
    log('Discover', `Cached: ${cached.spaces.length} spaces, ${cached.pages.length} pages`)
    return cached
  }
  ...
  for (const name of RELEVANT_SPACE_NAMES) {
    ...
    if (space) {
      ...
      log('Discover', `  Matched: "${name}" → key=${space.key}, id=${space.id}`)
    } else {
  ...
  if (relevantSpaces.length === 0) {
    throw new Error('No relevant spaces found. Check space names and permissions.')
  }
