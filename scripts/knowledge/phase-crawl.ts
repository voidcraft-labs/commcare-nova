...
export async function crawl(config: PipelineConfig, discovery: DiscoveryResult): Promise<void> {
  ...
  for (const space of discovery.spaces) {
    ...
    log('Crawl', `Space "${space.name}" (${space.key}): ${spacePages.length} pages`)
    let spaceDone = 0
    ...
    for (let i = 0; i < spacePages.length; i++) {
      ...
      if (fs.existsSync(cachePath)) {
        try {
          const cached = JSON.parse(fs.readFileSync(cachePath, 'utf-8')) as CrawledPage
      ...
      try {
        ...
        if (spaceDone % 50 === 0) {
          log('Crawl', `  Space ${space.key}: ${spaceDone}/${spacePages.length}`)
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err)
        failures.push({ id: page.id, title: page.title, error: msg })
        failed++
        log('Crawl', `  WARNING: Failed to fetch page "${page.title}" (${page.id}): ${msg}`)
      }
    }
  }
...
export function loadCrawledPages(discovery: DiscoveryResult): CrawledPage[] {
  ...
  for (const page of discovery.pages) {
    ...
    if (fs.existsSync(cachePath)) {
      try {
        const crawled = JSON.parse(fs.readFileSync(cachePath, 'utf-8')) as CrawledPage
