...
export function logCost(phase: Phase, label: string, inputTokens: number, outputTokens: number, costPerMInput: number, costPerMOutput: number) {
  const inputCost = (inputTokens / 1_000_000) * costPerMInput
  const outputCost = (outputTokens / 1_000_000) * costPerMOutput
  const totalCost = inputCost + outputCost
  console.log(`[${phase}] ${label}: ${inputTokens.toLocaleString()} in / ${outputTokens.toLocaleString()} out — $${totalCost.toFixed(4)}`)
  return totalCost
}
