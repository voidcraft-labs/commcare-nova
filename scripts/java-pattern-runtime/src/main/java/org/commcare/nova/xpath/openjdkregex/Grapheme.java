...
final class Grapheme {
    ...
    static int nextBoundary(CharSequence src, int off, int limit) {
        ...
        int ch0 = OpenJdkCharacter.codePointAt(src, off);
        int ret = off + OpenJdkCharacter.charCount(ch0);
        // indicates whether gb11 or gb12 is underway
        int t0 = getType(ch0);
        int riCount = t0 == RI ? 1 : 0;
        boolean gb11 = t0 == EXTENDED_PICTOGRAPHIC;
        while (ret < limit) {
            int ch1 = OpenJdkCharacter.codePointAt(src, ret);
            int t1 = getType(ch1);
            ...
            ret += OpenJdkCharacter.charCount(ch1);
        }
        return ret;
    }
    ...
    static int getType(int cp) {
        ...
        int type = OpenJdkCharacter.getType(cp);
        switch(type) {
        case OpenJdkCharacter.UNASSIGNED:
        ...
        case OpenJdkCharacter.CONTROL:
        case OpenJdkCharacter.LINE_SEPARATOR:
        case OpenJdkCharacter.PARAGRAPH_SEPARATOR:
        case OpenJdkCharacter.SURROGATE:
            return CONTROL;
        case OpenJdkCharacter.FORMAT:
            if (cp == 0x200C ||
            ...
            if (cp >= 0x0600 && cp <= 0x0605 ||
                ...
                return PREPEND;
            return CONTROL;
        case OpenJdkCharacter.NON_SPACING_MARK:
        case OpenJdkCharacter.ENCLOSING_MARK:
            // NOTE:
            ...
            return EXTEND;
        case  OpenJdkCharacter.COMBINING_SPACING_MARK:
            ...
            return SPACINGMARK;
        case OpenJdkCharacter.OTHER_SYMBOL:
            if (cp >= 0x1F1E6 && cp <= 0x1F1FF)
                return RI;
            return OTHER;
        case OpenJdkCharacter.MODIFIER_LETTER:
        case OpenJdkCharacter.MODIFIER_SYMBOL:
            // WARNING:
            ...
            if (cp == 0xFF9E || cp == 0xFF9F ||
                ...
                return EXTEND;
            return OTHER;
        case OpenJdkCharacter.OTHER_LETTER:
