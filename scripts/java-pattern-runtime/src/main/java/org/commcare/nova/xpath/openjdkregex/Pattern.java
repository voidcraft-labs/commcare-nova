...
import org.commcare.nova.xpath.BrowserNormalizer;
 ...
 *         ({@link java.lang.Character#MIN_CODE_POINT OpenJdkCharacter.MIN_CODE_POINT}
 *         &nbsp;&lt;=&nbsp;{@code 0x}<i>h...h</i>&nbsp;&lt;=&nbsp;
 *          {@link java.lang.Character#MAX_CODE_POINT OpenJdkCharacter.MAX_CODE_POINT})</td></tr>
 ...
 *     <td headers="matches java javaLowerCase">Equivalent to java.lang.OpenJdkCharacter.isLowerCase()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaUpperCase">{@code \p{javaUpperCase}}</th>
 *     <td headers="matches java javaUpperCase">Equivalent to java.lang.OpenJdkCharacter.isUpperCase()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaWhitespace">{@code \p{javaWhitespace}}</th>
 *     <td headers="matches java javaWhitespace">Equivalent to java.lang.OpenJdkCharacter.isWhitespace()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaMirrored">{@code \p{javaMirrored}}</th>
 *     <td headers="matches java javaMirrored">Equivalent to java.lang.OpenJdkCharacter.isMirrored()</td></tr>
 *
 ...
 * {@link java.lang.Character#codePointOf(String) OpenJdkCharacter.codePointOf(name)}.
 * <p>
 ...
 * {@link java.lang.OpenJdkCharacter.UnicodeScript#forName(String) UnicodeScript.forName}.
 *
 * <p>
 ...
 * accepted and defined by
 * {@link java.lang.OpenJdkCharacter.UnicodeBlock#forName(String) UnicodeBlock.forName}.
 * <p>
 *
    ...
    implements java.io.Serializable
{
    ...
    private static int newArrayLength(int oldLength, int minGrowth, int prefGrowth) {
        int prefLength = oldLength + Math.max(minGrowth, prefGrowth);
        if (0 < prefLength && prefLength <= Integer.MAX_VALUE - 8) return prefLength;
        int minLength = oldLength + minGrowth;
        if (minLength < 0) throw new OutOfMemoryError("Required array length is too large");
        return minLength <= Integer.MAX_VALUE - 8 ? Integer.MAX_VALUE - 8 : minLength;
    }
    ...
    private static String[] producePermutations(String input) {
        ...
        if (input.length() == countChars(input, 0, 2)) {
            int c0 = OpenJdkCharacter.codePointAt(input, 0);
            int c1 = OpenJdkCharacter.codePointAt(input, OpenJdkCharacter.charCount(c0));
        ...
        for(int x=0, i=0; x<nCodePoints; x++) {
            int c = OpenJdkCharacter.codePointAt(input, i);
            combClass[x] = getClass(c);
            i +=  OpenJdkCharacter.charCount(c);
        }
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private void compile() {
        ...
        for (int x = 0; x < patternLength; x += OpenJdkCharacter.charCount(c)) {
    ...
    private static final boolean isSupplementary(int ch) {
        return ch >= OpenJdkCharacter.MIN_SUPPLEMENTARY_CODE_POINT ||
               OpenJdkCharacter.isSurrogate((char)ch);
    }
    ...
    private CharPredicate single(final int ch) {
        if (has(CASE_INSENSITIVE)) {
            int lower, upper;
            if (has(UNICODE_CASE)) {
                upper = OpenJdkCharacter.toUpperCase(ch);
                lower = OpenJdkCharacter.toLowerCase(upper);
    ...
    private CharPredicate family(boolean singleLetter, boolean isComplement) {
        ...
        if (singleLetter) {
            int c = temp[cursor];
            if (!OpenJdkCharacter.isSupplementaryCodePoint(c)) {
    ...
    private int x() {
        ...
        if (ASCII.isHexDigit(n)) {
            ...
            while (ASCII.isHexDigit(n = read())) {
                ...
                if (ch > OpenJdkCharacter.MAX_CODE_POINT)
    ...
    private int u() {
        int n = uxxxx();
        if (OpenJdkCharacter.isHighSurrogate((char)n)) {
            int cur = cursor();
            if (read() == '\\' && read() == 'u') {
                int n2 = uxxxx();
                if (OpenJdkCharacter.isLowSurrogate((char)n2))
                    return OpenJdkCharacter.toCodePoint((char)n, (char)n2);
            }
            setcursor(cur);
        }
        return n;
    ...
    private int N() {
        if (read() == '{') {
            ...
            try {
                return OpenJdkCharacter.codePointOf(name);
    ...
    private static final int countChars(CharSequence seq, int index,
        ...
        if (lengthInCodePoints == 1 && index >= 0 && index < seq.length() &&
            !OpenJdkCharacter.isHighSurrogate(seq.charAt(index))) {
            return 1;
        }
        ...
        if (lengthInCodePoints >= 0) {
            ...
            for (int i = 0; x < length && i < lengthInCodePoints; i++) {
                if (OpenJdkCharacter.isHighSurrogate(seq.charAt(x++))) {
                    if (x < length && OpenJdkCharacter.isLowSurrogate(seq.charAt(x))) {
                        x++;
                    }
                }
            }
        ...
        for (int i = 0; x > 0 && i < len; i++) {
            if (OpenJdkCharacter.isLowSurrogate(seq.charAt(--x))) {
                if (x > 0 && OpenJdkCharacter.isHighSurrogate(seq.charAt(x-1))) {
                    x--;
                }
            }
        }
    ...
    private static final int countCodePoints(CharSequence seq) {
        ...
        for (int i = 0; i < length; ) {
            n++;
            if (OpenJdkCharacter.isHighSurrogate(seq.charAt(i++))) {
                if (i < length && OpenJdkCharacter.isLowSurrogate(seq.charAt(i))) {
                    i++;
                }
            }
        }
        ...
        BitClass add(int c, int flags) {
            ...
            if ((flags & CASE_INSENSITIVE) != 0) {
                if (ASCII.isAscii(c)) {
                    ...
                    bits[OpenJdkCharacter.toLowerCase(c)] = true;
                    bits[OpenJdkCharacter.toUpperCase(c)] = true;
                }
            }
            bits[c] = true;
            return this;
        ...
        if (has(CASE_INSENSITIVE)) {
            if (has(UNICODE_CASE)) {
                for (int i = 0; i < count; i++) {
                    tmp[i] = OpenJdkCharacter.toLowerCase(
                                 OpenJdkCharacter.toUpperCase(buf[i]));
                }
    ...
    static final class StartS extends Start {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            while (i <= guard) {
                ...
                if (OpenJdkCharacter.isHighSurrogate(seq.charAt(i++))) {
                    if (i < seq.length() &&
                        OpenJdkCharacter.isLowSurrogate(seq.charAt(i))) {
                        i++;
                    }
                }
            }
    ...
    static class CharProperty extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                int ch = OpenJdkCharacter.codePointAt(seq, i);
                i += OpenJdkCharacter.charCount(ch);
                if (i <= matcher.to) {
    ...
    private static class NFCCharProperty extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                int ch0 = OpenJdkCharacter.codePointAt(seq, i);
                int n = OpenJdkCharacter.charCount(ch0);
                ...
                if (i + n == j) { // single cp grapheme, assume nfc
                    ...
                    while (i + n < j) {
                        String nfc = BrowserNormalizer.normalizeNfc(
                            seq.toString().substring(i, j));
                        ...
                        ch0 = OpenJdkCharacter.codePointBefore(seq, j);
                        j -= OpenJdkCharacter.charCount(ch0);
                    }
                }
                if (j < matcher.to)
                    return false;
    ...
    static class GraphemeBound extends Node {
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i == startIndex) {
                ...
                if (OpenJdkCharacter.isSurrogatePair(seq.charAt(i - 1), seq.charAt(i))) {
                    return false;
                }
    ...
    static final class SliceU extends SliceNode {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            for (int j=0; j<len; j++) {
                ...
                int c = seq.charAt(i+j);
                if (buf[j] != c &&
                    buf[j] != OpenJdkCharacter.toLowerCase(OpenJdkCharacter.toUpperCase(c)))
                    return false;
            }
            return next.match(matcher, i+len, seq);
        }
    ...
    static final class SliceS extends Slice {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            for (int j = 0; j < buf.length; j++) {
                ...
                int c = OpenJdkCharacter.codePointAt(seq, x);
                if (buf[j] != c)
                    return false;
                x += OpenJdkCharacter.charCount(c);
                if (x > matcher.to) {
                    matcher.hitEnd = true;
                    return false;
                }
    ...
    static final class SliceUS extends SliceIS {
        ...
        int toLower(int c) {
            return OpenJdkCharacter.toLowerCase(OpenJdkCharacter.toUpperCase(c));
        }
    }
    ...
    static class CharPropertyGreedy extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            while (i < to) {
                int ch = OpenJdkCharacter.codePointAt(seq, i);
                int len = OpenJdkCharacter.charCount(ch);
                if (i + len > to) {
            ...
            while (n >= cmin) {
                ...
                int ch = OpenJdkCharacter.codePointBefore(seq, i);
                // check if the region cut off the low half of a surrogate pair
                i = Math.max(starti, i - OpenJdkCharacter.charCount(ch));
                n--;
            }
            return false;
        }
    ...
    static class CIBackRef extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            int x = i;
            for (int index=0; index<groupSize; index++) {
                int c1 = OpenJdkCharacter.codePointAt(seq, x);
                int c2 = OpenJdkCharacter.codePointAt(seq, j);
                if (c1 != c2) {
                    if (doUnicodeCase) {
                        int cc1 = OpenJdkCharacter.toUpperCase(c1);
                        int cc2 = OpenJdkCharacter.toUpperCase(c2);
                        if (cc1 != cc2 &&
                            OpenJdkCharacter.toLowerCase(cc1) !=
                            OpenJdkCharacter.toLowerCase(cc2))
                            return false;
                    } else {
                ...
                x += OpenJdkCharacter.charCount(c1);
                j += OpenJdkCharacter.charCount(c2);
            }
    ...
    static final class Bound extends Node {
        ...
        boolean isWord(int ch) {
            return useUWORD ? CharPredicates.WORD().is(ch)
                            : (ch == '_' || OpenJdkCharacter.isLetterOrDigit(ch));
        }
        ...
        int check(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i > startIndex) {
                ch = OpenJdkCharacter.codePointBefore(seq, i);
                left = (isWord(ch) ||
                    ((OpenJdkCharacter.getType(ch) == OpenJdkCharacter.NON_SPACING_MARK)
                     && hasBaseCharacter(matcher, i-1, seq)));
            }
            boolean right = false;
            if (i < endIndex) {
                ch = OpenJdkCharacter.codePointAt(seq, i);
                right = (isWord(ch) ||
                    ((OpenJdkCharacter.getType(ch) == OpenJdkCharacter.NON_SPACING_MARK)
                                            ...
                                            CharSequence seq)
    {
        ...
        for (int x=i; x >= start; x--) {
            int ch = OpenJdkCharacter.codePointAt(seq, x);
            if (OpenJdkCharacter.isLetterOrDigit(ch))
                return true;
            if (OpenJdkCharacter.getType(ch) == OpenJdkCharacter.NON_SPACING_MARK)
                continue;
            return false;
        }
        return false;
...
NEXT:       while (i <= last) {
    ...
    static final class BnMS extends BnM {
        ...
        BnMS(int[] src, int[] lastOcc, int[] optoSft, Node next) {
            ...
            for (int cp : buffer) {
                lengthInChars += OpenJdkCharacter.charCount(cp);
            }
        }
        boolean match(Matcher matcher, int i, CharSequence seq) {
            int[] src = buffer;
...
NEXT:       while (i <= last) {
                ...
                for (int j = countChars(seq, i, patternLength), x = patternLength - 1;
                     j > 0; j -= OpenJdkCharacter.charCount(ch), x--) {
                    ch = OpenJdkCharacter.codePointBefore(seq, i+j);
                    if (ch != src[x]) {
                        // Shift search to the right by the maximum of the
                        // bad character shift and the good suffix shift
    ...
    static CharPredicate SingleU(int lower) {
        return ch -> lower == ch ||
                     lower == OpenJdkCharacter.toLowerCase(OpenJdkCharacter.toUpperCase(ch));
    }
    ...
    static CharPredicate Range(int lower, int upper) {
        if (upper < OpenJdkCharacter.MIN_HIGH_SURROGATE ||
            lower > OpenJdkCharacter.MAX_LOW_SURROGATE &&
            upper < OpenJdkCharacter.MIN_SUPPLEMENTARY_CODE_POINT)
    ...
    static CharPredicate CIRangeU(int lower, int upper) {
        return ch -> {
            if (inRange(lower, ch, upper))
                return true;
            int up = OpenJdkCharacter.toUpperCase(ch);
            return inRange(lower, up, upper) ||
                   inRange(lower, OpenJdkCharacter.toLowerCase(up), upper);
        };
    }
