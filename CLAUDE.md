...
**Multi-container: nginx h2c ingress + Next app sidecar.** The service runs two containers (`service.yaml`): an `nginx` ingress container whose port is named `h2c`, and the Next `app` sidecar on the loopback (`PORT=8888`). Cloud Run terminates TLS and forwards HTTP/2 cleartext to nginx — which is what lifts Cloud Run's 32 MiB HTTP/1 request cap so a media upload's bytes can reach the app (uploads stream through `/api/media/upload/bytes`, not a cross-origin GCS PUT). nginx proxies HTTP/1.1 to the app; `container-dependencies` starts the app first. `cloudbuild.yaml` builds both images (`Dockerfile` + `docker/nginx.Dockerfile`) tagged with the commit and swaps each container's image with `gcloud run services update --container … --image …` — the per-commit tag goes on the command line, not into the YAML (`services replace` reads `service.yaml` literally and never substitutes a tag). **`service.yaml` is the full-spec source of truth** for env/secrets/scaling/ports/probes; routine deploys swap only images, so config set there persists — change it there, not in the console, and `services replace` it once (placeholders rendered) when config changes.
