...
**Navigation is URL-owned.** The URL on `/build/[id]` is the sole source of truth for screen + selection. Path-based schema: `/build/[id]` (home), `/build/[id]/{moduleUuid}` (module), `/build/[id]/{formUuid}` (form), `/build/[id]/{formUuid}/{questionUuid}` (form + selection), `/build/[id]/{moduleUuid}/cases` (case list), `/build/[id]/{moduleUuid}/cases/{caseId}` (case detail). All entity UUIDs are globally unique — a single UUID segment identifies the entity type by checking `doc.modules`, `doc.forms`, `doc.questions`. Navigation uses the browser History API (`pushState`/`replaceState`) via `lib/routing/useClientPath.ts` instead of Next.js's router to avoid server-side RSC re-renders. `useLocation()` parses `useBuilderPathSegments()` + doc state into a discriminated `Location` union (`home | module | cases | form`). `useNavigate()` returns a stable action bag (`goHome`, `openModule`, `openCaseList`, `openCaseDetail`, `openForm`, `back`, `up`) — screen changes go through `pushState` so back/forward traverse them. `useSelect()` flips the selection segment via `replaceState` (no history entry) and honors `useConsultEditGuard()` from `EditGuardContext` so inline editors with unsaved invalid content can block the transition. `useBreadcrumbs()` derives from location + doc entities with shallow-stable selectors.

**URL validation.** `LocationRecoveryEffect` (`components/builder/LocationRecoveryEffect.tsx`) subscribes to doc entity maps and runs two recovery strategies: (1) `recoverLocation` strips stale UUIDs from the parsed location, (2) URL-mismatch detection compares the current path segments against the canonical serialization of the parsed location (catches cases where `parsePathToLocation` already degraded unresolvable UUIDs at parse time but the browser URL still shows the old path). Both strategies issue `replaceState` when a fix is needed. No server-side URL validation — the RSC page (`app/build/[id]/[[...path]]/page.tsx`) ignores the `path` param entirely.
...
**Selection changes re-render only the 2 affected wrappers** (old + new) via `useIsQuestionSelected(uuid)` — each wrapper receives its own identity, only the matching booleans flip. Nuance: because `useIsQuestionSelected` / `useIsModuleSelected` / `useIsFormSelected` call `useLocation()` (which subscribes to `useBuilderPathSegments()` via `useSyncExternalStore`), every consumer re-runs its hook body on any URL change. The boolean return still prevents child reconciliation for non-matching rows, so the net cost is just the component function call. Phase 5's virtualization bounds the consumer count to visible rows. Components that only dispatch actions (mutations, scroll, energy) read the store API via `useBuilderStoreApi()` or scoped context hooks with zero re-render cost.
