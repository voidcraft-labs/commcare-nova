...
- Per-form `connect` rides the creation tools (`createModule` / `createForm`) and the `planAppDesign` plan, so a Connect form lands WITH its block. Validator's `CONNECT_FORM_MISSING_BLOCK` fires on Connect-typed apps where a form has no connect block — and because the commit gate rejects introducing it, a Connect app's forms always carry blocks: enable Connect before creating modules (`updateApp`), or give every existing form its block first and flip last.
