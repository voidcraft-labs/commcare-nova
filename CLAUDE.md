...
Client-side errors (uncaught JS errors, unhandled promise rejections, React rendering crashes) are captured and sent to the server for ingestion into GCP Cloud Logging alongside server errors.
...
- **Filtering** — in Cloud Logging, `origin=client` labels distinguish client errors from server errors. The `source` label (`window.onerror`, `unhandledrejection`, `error-boundary`) identifies the capture point.
