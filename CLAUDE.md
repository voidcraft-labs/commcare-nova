...
- **Legacy builder store** (`lib/services/builderStore.ts`) — the shrinking session/lifecycle store. Phase 3 deleted the `BuilderEngine` wrapper class and the entity-mirror adapter; what's left is `phase`, `agentActive`, `postBuildEdit`, `generationData`, `appId`, and the replay metadata. Phase 4 migrates generation + replay into a mutation stream and Phase 6 deletes this file entirely.
- **BuilderSession store** (`lib/session/store.ts`) — ephemeral UI state: `cursorMode`, sidebar visibility + stash, active field id, connect stash, focus hint, new-question marker. Mutated via `lib/session/hooks.tsx` (`useSwitchCursorMode`, `useSetSidebarOpen`, `useSetFocusHint`, etc.).

**Provider stack.** `BuilderProvider` (`hooks/useBuilder.tsx`) mounts a stack of small capability providers — no `BuilderEngine` class anymore. Order (outer → inner): `StoreContext` → `BlueprintDocProvider` → `BuilderSessionProvider` → `ScrollRegistryProvider` → `EditGuardProvider` → `BuilderFormEngineProvider`. A tiny `SyncBridge` installs the doc store reference on the legacy and session stores so non-React callers (generation-stream setters, `switchConnectMode`) can reach it. DOM helpers that used to hang off the engine (`findFieldElement`, `flashUndoHighlight`) live in `lib/routing/domQueries.ts`; the reset routine moved to `lib/services/resetBuilder.ts`. Each responsibility lives in the smallest scope that owns it.
...
**Hook API** (`hooks/useBuilder.tsx` — legacy store; `lib/routing/hooks.tsx` — URL):
- `useBuilderStore(selector)` / `useBuilderStoreShallow(selector)` — reactive subscriptions to the legacy store (`Object.is` / shallow equality).
- `useBuilderStoreApi()` — imperative handle on the legacy store; read/write via `storeApi.getState()` without subscribing. Use for effect-time snapshots and callback closures.
- Convenience hooks (legacy store): `useBuilderPhase()`, `useBuilderIsReady()`, `useBuilderHasData()`, `useBuilderInReplayMode()`, `useBuilderTreeData()`, `useBuilderAgentActive()`. (Cursor mode moved to `useCursorMode()` in `lib/session/hooks`.)
...
**Selector naming convention** in `builderSelectors.ts`: `select*` functions return primitives or Immer-stable references — safe to pass directly to `useBuilderStore(selectFoo)`. `deriveTreeData` constructs new object trees via `.map()` and MUST be wrapped in `useMemo` by its consuming hook (`useBuilderTreeData`), never passed to `useBuilderStore` — `Object.is` on a fresh object triggers an infinite re-render loop. Breadcrumb derivation is URL-driven and lives in `useBreadcrumbs` (`lib/routing/hooks.tsx`), not in this file.
...
**Undo tracking is paused during hydration.** The doc store's temporal middleware is paused while the store is populated from `loadApp` or replay stages so the empty→populated transition never enters history. `BlueprintDocProvider` resumes tracking after synchronous hydration via `startTracking={true}` for loaded apps; generation resumes on `data-done`. New builds start with tracking paused (doc is empty) until the first blueprint lands. Do not remove the pause/resume calls; without them the first undo restores a blank state.

**subscribeMutation** is owned by `BuilderReferenceProvider` (Fix Commit 2): it subscribes to the legacy store's `[questions, modules, forms]` tuple with a reference-equality comparator and forwards invalidations to `ReferenceProviderWrapper`. Lives there so the reference cache flushes whenever any entity map gets a new Immer reference, without adding a store subscription to `BuilderLayout`.

**Builder initial phase.** `BuilderProvider` in `hooks/useBuilder.tsx` starts the legacy store with `BuilderPhase.Loading` when `initialBlueprint` or `replay` is provided, `Idle` otherwise. For existing apps, the RSC page (`app/build/[id]/page.tsx`) fetches the blueprint server-side and passes it to `BuilderProvider` — a mount-time effect calls `loadApp()` synchronously, transitioning to `Ready` before the first paint. `Loading` is a safe fallback: if any frame paints before the effect runs, BuilderLayout shows the loading skeleton instead of the centered chat. `loading.tsx` at `app/build/[id]/` covers the server-side wait (auth + Firestore read). New builds start in `Idle` for chat-driven generation.
