...
**Multi-container: nginx h2c ingress + Next app sidecar.** The service runs two containers (`service.yaml`): an `nginx` ingress container whose port is named `h2c`, and the Next `app` sidecar on the loopback (`PORT=8888`). Cloud Run terminates TLS and forwards HTTP/2 cleartext to nginx — which is what lifts Cloud Run's 32 MiB HTTP/1 request cap so a media upload's bytes can reach the app (uploads stream through `/api/media/upload/bytes`, not a cross-origin GCS PUT). nginx proxies HTTP/1.1 to the app; `container-dependencies` starts the app first. `cloudbuild.yaml` builds both images (`Dockerfile` + `docker/nginx.Dockerfile`) and deploys via `gcloud run services replace service.yaml`, so **`service.yaml` is the source of truth for the whole service spec** — change env/secrets/scaling there, not in the console.
