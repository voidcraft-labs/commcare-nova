...
- **Failure detection:** Two-layer system ensures failed projects never stay stuck in `generating`. Layer 1 (server catch): route handler catch blocks call `failProject()` with the classified error type — fire-and-forget. Layer 2 (timeout inference): `listProjects()` checks `created_at` age — any project still `generating` after `MAX_GENERATION_MINUTES` (10 min, well above the 5 min `maxDuration` route timeout) is inferred as failed, returned as `status: 'error'`, and lazily persisted via `failProject()`. The `error_type` field on `ProjectDoc` stores the `ErrorType` string from the classifier (or `'internal'` for timeout-inferred failures).
- **Auto-save:** `useAutoSave` hook subscribes to `builder.subscribeMutation` and uses a leading+trailing throttle (1s cooldown) to `PUT /api/projects/{id}`. First mutation after quiet time saves immediately (leading edge); rapid subsequent mutations batch into a single trailing save when the cooldown expires. Returns a `SaveState` (`{ status, savedAt }`) consumed by `SaveIndicator` in the builder subheader — "Saving…" only appears when a fetch is actually in-flight, then a persistent relative timestamp ("Saved just now", "Saved 2m ago") so the user always knows their work is persisted. Error state surfaces as "Save failed" with `role="alert"`. Save failures are reported to GCP Cloud Logging via `reportClientError()` with the HTTP status and server error message (or network error details) so failures are diagnosable — a Firestore outage does not interrupt editing.
...
- **Cost tracking:** Step events carry `TokenUsage` (model, tokens, cost). EventLogger accumulates request-level cost across all steps and flushes a single `incrementUsage` write in `finalize()`. The route awaits `finalize()` in the execute `finally` block (primary path), with `onFinish` and `req.signal.abort` as idempotent safety nets — the `_finalized` guard ensures exactly one write.
...
- **Post-request cost flush:** EventLogger accumulates cost across all agent steps (outer step + inner tool sub-generations) in private fields. `finalize()` awaits a single atomic `incrementUsage` call using `FieldValue.increment()` — concurrent-safe, creates the document automatically on the first request of a new month via `set({ merge: true })`. Single attempt, throws on failure — consistent with all other Firestore writes. If the write fails, the next request's pre-flight `getMonthlyUsage()` read will also fail (same Firestore outage), returning 503 and blocking the user.
- **Cancellation safety:** The route awaits `logger.finalize()` in the execute `finally` block (primary path), with `onFinish` and `req.signal.abort` as idempotent safety nets. The `_finalized` guard ensures exactly one write regardless of which fires.
- **Cap configuration:** `MONTHLY_SPEND_CAP_USD` reads from env var, defaults to $30. Applies globally to all authenticated users.

### Key Classes

- **Builder** (`lib/services/builder.ts`) — state machine scoped to the build page via `BuilderProvider` (`hooks/useBuilder.tsx`). `BuilderProvider` creates a fresh Builder per `buildId` and handles project loading from Firestore. `useBuilder()` reads from React context. When `buildId` changes (navigation between projects), the provider creates a new Builder — old one is garbage collected. When the user leaves `/build/*`, the provider unmounts and the Builder is gone. Phases: `Idle → Loading → Generating → Ready`. Generation progress is tracked separately via `GenerationStage` (`DataModel`, `Structure`, `Modules`, `Forms`, `Validate`, `Fix`) — metadata only meaningful during `phase === Generating`. Errors during generation are metadata (`GenerationError: { message, severity }`) on the `Generating` phase, not a separate phase. Holds a persistent `MutableBlueprint` instance. Exposes `subscribe` + `getSnapshot` for `useSyncExternalStore`. Tracks `projectId` for Firestore persistence.
...
- **External store subscription** — `useBuilder()` and `useFormEngine()` use `useSyncExternalStore` with versioned snapshots. No useState/useEffect for subscription. `getServerSnapshot` must return a **cached** (module-level) value — returning a new object each call causes infinite loops. `useBuilder()` reads the Builder from `BuilderContext` (provided by `BuilderProvider` in the build page), not from a module-level singleton. `useBuilderInstance()` reads the same context without subscribing — use when a component only needs imperative builder methods (e.g. `setEditGuard`) and does not read reactive state.
...
- **Error boundaries** — Route-level (`app/error.tsx`, `app/build/[id]/error.tsx`) and component-level (`ErrorBoundary` wrapping ChatSidebar, PreviewShell, ContextualEditor). All three report caught errors to the server via `reportClientError()` from `lib/clientErrorReporter.ts` — route-level via `useEffect`, class component via `componentDidCatch`. Route-level error boundaries use `window.location.href` for navigation (not `router.push`) because React's tree is in an error state and can't handle client-side transitions.

### Error Handling

End-to-end error system: API/stream errors are classified (`lib/services/errorClassifier.ts`), logged to the event stream (`EventLogger.logError()`), emitted to the client as `data-error` parts, and surfaced via toast notifications (`lib/services/toastStore.ts` → `ToastContainer`). Error types: `api_auth`, `api_rate_limit`, `api_overloaded`, `api_timeout`, `api_server`, `model_error`, `stream_broken`, `spend_cap_exceeded`, `internal`. The `MESSAGES` record (exported from `errorClassifier.ts`) maps each type to a user-facing string. The signal grid has two error modes: `error-recovering` (reasoning with warm-hued cells) and `error-fatal` (flicker settling into dim rose-pink pulse). `GenerationProgress` shows which stage failed with rose indicators. Errors during generation are metadata on the `Generating` phase — `builder.generationError` returns `{ message, severity: 'recovering' | 'failed' } | null`. Phase stays `Generating`; error state is set via `builder.setGenerationError(msg, severity)`. The route handler uses a manual reader loop (not `writer.merge()`) so stream errors can be caught and emitted before the stream closes. Fallback: if the writer is broken, `useChat`'s error property fires a toast on the client. `parseApiErrorMessage()` in `lib/apiError.ts` extracts the `error` field from JSON error responses so toast messages are human-readable.

### Structured Logging (`lib/log.ts`)

All server-side logging uses the structured logger (`log.info`, `log.warn`, `log.error`, `log.critical`) instead of raw `console.error`. In production (Cloud Run), emits single-line JSON to stdout/stderr with fields Cloud Logging auto-parses: `severity` (maps to Cloud Logging levels), `message`, `stack_trace` (feeds GCP Error Reporting for grouping), `time` (ISO 8601), and `logging.googleapis.com/labels` (custom key-value pairs for filtering). In local dev (`NODE_ENV !== 'production'`), falls back to `console.*` for readable terminal output. ERROR/CRITICAL severity writes to stderr; INFO/WARNING to stdout — matching Cloud Run's default severity mapping.

### Client Error Reporting

Client-side errors (uncaught JS errors, unhandled promise rejections, React rendering crashes) and operational failures (auto-save errors) are captured and sent to the server for ingestion into GCP Cloud Logging alongside server errors.

- **ErrorReporter** (`components/ErrorReporter.tsx`) — client component mounted once in the root layout. Attaches `window 'error'` and `'unhandledrejection'` listeners. Pure side effect, renders nothing.
- **Error boundaries** — `app/error.tsx`, `app/build/[id]/error.tsx`, and `components/ui/ErrorBoundary.tsx` all report caught errors on mount/catch.
- **Transport** (`lib/clientErrorReporter.ts`) — `reportClientError()` sends payloads to `POST /api/log/error` via `navigator.sendBeacon` (survives page unloads). Client-side dedup (Set of fingerprints) + rate limiting (max 10 unique errors per page load) prevents crash loop floods.
- **Endpoint** (`app/api/log/error/route.ts`) — no auth required (errors happen before/during auth). Zod-validated, sanitized (max 2KB message, 8KB stack). Logs via the structured logger with `origin: 'client'` label for filtering. Returns 204.
- **Auto-save reporting** — `useAutoSave` calls `reportClientError()` on every save failure with `source: 'manual'`. HTTP errors include status code and the server's error message (extracted from the JSON response body). Network errors include the exception message and stack trace.
- **Filtering** — in Cloud Logging, `origin=client` labels distinguish client errors from server errors. The `source` label (`window.onerror`, `unhandledrejection`, `error-boundary`, `manual`) identifies the capture point.
...
**Global header** — `AppHeader` (`components/ui/AppHeader.tsx`, client component) is rendered once in the root layout (`app/layout.tsx`). The root layout resolves `getSession()` and passes `isAdmin` as a prop. `AppHeader` uses `usePathname()` for route-aware behavior: returns null on landing (`/`), CSS grid collapse controlled by `headerVisibility` store on builder (`/build/*`), always visible elsewhere. Layout: `[Logo] ... [Projects] [Admin?] [AccountMenu]`. On builder routes, the Logo is wrapped in `motion.div layoutId="nova-logo"` for shared layout animation with the builder's centered hero Logo. `HeaderNav` (`components/ui/HeaderNav.tsx`, client component) renders the right-side nav links with active state via `usePathname()` and `AccountMenu`. Nav items are a fixed set defined in `HeaderNav`: Projects (`/builds`, folder icon) and Admin (`/admin`, user-shield icon, admin-only). Active state uses pathname prefix matching. `isAdmin` flows as a prop from the root layout (where the session is already resolved) to avoid client session fetches and hydration flicker.

**Header visibility store** — `lib/stores/headerVisibility.ts` is a module-level store coordinating the global `AppHeader` with `BuilderLayout`. The builder calls `setHeaderVisible(false)` in centered/hero mode and `setHeaderVisible(true)` when generation starts. `AppHeader` subscribes via `useSyncExternalStore`. The `useLayoutEffect` in BuilderLayout ensures the store update fires before paint so React batches the header reveal into the same frame as the hero Logo unmount — enabling smooth Motion `layoutId` animation. On non-builder routes the store is ignored (header always visible).
...
Admin-only dashboard at `/admin` for org-wide visibility into usage and user activity. Protected by `app/admin/layout.tsx` — an async Server Component that calls `requireAdminAccess()` and redirects non-admins before any HTML is sent. The layout is a pure auth gate (no header rendering — the global header is in the root layout). All pages under `/admin/*` inherit this gate and can assume admin access without self-gating. Server-side: all admin API routes use `requireAdmin()` from `lib/auth-utils.ts`.

- **Dashboard page** (`/admin`): Suspense-streamed headline stat cards (total users, generations this month, total spend) + sortable user table via `@tanstack/react-table` with live search. Columns: user (avatar + name), email, role, projects, generations, spend, last active. Click a row → user profile. Table supports keyboard navigation (`tabIndex`, `role="link"`, Enter/Space).
- **User profile** (`/admin/users/[email]`): In-page breadcrumb (chevron-left back to Admin + email). Three independent Suspense streams: profile card, usage history table (per-month breakdown), and project list with replay buttons. Data functions split in `lib/db/admin.ts` (`getAdminUserProfile`, `getAdminUserUsage`, `getAdminUserProjects`) for granular streaming. Replay uses the shared `useReplay` hook with the admin log endpoint.
