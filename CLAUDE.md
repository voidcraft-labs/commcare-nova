...
- **`lib/media`** — asset validation, the attach- and export-time verdicts, the export budget, the wire manifest, and the deletion guard. Bytes live in GCS (`lib/storage`), a status row in Firestore (`lib/db/mediaAssets`). Media is shared at **Project** scope with no byte migration: an app's media lives in the app *owner's* namespace (where compile resolves it), the web routes scope app-context operations by `app.owner` (`resolveMediaOwner`), and reads authorize by Project-membership-with-owner (`usersShareAnyProject`) — see its `CLAUDE.md`.
