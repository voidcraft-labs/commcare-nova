...
### Valid by construction — the validity gates

Every committed mutation batch is gated BEFORE persistence/dispatch by `evaluateCommit` (`lib/commcare/validator/gate.ts`) via the shared verdict `lib/doc/commitVerdicts.ts::mutationCommitVerdict`: a batch may not introduce a validator finding of a gating class — soundness always; completeness deferred while the app is still building (`commitPhase: "building"`), ratcheted once complete (an edit may never take a complete entity incomplete). Pre-existing findings in legacy docs never block an unrelated edit (introduced-error identity diffing). One verdict, every commit surface: the shared tool layer covers chat-SA and MCP alike (`lib/agent/tools/common.ts::guardedMutate` — a rejected call returns the findings in its `{ error }` envelope and writes nothing), and the builder UI gates at the dispatch hook (`useBlueprintMutations` — a rejected edit never dispatches and surfaces as an error toast; undo/redo, hydration, the agent stream, and replay bypass the gate because they replay already-committed states). ONE live user-edit path stays deliberately ungated pending an owner decision: the session store's `switchConnectMode` (the app-level Connect toggle) dispatches `applyMany` directly — enabling Connect necessarily introduces the per-form `CONNECT_FORM_MISSING_BLOCK` completeness findings (no form has a block before the mode exists), so gating it naively would make Connect un-enableable on complete apps. The export boundary still rejects the incomplete state; the right construction shape (seed/restore every form's block in the enabling batch) needs the owner's call on Connect semantics. Exports are zero-tolerance: `collectBoundaryViolations` runs the full validator (media manifest included) at every compile/upload entry point and ANY finding rejects with per-finding prose — an invalid app never reaches a device or CommCare HQ. **Structural creation is atomic** (`createForm` requires its `fields`; `createModule` takes `forms` + `case_list_columns`) so the completeness ratchet never dead-ends growth on a complete app. **The lifecycle**: chat builds run under `status: "generating"`, MCP builds under `status: "draft"` (reaper-exempt — an external agent has no liveness timer), and a failed build's `"error"` re-enters the construction window on retry; all three gate as `building` (`commitPhaseForAppStatus`). The shared `completeBuild` tool (`complete_build` on MCP) is the one completion boundary: a zero-tolerance evaluation that, on a clean pass, materializes the case-store schemas and flips the app `complete` — on findings it returns them person-to-person and repairs nothing. There is no post-hoc fix loop; the construction fuzz (`lib/agent/tools/__tests__/constructionFuzz.test.ts`) plus per-mutation-kind guard coverage (`lib/doc/__tests__/rescopingGuardCoverage.test.ts`) prove the retired registry's conditions unreachable from the construction surface.
