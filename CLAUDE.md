...
App-level `connect_type: 'learn' | 'deliver'` set during scaffolding. Individual forms opt in via `connect` config. The system auto-derives most Connect fields — `entity_id`, `entity_name`, `assessment.user_score` (defaults to `100`), `learn_module`, `deliver_unit` — via `deriveConnectDefaults()` in `validateAndFix`. The SA only needs to set `connect_type` on the scaffold and optionally `deliver_unit.name` or `learn_module` fields via `updateForm`. Connect apps set `auto_gps_capture: true` at the app level so HQ handles GPS capture during build. UI: app-level settings in Tier 2 subheader (`AppConnectSettings`), form-level toggle + config in `FormSettingsPanel` (gear icon in FormScreen header).
