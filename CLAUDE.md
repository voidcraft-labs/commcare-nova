...
**Connect state stash.** `MutableBlueprint` maintains a per-mode stash (`Map<number, Map<number, ConnectConfig>>`) that preserves form connect configs across app-level mode switches (learn ↔ deliver). Switching modes stashes the current mode's form configs and restores the new mode's stashed configs. Form-level toggles also stash data before toggling off, so re-enabling restores previous values.
