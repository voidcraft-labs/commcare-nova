...
- **Legacy builder store** (`lib/services/builderStore.ts`) — the shrinking Phase 1b store. Still holds `phase`, `cursorMode`, `activeFieldId`, `chatOpen`/`structureOpen`/`sidebarStash`, `agentActive`, `generationData`, `replayStages`, and mirrored entity maps. Entity maps are write-only-by-adapter — `startSyncOldFromDoc` (in `BuilderProvider`) projects the doc's entity slices into the legacy store so un-migrated consumers keep rendering. Phase 3 deletes the adapter and the mirrored fields.

**BuilderEngine** (`lib/services/builderEngine.ts`) is now purely imperative utilities — no composing methods, no navigation. It owns energy counters, the edit guard, the scroll callback, focus/panel hints, drag state, edit scope, the connect mode stash, and the form engine controller. Undo/redo, delete, and nav/select composition live in `lib/routing/builderActions.ts` and `lib/routing/hooks.tsx`.
...
- Convenience hooks (legacy store): `useBuilderPhase()`, `useBuilderIsReady()`, `useBuilderHasData()`, `useBuilderInReplayMode()`, `useBuilderTreeData()`, `useBuilderCursorMode()`, `useBuilderAgentActive()`.
...
**Selection changes re-render only the 2 affected wrappers** (old + new) via `useIsQuestionSelected(uuid)` — each wrapper receives its own identity, only the matching booleans flip. Nuance: because `useIsQuestionSelected` / `useIsModuleSelected` / `useIsFormSelected` call `useLocation()` (which subscribes to `useSearchParams()`), every consumer re-runs its hook body on any URL change. The boolean return still prevents child reconciliation for non-matching rows, so the net cost is just the component function call. Phase 5's virtualization bounds the consumer count to visible rows. Components that only dispatch actions (mutations, scroll, energy) stay on `useBuilderEngine()` with zero re-render cost.
