...
const seedSchema = z.object({ localizationAppIds: z.array(z.string()) });
...
test("language edits preserve source content and the URL lens across Builder, reload and Preview", async ({
	page,
	baseURL,
}, testInfo) => {
	const seed = seedSchema.parse(
		JSON.parse(readFileSync("e2e/.auth/seed.json", "utf8")),
	);
	const appId = seed.localizationAppIds[testInfo.retry];
	...
	const field = page.getByRole("button", { name: "Select field", exact: true });
	await field.getByRole("button", { name: "Client name", exact: true }).click();
	...
	await expect(page.getByRole("textbox", { name: /Client name$/ })).toHaveValue(
		"Asha",
	);
