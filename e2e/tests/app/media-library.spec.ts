...
test.describe("touch file manager", () => {
	...
	test("uploads from the empty-state action, waits for confirmation, previews, and deletes only after confirmation", {
		tag: "@seed:open",
	}, async ({ scenario, page }) => {
		const _seed = seedFor(scenario, "open");
...
test("search and pagination use server results; full document names disclose on keyboard focus and icons remain legible", {
	tag: "@seed:open",
}, async ({ scenario, page }) => {
	const _seed = seedFor(scenario, "open");
...
test("a live editor-to-viewer change closes pending deletion and reopens the file manager with preview access", {
	tag: "@seed:open",
}, async ({ scenario, page, context }) => {
	...
	try {
		...
		const viewer = JSON.parse(JSON.stringify(seed.viewerStorageState)) as {
...
test("a staged app-logo upload exposes progress and cancels without attaching a pending file", {
	tag: "@seed:open",
}, async ({ scenario, page }) => {
	const seed = seedFor(scenario, "open");
