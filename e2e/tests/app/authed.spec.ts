...
import type { SmokeProfileData } from "../../lib/smokeSeedFactory";

/**
 * Authenticated smoke checks, each driven by its native attempt's typed
 * scenario data and independent session cookie.
 *
...
test.describe("authenticated builder", () => {
	test("home lists the seeded apps and opens one in the builder", {
		tag: "@seed:app-list",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "app-list");
		...
		expect(Math.round(moveControlBox?.width ?? 0)).toBeGreaterThanOrEqual(44);
		expect(Math.round(moveControlBox?.height ?? 0)).toBeGreaterThanOrEqual(44);
	...
	test(
		"authors and restores a responsive organization hierarchy",
		{ tag: "@seed:organization" },
		async ({ scenario, page, baseURL }, _testInfo) => {
			const seed = seedFor(scenario, "organization");

			test.setTimeout(300_000);
			const appId = seed.organizationAppId;
			...
			await expect(
				levels.getByRole("heading", { name: "Levels" }),
			).toBeVisible();
			await expect(
			...
			await expect(
				levels.getByRole("button", { name: /Region/ }),
			).toBeFocused();
			...
			await levels
				...
				.click();
			await page
				.getByRole("option", { name: "The whole organization" })
				.click();
			await expect(
				levels
					.getByLabel("How much of the organization workers here can see")
			...
			await expect(places.getByText("Archived", { exact: true })).toHaveCount(
				0,
			);
			...
			const caseChangesRoute = seed.organizationCaseChangeRoute;
			...
			await conflictPlaces
				.getByRole("button", { name: /Coast Region/ })
				.click();
			...
			const viewerState = seed.viewerStorageState;
	...
	test("the Publishing section states where the app stands and keeps the one publish flow in the header", {
		tag: "@seed:open",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "open");
	...
	test("builder secondary headers stay aligned through sidebar and inspector states", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
	...
	test("case workspace composes result filters, owns its scrolling, and keeps searchable menus interactive", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	...
	test("a module's display condition explains where it applies, and Preview runs the screen it governs", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	 ...
	 */
	test("the form edit canvas gives its virtualized list a real viewport and draws the fields", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	 ...
	 */
	test("temporal questions show their stored answers, hold their row geometry, and refuse a half-typed clock", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	 ...
	 */
	test("a tile-laid-out case list renders at parity and pins its tile above the form", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	 ...
	 */
	test("a grouped case list draws one card per group and opens the group's first case", {
		tag: "@seed:workspace",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "workspace");
	...
	test(
		"a close condition keeps its friendly field projection through rename and move",
		{ tag: "@seed:case-changes" },
		async ({ scenario, page }, testInfo) => {
			const seed = seedFor(scenario, "case-changes");

			test.setTimeout(120_000);
			const fixture = seed.caseChanges;
			...
			const waitForSavedMutation = async (
				...
				const [response] = await Promise.all([responsePromise, mutate()]);
			...
			await test.step("reopening and Preview show names, never UUID-shaped XPath", async () => {
				...
				const previewOrder = await page
					...
					.evaluateAll((elements) =>
						elements.map(
							(element) => (element as HTMLElement).dataset.fieldUuid,
						),
					);
				expect(previewOrder).toEqual([
					identity.noteUuid,
					identity.firstNameUuid,
				]);
			});
	...
	test(
		"after-submit links are authored in the builder and followed in Preview",
		{ tag: "@seed:form-links" },
		async ({ scenario, page }, testInfo) => {
			const seed = seedFor(scenario, "form-links");

			test.setTimeout(120_000);
			const fixture = seed.formLinks;
			if (fixture === undefined) {
				throw new Error(
			...
			const conditionEditor = main.locator(
			...
			await test.step("adding a conditional link lands on its detail with the editor open", async () => {
				...
				await page
					.getByRole("button", {
						name: /Go somewhere when a condition is true/,
					})
					.click();
				await page
			...
			await test.step("a condition that reads the form is refused in the editor's own words", async () => {
				...
				// The idle editor confirms local admission. This journey next opens
				// a new document, so wait for this exact write's server receipt too.
				const conditionSaved = page.waitForResponse((response) => {
					const request = response.request();
					return (
						request.method() === "PUT" &&
						new URL(response.url()).pathname === `/api/apps/${fixture.appId}` &&
						(request.postData() ?? "").includes(FORM_LINKS_SEED.linkingNote)
					);
				});
	...
	test(
		"a search-first module registers what a search could not find",
		{ tag: "@seed:search-first" },
		async ({ scenario, page }, testInfo) => {
			const seed = seedFor(scenario, "search-first");

			test.setTimeout(120_000);
			const fixture = seed.searchFirst;
			if (fixture === undefined) {
				throw new Error(
			...
			await test.step("the Search canvas names the form Results offers, and the tree marks it", async () => {
				...
				await expect(
					setting.getByRole("heading", {
						name: "When no cases match",
						level: 2,
					}),
				).toBeVisible();
				await expect(
	...
	test("a sectioned form pages in Preview on the device's rules", {
		tag: "@seed:sections",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "sections");
	...
	test(
		"case changes add, retarget, preserve table lookups, and stay navigable to viewers",
		{ tag: "@seed:case-changes" },
		async ({ scenario, page, browser, baseURL }, testInfo) => {
			const seed = seedFor(scenario, "case-changes");

			test.setTimeout(120_000);
			const caseChanges = seed.caseChanges;
			...
			let heldActions = 0;
			await page.route(`**/build/${caseChanges.appId}/**`, async (route) => {
				if (
			...
			try {
				...
				await expect(lookupCatalog).toHaveAttribute("data-state", "loading");
				// This alert is produced only by a refused edit. Its absence while
				// the real reads are held cannot establish mutation readiness.
				await expect(
					page
						.getByRole("alert")
			...
			await expect(lookupCatalog).toHaveAttribute("data-state", "ready");
			...
			await expect(noteHandle).toBeFocused();
			...
			await test.step("a change deep in the sequence states where it is and walks to its neighbours", async () => {
				...
				await page
					.locator(`[data-case-operation-select="${deep.uuid}"]`)
					.click();
				await expect(
			...
			await test.step("a viewer can still open and inspect a case change", async () => {
				const viewerContext = await createSmokeContext(browser, {
					baseURL: baseURL ?? undefined,
					storageState: seed.viewerStorageState,
				});
				try {
	...
	test("/build/new renders the new-app builder (no LLM)", {
		tag: "@seed:open",
	}, async ({ scenario, page, browser, baseURL, request }) => {
		const seed = seedFor(scenario, "open");
		...
		const appResponse = await request.get(`/api/apps/${seed.openAppId}`);
		expect(appResponse.ok()).toBe(true);
		const { projectId } = await appResponse.json();
		const viewerState = seed.viewerStorageState;
		if (!viewerState) throw new Error("Missing viewer session");
		const viewerContext = await createSmokeContext(browser, {
			baseURL,
			storageState: viewerState,
		});
		try {
			const current = await viewerContext.request.get("/api/auth/get-session");
			const previous = await current.json();
			const viewerPage = await viewerContext.newPage();
			const guard = await attachErrorGuard(viewerPage, baseURL);
			try {
				const active = await viewerContext.request.post(
				...
				await active.json();
				await viewerPage.goto("/build/new");
				await expect(
					viewerPage.getByText(
				...
				const restore = await viewerContext.request.post(
					"/api/auth/organization/set-active",
					{
						headers: { Origin: baseURL ?? "http://localhost:3000" },
						data: {
							organizationId: previous.session.activeOrganizationId ?? null,
						},
					},
				);
	...
	test("the header mark keeps its touch target and its home link on every surface", {
		tag: "@seed:open",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "open");
	...
	test("help is reachable from inside a build, through the account menu", {
		tag: "@seed:open",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "open");
	...
	test("the from-scratch escape hatch mints the canonical starter and opens it (no LLM)", {
		tag: "@seed:auth",
	}, async ({ scenario, page }) => {
		const _seed = seedFor(scenario, "auth");
	...
	test("conversations open at the bottom and switch without exposing the prior transcript", {
		...
		const seed = seedFor(scenario, "threads");
	...
	test("assistant text can be selected without unmounting the chat shell", {
		tag: "@seed:threads",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "threads");
		...
		// Raw mouse coordinates do not wait for the drawer entrance animation.
		// Measure only after the text is stable, in view and receives pointer hits.
		await assistantMessage.click({ trial: true });
		...
		if (!messageBox)
			throw new Error("Assistant message has no rendered geometry.");
	...
	test("sending a message returns the view to it — a jump, never an animated trip", {
		tag: "@seed:scroll",
	}, async ({ scenario, page }) => {
	...
	test("answering a waiting question round returns the view to the conversation tail", {
		tag: "@seed:scroll",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "scroll");
	...
	test("GET /api/auth/get-session returns the seeded user", {
		tag: "@seed:auth",
	}, async ({ scenario, request }) => {
		const seed = seedFor(scenario, "auth");
	...
	test("owners delete and restore apps while viewers retain a read-only app list", {
		tag: "@seed:delete",
	}, async ({ scenario, page, browser, baseURL, request }) => {
		const seed = seedFor(scenario, "delete");
		...
		const href = await page
			.getByRole("link", { name: `Open ${seed.deleteAppName}` })
			.first()
			.getAttribute("href");
		if (!href) throw new Error("Missing throwaway app link");
		const appId = href.split("/").at(-1);
		const appResponse = await request.get(`/api/apps/${appId}`);
		expect(appResponse.status()).toBe(200);
		const { projectId } = (await appResponse.json()) as { projectId: string };
		...
		try {
			...
			const viewerGuard = await attachErrorGuard(viewerPage, baseURL);
			const current = await viewerContext.request.get("/api/auth/get-session");
			const previous = (await current.json()) as {
			...
			try {
				...
				await expect(
					viewerPage.getByRole("heading", { name: "Your apps" }),
				).toBeVisible();
				await expect(
					viewerPage.getByRole("button", {
				...
				await expect(
					viewerPage.getByRole("button", {
						name: /^Move .* to another Project$/,
					}),
				).toHaveCount(0);
				await viewerPage
					.getByRole("tab", { name: /^Recently deleted/ })
					.click();
				await expect(
					viewerPage
						.getByRole("heading", { name: seed.deleteAppName, level: 3 })
						.first(),
				).toBeVisible();
				await expect(
					viewerPage.getByRole("button", { name: "Restore", exact: true }),
				).toHaveCount(0);
			} finally {
				try {
					const restoredScope = await viewerContext.request.post(
						"/api/auth/organization/set-active",
						{
							headers: { Origin: baseURL ?? "http://localhost:3000" },
							data: {
		...
		const card = page
			.getByRole("listitem")
			.filter({
				has: page.getByRole("heading", { name: seed.deleteAppName, level: 3 }),
			})
			.first();
		await expect(card).toBeVisible();
		const deletedCount = await deleteHeadings.count();
		await card.getByRole("button", { name: "Restore", exact: true }).click();
		await expect(deleteHeadings).toHaveCount(deletedCount - 1);
	 ...
	 *
	 * Opening the moved app also proves its new Project authorization without
	 * requiring a separate Project switch.
	 *
	 * The seed mints one throwaway app per native Playwright attempt,
	 * since a moved app is gone from the source Project — the same idempotency
	 * rule the delete test follows.
	 */
	test("an owner moves an app to another Project and it stays moved", {
		tag: "@seed:move",
	}, async ({ scenario, page }) => {
		const seed = seedFor(scenario, "move");
		...
		// Capture the native link for this attempt's own app.
		...
		await page
			...
			.click();
		await page
			.getByRole("button", { name: seed.moveProjectName, exact: true })
			.click();
		await expect(moveHeadings).toHaveCount(1);
		await expect(
			page.getByRole("heading", { name: seed.openAppName, level: 3 }),
		).toHaveCount(0);
		await page.reload();
	 ...
	 */
	test("a select can be pointed at a shared data table's column", {
		tag: "@seed:workspace",
	}, async ({ scenario, page, browser, baseURL }) => {
		const seed = seedFor(scenario, "workspace");
		...
		await expect(moveColumnLeft).toBeDisabled();
		await expect(moveColumnRight).toBeFocused();
		...
		// Leaving Project data keeps the real lazy controller and its draft alive.
		await page.evaluate((path) => {
			window.history.pushState(window.history.state, "", path);
			window.dispatchEvent(new PopStateEvent("popstate"));
		}, seed.caseWorkspace.routes.selectField);
		await expect(
			page.getByRole("combobox", { name: "Where the choices come from" }),
		).toBeVisible();
		await page.goBack();
		await expect(
			page.getByRole("heading", {
		...
		await expect(moveRowUp).toBeFocused();
		...
		await expect(moveRowDown).toBeDisabled();
		await expect(moveRowUp).toBeFocused();
		await page.getByRole("button", { name: "Delete row" }).click();
		await expect(page.getByText("Delete this row?")).toBeVisible();
		await page.getByRole("button", { name: "Delete row" }).click();
		...
		const recoveryContext = await createSmokeContext(browser, {
			baseURL,
			storageState: await page.context().storageState(),
		});
		let recoveryGuard: Awaited<ReturnType<typeof attachErrorGuard>> | undefined;
		try {
			...
			const peerContext = await createSmokeContext(browser, {
				baseURL,
				storageState: await recoveryContext.storageState(),
			});
			let peerGuard: Awaited<ReturnType<typeof attachErrorGuard>> | undefined;
			try {
				const peerPage = await peerContext.newPage();
				peerGuard = await attachErrorGuard(peerPage, baseURL);
				...
				await recoveryPage
					...
					.click();
				await expect(
					recoveryPage.locator(
						'[data-slot="date-picker"][aria-invalid="true"]',
					),
				).toHaveCount(2);
				const reconciledMomentTime = recoveryPage.getByRole("textbox", {
					name: "Visit moment time",
				});
				await expect(reconciledMomentTime).toHaveAttribute(
					"aria-invalid",
					"true",
				);
				...
				await recoveryPage
					.getByRole("button", { name: "All data tables" })
					.click();
				await recoveryPage
					.getByRole("button", {
						name: new RegExp(`^${CASE_WORKSPACE_SEED.emptyLookupTableName}`),
				...
				await expect(
					peerDeleteTable.getByText(
						"No app in this Project uses it right now.",
					),
				).toBeVisible();
				await peerDeleteTable
