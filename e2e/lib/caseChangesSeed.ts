...
export function buildCaseChangesBlueprint(): BlueprintDoc {
	const doc = buildDoc({
		caseTypes: [
			{
				name: "patient",
				properties: [{ name: "last_note", label: "Last note" }],
			},
			{
				name: "referral",
				properties: [{ name: "source_note", label: "Source note" }],
			},
		],
		modules: [
			{
	...
	return {
		...
		forms: {
			...doc.forms,
			[CASE_CHANGES_SEED.formUuid]: { ...form, caseOperations: OPERATIONS },
		},
	};
}
