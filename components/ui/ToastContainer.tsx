...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
  ...
  return (
    <motion.div
      ...
      onPointerEnter={() => { hovering.current = true; clearTimer() }}
      onPointerLeave={() => { hovering.current = false; startTimer() }}
    ...
    >
      {/* Left accent bar */}
      <div className={`shrink-0 w-1 rounded-full self-stretch ${ACCENT_COLORS[toast.severity]}`} />

      {/* Content */}
      <div className="min-w-0">
        <p className="text-sm font-medium text-nova-text leading-tight">{toast.title}</p>
        {toast.message && (
          <p className="mt-0.5 text-xs text-nova-text-secondary leading-snug">{toast.message}</p>
        )}
      </div>
