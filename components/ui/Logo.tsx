export function Logo({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' | 'hero' }) {
  ...
  return (
    <div className={`${textSizes[size]} font-display font-bold tracking-tight flex items-center ${gaps[size]}`}>
      <div className={`relative ${dotSizes[size]} shrink-0`}>
        <div className={`${dotSizes[size]} rounded-full bg-nova-violet`} />
        <div className={`absolute inset-0 ${dotSizes[size]} rounded-full bg-nova-violet animate-ping opacity-30`} />
      </div>
