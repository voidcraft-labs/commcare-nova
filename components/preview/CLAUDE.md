...
- **PreviewShell** — Screen dispatch container. Accepts optional `nav` prop (from `usePreviewNav`), `hideHeader`, `onBack`, and `onUp`. When used inside BuilderLayout, the header is rendered externally in the subheader bar, nav state is shared via prop, and `onBack`/`onUp` sync builder selection. Passes `canGoBack`/`canGoUp`/`onBack`/`onUp` to all screen components. Screen transitions use shared `SCREEN_TRANSITION` constants from `screenTransition.ts`. Content fills the full pane (no border/padding wrapper).
- **PreviewHeader** — Nav buttons (back + up via `ScreenNavButtons compact static`), breadcrumb, actions slot. Used by PreviewShell when rendering standalone (not used by BuilderLayout).
- **ScreenNavButtons** (`ScreenNavButtons.tsx`) — Shared back (left arrow) + up (up arrow) nav buttons used by both PreviewHeader and all screen headers. Inside AnimatePresence screen transitions, a counter-sliding `motion.div` cancels the parent's x-offset so buttons stay visually pinned (config from `screenTransition.ts`). Props: `compact` (smaller 18px icons for header bar), `static` (skip counter-animation when outside AnimatePresence). Up button disabled at home level.
- **screenTransition.ts** — Shared slide/fade transition constants (`SCREEN_TRANSITION`) consumed by PreviewShell's AnimatePresence and ScreenNavButtons' counter-animation. Single source of truth for slide distance, duration, and easing.
...
All screens render a consistent header row: nav buttons (back + up, via `ScreenNavButtons`) + screen title. Back is disabled when no history; up is disabled at the home level. Both buttons are the same size and position across all screens so layout doesn't shift on navigation. `navigateUp` pushes the parent screen onto the history stack (derived via `getParentScreen` from `lib/preview/engine/types.ts`).
