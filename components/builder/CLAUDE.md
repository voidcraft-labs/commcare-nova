...
- **EditableText** — Always-rendered input/textarea styled as static text when unfocused, editing chrome on focus. Click places cursor at the native click position (no DOM swap). Single-line: Enter saves. Multiline: 1–4 row textarea sized to content (scrollable, doesn't auto-expand), Enter inserts newline, Cmd/Ctrl+Enter saves. Escape cancels (reverts to pre-edit value, `stopPropagation` prevents `useDismissRef` from closing the parent popover). Blur saves. Emerald checkmark fades on save. `autoFocus` focuses on mount. `selectAll` selects all text on focus (used for newly added questions via `builder.isNewQuestion(path)`, cleared on first save via `builder.clearNewQuestion()`). Optional `labelRight` renders right-aligned content in the label row. Label row order: label text, checkmark (left, inline), shortcut hint (right via `ml-auto`), `labelRight`.
- **EditableTitle** (`components/builder/EditableTitle.tsx`) — Inline editable title for screen headers (app name, module name, form name). Renders an `<input>` styled as an `<h2>` — transparent border at rest, `border-nova-border` on hover, violet border on focus. Hidden `<span>` mirror sizes the input to exact text width. Calls `onSaved` callback on successful save so the parent controls checkmark placement. **`readOnly` prop** — renders the identical `<input>` element non-interactively (`pointer-events-none`, no handlers); used by all three screens in preview mode so the title occupies the exact same box as in design mode, preventing layout shift on toggle. Typography and box-model classes are extracted to module-level constants (`MEASURE_SPAN_CLASS`, `INPUT_BASE_CLASS`) so both paths are guaranteed to stay in sync. **SavedCheck** — companion component rendering an animated emerald checkmark; each screen positions it independently (after settings button on FormScreen, after title on others).
