...
`useValidityPropagator` (in `useInnerValidityShadow.ts`) is the single hook every editor with an `onValidityChange` prop consumes — used across the case-list-config sections + cards, the shared `PredicateSlotCard`, and the case-search-config `ClaimSection`. The hook handles the `useEffect([isValid]) → onValidityChangeRef.current(isValid)` pattern uniformly; eliminates the prior hand-rolled `onValidityChangeRef` triples that all carried the same reorder-desync bug class. `useInnerValidityShadow` (the WeakMap-backed sibling) auto-GCs per-row validity when rows are removed; reorder-then-flip preserves per-row verdicts.

`components/builder/shared/` holds primitives that more than one builder workspace consumes. Today: `PredicateSlotCard` — the optional-`Predicate` slot card (header chrome + add/clear affordance + slot-presence body switch + slot-presence validity short-circuit). `FiltersSection` (case-list-config) and `ClaimSection` (case-search-config) both compose it. New cross-workspace primitives go here so workspace packages don't import each other's chrome.
