...
- **Filter section** — owns `caseListConfig.filter`. `FiltersSection.tsx` mounts the `PredicateCardEditor` (the Plan 3 Task 2 cards, registry-driven). Add-filter initializes via `matchAll()`; clear-filter sets the slot to `undefined`. Validity propagates via the slot-presence short-circuit `!filterPresent || predicateValid`.
