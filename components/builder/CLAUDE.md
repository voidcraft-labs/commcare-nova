...
- **Two scroll containers carry `data-preview-scroll-container`**: the always-present outer (`PreviewShell`) is the live scroller; `VirtualFormList` adds its own inner one in edit mode. `document.querySelector` (BuilderLayout's capture + DOM-nudge restore) returns the outer — which IS the live scroller, so that restore only works when landing on live. Landing on EDIT can't go through it: the edit canvas is virtualized, so the anchor row usually isn't in the DOM and the inner scroller is the one that moves. So preview→edit hands the anchor field uuid through the session store (`flipbookScrollAnchor`); the freshly-mounted `VirtualFormList` reads it and calls `virtualizer.scrollToIndex`, the only thing that reaches an off-screen row. That scroll is `setTimeout`-deferred (not inline, not rAF): the virtualizer re-applies its observed offset — 0 — during mount/StrictMode churn and would stomp an inline scroll, and rAF is paused when the build tab is backgrounded.
