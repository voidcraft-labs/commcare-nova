...
dialog reopen, destination or connected-server change, and on a landed publish, because the ledger remembers the
...
The three Users collections read `builderWriteAdmission` before enabling Add,
row inputs, value selectors, persona assignments, and removal. While Project
lookup definitions load or fail, one workspace notice explains the pause and
its retry uses the catalog provider. Permission visibility remains separate:
viewers still read the rows, and disclosures, pagination, cancellation, and
local drafts remain available through a catalog pause.
...
union. `automationEditorModel.ts` owns the draft transitions, recipient admission, reminder parsing, and date/schedule/reference projections used by the editor. A new rule is born as one complete valid object with caller-minted UUIDs;
...
A `section` is a page of the form: a root-only container whose questions render at depth 0 (a page is a break, not an indent). The edit canvas draws it as a `section-header` row (`virtual/rows/SectionHeaderRow.tsx`) over the shared `sections/SectionHeading.tsx` box, kicker "Section k of n" plus the inline-editable title, at the fixed `SECTION_HEADER_HEIGHT_PX`; the page has no close row, the next heading or the end of the form is the break, and an empty page shows the `empty-container` row in its `section` variant. Four rules bind every section gesture on the canvas:

- **Every gesture is a planner, applied through `applyFormSectionPlan`.** The insertion gap's picker asks `sectionGestureItems(doc, parentUuid, atIndex)` what the gap can do: the root of a sectioned form offers ONLY a new page (its "+" reads "Add a section"), a sectionless root offers the kinds plus "Split into sections here", a page offers the kinds plus "Split section here" (or "New section after this one" at its end), a group offers no page gesture. A page is never a flat kind in the menu, because its place is planned, not guessed. The rail's `SectionInspectorBody` applies `mergeWithPrevious` / `removeSectionKeepingQuestions` (naming where the questions go) the same way; the page's id and Move up / Move down ride the shared `FieldIdentitySection`.
- **Drag reads the one placement verdict.** `useRowDnd.canDrop` asks `fieldPlacementVerdict` for each of a row's landings (`landingContainerUuids`: a header lists its parent for the top edge and itself for the bottom), and `useDragIntent` re-asks with the resolved edge, so the placeholder never opens on a landing the gate refuses: a question at the root of a paged form, a page anywhere but the root, an add-entries repeat on a page. `drop-section-header` is a `dragData.ts` kind with the same top/bottom split a group header has; `draggedRowSpan` (`dropNoOp.ts`) is what makes "dropped next to itself" a cancel for a page, whose span runs to its trailing gap.
- **Flipbook parity is the heading box plus the page.** Edit and preview render the same `SectionHeading` at the same height, so rows below land at the same Y; and the session's `activeSectionByForm` carries WHICH page is open across the flip: `VirtualFormList` records the first visible row's page on unmount and opens on the pager's page through `initialOffset` (`sectionScroll.ts::initialEditOffset`, never an after-mount scroll), and the preview pager reads and writes the same slot.
- **The keyboard and the rail cross pages, never to the root.** `getCrossLevelFieldMoveTargets` gives a page's first / last question "Move to previous section" / "Move to next section" (`crossLevelMoveLabel`); a page itself reorders with plain arrows and has no Shift moves.
...
**What a case-bound field does at form open is ONE predicate, and the rail reads it from where the field lives.** `lib/domain/casePreload.ts::writerPreloadsFromLoadedCase(field, module, form)` is the rule the preview engine seeds from and the rule the Saves to line (`editor/fields/caseWritePresentation.ts::caseWriteGuidance`: several-case copy first, then "Opens with this case's current value" or "The calculation sets this value" for a preloaded writer, "Creates a new … case on each submission" for a child type), so the rail and the running form cannot disagree. Entry `visible` predicates therefore receive `(field, context)`, where `context` is the `FieldEditorContext` (`{module, form}` or `null`) that `FieldEditorPanel` reads ONCE from `useSelectedFormContext()` and threads through `sectionHasContent` / `partitionEditorEntries` / `useSectionActivation` / `FieldEditorSection` (optional prop, `null` by default so a fixture mounting one section compiles). A context-forced entry is an active row, never a pill.

**The default-value row on a preloaded writer is read-only, and it offers to clear a dead expression rather than hide it.** `defaultValueEntry()` is forced visible on such a writer and `DefaultValueEditor` renders `editor/fields/PreloadedValueRow.tsx` there instead of the XPath editor: the hint says the case supplies the value, and a stored expression (written before the rail said so, or carried over from another kind) shows read-only with "This expression never runs here" and a ghost **Remove expression** that clears the slot through the entry's ordinary `onChange(undefined)`; an expression whose reference no longer resolves is named as such and keeps the same Remove, because a dead expression nobody can clear is the worse state. The capture kinds have no default-value entry at all.

**A hidden field has one Value control, and every write it makes names both slots.** `editor/fields/HiddenValueEditor.tsx` is mounted on the `calculate` entry (always visible, never a pill; there is no separate `default_value` entry for hidden) with a **Keep in step** / **Set once** `SegmentedRow`; the pure model in `editor/fields/hiddenValueModel.ts` decides the stored mode (a calculation present wins, including a historical both-slot field) and builds every patch as `{[mode]: expression, [other]: null}`, so a save, a mode switch (the expression carries across), and a dead-expression clear are each one `inline.updateField(uuid, "hidden", patch)`: one undo entry, one gate evaluation, and no path to a field holding both slots. **The inert placeholder never enters `calculate`.** A hidden field is born set once with `HIDDEN_INERT_VALUE` in `default_value` (both minting paths), an empty save in either mode lands on that same shape (`HIDDEN_VALUE_EMPTY_PATCH`), and choosing keep in step with nothing to carry is a local `await-calculation` state, not a write: the editor opens and only a typed calculation commits. The control shows keep in step at rest over a placeholder-only field (a fresh insertion, a conversion that carried nothing) because that is the mode a typed calculation lands in, except on a preloaded writer, where set once is the fact worth showing. `data-field-id` conventions: the mode switch is `value_mode`, and the body is the SHOWN slot key (`calculate` or `default_value`), so undo focus, `findFieldElement`, and Playwright address whichever slot holds or is about to hold the value; on a preloaded hidden writer in set-once mode the body is the preloaded-value row under `default_value`.
...
**The options editor keeps a select's stored values inside the wire's grammar as they are typed.** `editor/fields/OptionsEditor.tsx`'s value box runs `sanitizeSelectOptionValue` on every keystroke (a space becomes `_`, a quote mark is dropped) and puts the caret back where the typing was whenever the sanitizer changed the text, since React's reassignment of a controlled input parks it at the end. A row Nova minted (`mintSelectOptionPlaceholder`: `option_N` / "Option N") has a value nobody chose, so the first real label it is given also names the value (`suggestSelectOptionValue`, suffixed past siblings); once either side has been hand-edited, the two are independent and a later label change never rewrites the value. A value box emptied at commit is filled from its label (`repairSelectOptionValue`) rather than landing a choice that saves nothing, and the adapter shows EVERY gate message beneath the rows, because the gate judges the whole document and the first finding is as likely to be about another field as about this edit.
...
The structure tree mirrors Nova's one-tier menu hierarchy as nested semantic
lists: root modules own their forms and, when present, one child-module list;
each child module owns its own forms. A form is rendered exactly once under its
owning module. Expansion, search ancestry, post-move focus, and row identity are
keyed by UUID, never by an array index or visual path, so a reorder or reparent
does not collapse or select a different entity. The compact rail qualifies child
modules and their forms with the parent-menu path while keeping the child
module's case-list entry distinct from any parent case list.

Root insertion says **Add module**; insertion inside a root module says **Add
submenu**. Both create at an explicit stable sibling anchor. Module settings and
the row action menu use the same placement planner for top-level or child
placement and sibling moves. Reordering is action-based, not drag-and-drop;
search disables those order actions until the full sibling group is visible.
A root that still owns child modules cannot become a child. All mutation
triggers retain 44px targets, return focus to the moved module's action button,
and are absent for viewers or while editing is locked. Rows retained only for
an exit animation are excluded from the post-placement focus target, so
reparenting restores the live row.

The structure tree has the same hover-reveal insertion affordance the form canvas uses (`appTree/insertion/`), between modules and between a module's forms — literally the same reveal gating, shared through the insertion-intent model (`lib/ui/insertionIntent.ts`, pure and unit-tested; DOM/React binding in `lib/ui/hooks/useInsertionZone.tsx`). The tree's insertion surfaces name themselves: the strip opens to a labeled "+ Form", "+ Module", or "+ Submenu" pill (no tooltip — naming an affordance through a tooltip means naming it while it's invisible), with form and submenu strips indented to their owning module. Each surface mounts one `InsertionIntentProvider` (AppTree for the tree, VirtualFormList for the form canvas); zones register geometrically and the model opens a gap only on dwell-intent — a decelerating (aiming) pointer opens on arrival, constant-speed passes and sweeps never open, a flick that stops on a gap opens after a settle beat. The reveal expands the gap and pushes neighboring rows apart (shared visuals in `components/ui/insertionReveal.ts`); layout moving under the pointer is safe because containment is geometric and the binding re-measures rects through the reveal animation — never DOM hover state. Because the app is valid by construction, creation can't make an empty shell — each "+" dispatches an ATOMIC born-valid scaffold (`lib/doc/scaffolds.ts`, the UI twin of the SA's `createModule`/`createForm`) committed as ONE gated batch through `useBlueprintMutations` (`createCaseListModule` / `createSurveyModule` / `createForm`), then navigates to the new entity (renamed inline on its screen). A case-list module is born as a VIEWER — a `caseListOnly` module with its case type declared in the catalog and a `Name` column, NO form (a name-only registration form is valid wire, so there's no reason to force one — see `lib/doc/CLAUDE.md` § scaffolds); adding a form later flips the viewer to form-bearing. A childless `caseListOnly` module IS its case list, so creating one (and clicking its tree module row, its home-screen tile, or landing on its module URL) opens the case-list config directly. Once it owns child menus, the module row and compact rail open its module screen so those destinations remain reachable, while its separate Cases row still opens Results (see § Case-list workspace). A survey module is born WITH one survey form (a formless, case-list-less module is a hard CommCare build error — `NO_FORMS_OR_CASE_LIST`, regardless of case type — so a bare module isn't valid). The add-form menu offers every form type but DISABLES the case-managing ones with a reason on a module that has no case type — the same disabled-with-reason rule the case-list pickers follow. `shared/CaseTypePicker.tsx` (existing types + validated create-new + clear) is reused by both the add-module popover and the module-settings Case Type section; its create-new name is gated inline by `lib/doc/identifierVerdicts.ts::caseTypeNameVerdict`. Clearing from module settings is confirmed before mutation; the everyday helper stays brief, while clear consequences and gate rejections live in that confirmation state (and a formless module explains why clearing is unavailable instead of offering a dead action).
...
- **Search carries a parent property, not a related-case calculation.** A calculated Results/Details item in a module with effective Search may use the current case freely or show one ancestor property by itself. The expression editor asks the document boundary's shared projection verdict before every edit and disables wrapped, aggregate, child, or ambiguous relation shapes with a concrete repair. Without effective Search, the ordinary case-list calculation stays unrestricted by this projection. Whole-config findings route to the calculated item's inspector even when the item is present only for Default order.
...
- Default order may use information absent from Results and Details. Its new hidden column is born through `columnSnapshotBatchMutations` in the same gated batch as its sort; both stored display permutations gain the identity while visible rows stay unchanged.
...
- **Search first is a switch in the Search panel's rail, withheld until the gate would admit it.** `SearchPanelInspectorBody` shows the **Search first** `ToggleRow` (`data-search-first-setting`) only while the Search action is active and the workspace says the module may open on Search (`canOpenOnSearch`: `useIsCaseFirstModule` or a bare case list) or the setting is already on, so the switch never invites a commit refusal; it writes `caseSearchConfig.searchFirst` through `setOptionalSlot`, and the tool boundary makes the same refusal for an owner-only config. While it is on and no button condition is saved, the **When Search is available** slot gives way to a note that there is no Search button to gate; a saved condition keeps its editor so the finding stays repairable. The Search canvas header and `data-search-first` describe the shape (people search before they see any cases). The consequences for the module's forms (no **Previous screen**, the module default) are the validator's and the form settings' to explain, not the switch's.
- **When no cases match is one decision reachable from two places.** `case-list-config/canvas/NoMatchesFormSetting.tsx` (mounted by `SearchCanvas` only while the Search action is active and the module has a case type) and `detail/formSettings/FormEntrySection.tsx` (a registration form's **How this form opens** row) both drive `case-list-config/noMatchesFormReview.tsx::useNoMatchesFormEntry`. It plans through `lib/doc/searchNoMatchesForm.ts` (`noMatchesRegistrationFormMutations` births a new form carrying every prompt; `noMatchesFormEntryMutations` sets the entry on an existing registration form), reviews the exact batch with `inline.reviewMany`, and names the consequences in `NoMatchesFormReviewDialog` before `commitMany`: Search first turning on, the form leaving the menu, and where it opens after submit (explicit App home, or the scalar native return: Results with the new case while the module keeps a menu form, otherwise Search). New registration on a multiple-selection host includes explicit App home in the reviewed batch; changing an existing form's entry preserves its authored destination and refuses an unsupported return instead of changing it silently. Gate blockers render in that dialog instead of a toast. Clearing is reviewed the same way, because a menu registration form cannot open on Search: `noMatchesFormEntryMutations(…, null)` turns Search first off, takes `caseListOnly` off a bare host, and removes the `default_value` of every field whose only search read is its starting value (`searchAnswerDefaultsOf`; a read in any other slot stays and the gate names it), and the dialog says all three. The form row edits the action label through `updateForm` `entry`, appends carried answers with `carrySearchAnswersMutations` (one field per prompt the form does not carry yet), and `FormSettingsPanel` replaces the generic `AfterSubmitSection` with `NoMatchesAfterSubmitSection` for a no-matches form: only explicit App home and, on scalar hosts, the implicit native return are offered. The display-condition and link controls remain hidden. Clearing the entry preserves an explicit App home choice. `appTree/FormCard.tsx` marks the form (`data-no-matches-form`). Search-input removal review composes `searchInputRemovalDependencies` over the rendered config with `searchInputFormFieldDependencies` over the document (the `#search/` readers live on other screens); a `form-field` dependency closes the review and opens the field with `navigate.openForm`, and the person presses Remove again once nothing reads the answer.
...
- **Raw XPath function names have one valid-by-construction authoring contract.** The text editor admits only JavaRosa-native functions, path initializers in their required path-root position, and extensions with a proven production lowering, and every admitted call shape must execute faithfully in Preview. Familiar XPath names are not evidence: Core does not implement `last()`, `substring()`, or XForm `here()`. `normalize-space()` remains friendly authoring vocabulary because Preview implements its semantics and the wire boundary lowers it structurally to native `replace()` calls. Reaching an unsupported evaluation from a persisted document is an internal invariant failure; containment keeps the Builder usable but never turns that state into a normal compatibility boundary.
...
- Related-case conditions resolve choice options and literal input types from the saved relation destination. Originating properties with the same name never supply the connected case's metadata; an unresolved destination never falls back to the originating case.
- Relationship paths are catalog-driven and lossless. Canonical parent steps follow the actual `parent_type` chain; child destinations use direct children, while any-direction destinations use the union of the parent and direct children. Optional case-type hints stay out of the common path when one destination is provable. A custom saved index can reach any declared case type and therefore requires an explicit destination; on a graph leaf, choosing a direction opens one atomic connection-name + case-type step so the editor never commits a half-configured relation. Switching a saved custom connection between child-only and either-direction preserves its explicit destination and connection name, including at a graph leaf. Saved missing or stale hints remain readable with a focused recovery choice. Count and related-case filters preserve their entire authored condition when checking a proposed connection; an incompatible destination is disabled before mutation, and the author can explicitly remove or change the condition to choose it. Link names draft locally and commit on blur or Enter only after passing the relation identifier's XML-name rule, so ordinary typing can never hit the commit gate and snap back. Multi-step walks preserve every link name, and structural removal rebinds only position-dependent canonical-parent hints; custom destinations remain explicit and nested conditions are never rewritten.
...
- AST and mapping rows have no persisted UI uuid. Editable value-object lists MUST use the per-mounted `useStableListIdentity` sidecar and stage the exact local operation before dispatch: `replace` preserves keys, `splice` mints/drops only the named occurrences, and `move` permutes the same keys. The sidecar computes speculative render frames and adopts a staged vector only when that frame commits; abandoned renders cannot consume a newer intent. The next cloned document snapshot adopts that staged vector; external snapshots reconcile exact references and canonical structural occurrences conservatively. React rows and DnD payloads consume the SAME per-occurrence keys, while each DnD container uses a per-mount `useId()`. Active scalar editors key only by stable authored path. This keeps focus, drafts, open pickers, and staged media through planner/reducer `structuredClone` boundaries without putting UI identity in the Blueprint or emitted wire.
...
Running case-list cells and Quick Filter share `lib/preview/columnDisplay.ts::projectColumnDisplay` as their semantic-text boundary. The projection receives the effective property catalog, resolved calculated temporal types, and one shared `today`; the renderer, row action name, and filter all consume its `text` instead of independently coercing storage values. This keeps option labels, formatted dates/intervals, image labels, localized calculated values, and `Yes` / `No` answers searchable exactly as shown. JSONB multi-select arrays never surface as debug JSON, and malformed/structured values use a keyboard- and touch-accessible disclosure rather than a hover-only explanation.

## Deep links

App setup's Deep links section owns destination selection and entry-point
settings. A destination's settings can add its entry point atomically and open
that UUID-selected detail. Link IDs are explicit, stable external names; name
...
in order. The two case sources are never substituted. Generate HQ link performs
a fresh released-build check before exposing a copyable URL; it is a write-capable
operation and is unavailable to viewers. Its result and check failure are cleared on document,
selected destination, deployment, or access changes. The copy states the observed
check time, because HQ chooses the build each recipient opens.
...
### Inspector prose and draft outcomes

`RefLabelInput` consumers return their actual commit outcome. A refusal keeps
the local prose and focus; the consumer renders exactly one inline reason.
`TextEditor`, validation messages and inline option labels all follow this
contract. An absent optional slot remains a no-op in the document history.
Options-source changes are staged until confirmation. A saved table that
leaves the current catalog stays represented as a disabled selector entry;
select reconciliation cannot silently stage inline choices in its place.

Optional field-editor activation belongs to the currently selected field and
section. Leaving that scope forgets it; a blank new hint canceled with Escape
closes without writing. The Required add action commits `true`, and a condition
is introduced only through its explicit condition action.

Rich-text tables use `lib/tiptap/markdownTable.ts` in the real extension set.
Every cell serializes its inline content, including image-only and reference-only
cells; `textContent` cannot decide whether a cell has authored content. Complex
spanned or multi-block tables retain their HTML representation. Native tests
verify the editor saves and reopens image-only table cells.

Search field property metadata follows the complete authored relation through the domain graph resolver; unresolved or ambiguous walks never borrow the origin property. Column display drafts are local to the open inspector; returning to one merges current common slots and rechecks source/type validity plus effective Search compatibility before reuse.

Connect dialog drafts are owned by `detail/appSettings/connectDraft.ts`; XPath draft round trips use the real document text/identity bridge. Whole-hour validation rejects values outside the persistable safe-integer range before finalization.

Project data date-time drafts preserve malformed retained text for review, and finalization refuses an extra date/time separator rather than dropping its suffix.

Filter summaries preserve parentheses around nested logical groups. Quick Filter
consumes the same visible cell text as the renderer, including raw invalid-address
fallbacks and empty whitespace-only links. Data review replacement drafts refuse
numbers that overflow finite JSON numbers before offering a replacement action.

The Builder's reduced-motion preference uses a live `matchMedia` subscription.
Changing the device setting while a content-frame glide is active settles that
