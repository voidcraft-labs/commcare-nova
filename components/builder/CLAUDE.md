...
The canvases are artifact-first: the case list IS a live table, the search panel IS the worker's form, the detail card IS the opened-case view. **Clicking a thing configures that thing** in the inspector rail; selection is workspace-local state (`workspaceSelection.ts`), keyed by module, cleared on tab switches and Esc (Escape registers through `useKeyboardShortcuts` — the shared manager preventDefaults matched keys, so a raw window listener would never fire; later registrations win, which is what lets the workspace's Escape layer over builder-layout's).

**Add affordances land WORKING entities.** `seeds.ts` owns the smart seeds: a new search input or column binds a real property (`case_name` first, then unused text, then any unused — never unbound, because an unbound input matches nothing at runtime and reads as "search is broken"), takes a human-worded label and a legal unique wire name, matches the widget to the property's data type, and seeds text-shaped properties with Forgiving (fuzzy) match. The same bar applies to advanced→simple conversion (`SearchInputEditor`'s `convertToSimple` recovers the predicate's anchored property or re-seeds) and to property changes (`setProperty` auto-fixes widget type/mode when the new pair is inadmissible and carries derived labels/names along — hand-typed ones are never overwritten).

**Match/Type vocabulary is plain words** (`SEARCH_MODE_LABELS` / `SEARCH_INPUT_TYPE_DESCRIPTIONS` in `searchInputResolution.ts`, mirrored in the shared `MatchCard`): "Forgiving" not "Fuzzy", "Sounds like" not "Phonetic", every picker item carries a one-line description, and items the bound property's data type can't run are disabled with the reason instead of selectable into a validation error. The Preview tab's no-match card closes the loop: when a zero-match is explained by a letter-for-letter text input the worker typed into, it says so and offers a one-click "Switch to forgiving match" (a real, undoable config edit).
...
- **`canvas/SearchCanvas.tsx`** — worker-true depiction of the search panel: widget shape carried by the rendering (date-range = From/To pair, select = chevron, barcode = scan glyph), defaults pre-filled, match mode kept off the canvas (workers never see it — it lives in the inspector + hover title). Panel chrome selects the search panel; rows select their input; rows with structural errors get a rose ring from the shared `searchInputResolution.ts` derivation.
...
- **`canvas/WorkerPreviewCanvas.tsx`** — the Preview tab: the real `SearchInputForm` widgets filtering the real case store (`useCases`), results table, and the case detail opened IN PLACE (back affordance), composed search-beside-results when the canvas is wide and stacked when it isn't — width decides, like it does for workers, so authors never pick an export mode. The chat rail stays alive beside it. The store's `empty` arm means "this query matched nothing" too, so the Generate affordance only shows when no search is active.

Inspector bodies (under `inspector/`): the column selection mounts the existing `ColumnEditor` unchanged; `FilterInspectorBody` mounts `PredicateSlotCard` + a live match count (`loadFilterPreviewAction` with `limit: 1`); `SearchInputEditor` is the stacked per-input editor (simple/advanced arms, convert affordance, default-value slot — the former `SearchInputsSection` row, with its pure resolution helpers extracted to `searchInputResolution.ts`); `SearchPanelInspectorBody` owns every `caseSearchConfig` slot (title / markdown subtitle / button label / display condition / excluded owners); `ListPanelInspectorBody` (the list's title chrome) owns the `SortPriorityStack` (drag-to-reorder priority), the Generate/Reset sample-data cluster (Reset takes an inline two-step confirm — it deletes every case in the type), and, on `caseListOnly` modules only, the case-list menu-link media slots (`icon` / `audioLabel` — wholesale `setOptionalSlot` rebuilds, since `updateModule({ caseListConfig })` has no null-carrying clear).
...
`ColumnAffordancesRow.tsx` is the per-column compact controls row mounted at `CardShell.kindAccent` — the shell-level placement is what lets the affordances surface uniformly across all seven kinds (including calc, which has no `ColumnFieldRow` to attach them to). Three controls compose left-to-right: visibility eye-toggle pair (list + detail), neutral / asc / desc sort direction tri-state, and a read-only ordinal priority badge ("1st", "2nd", "3rd", …) with a small `×` to clear the column's sort.

Search inputs remain one source across the search and list screens — `caseListConfig.searchInputs` — edited from the search canvas. `caseSearchConfig` (screen labels, button condition, owner exclusions) is a separate module slot edited only through the search-panel inspector.
...
- **`PredicateCardEditor`** + **`ExpressionCardEditor`** — the top-level Predicate / ValueExpression authoring surfaces. Each composes the schema-driven `PredicateEditProvider` (caseTypes + currentCaseType + knownInputs + per-path validity index) and dispatches the root node into a tree of cards via the matching registry. Cross-family recursion works out of the box (a predicate's value slots compose `ExpressionPicker`; an expression's `if.cond` / `count.where` slots compose `ChildPredicateEditor`).
  - **Predicate cards** at `cards/`: 13 cards covering every kind in the `Predicate` discriminator union (comparison / membership / null/blank / sentinel / logical group / when-input-present / relational quantifier / text-match / geo-distance), dispatched through `cards/ChildPredicateEditor` with kind-replace + drag-reorder chrome.
...
- **`SlotCardHeader`** — header chrome for slot cards (violet rail, icon, title, hint, optional collapse chevron, optional `ml-auto` Clear button). Used by `OptionalSlotCard`; one home for the layout so consumers can't drift icon sizes, rail widths, and Clear glyphs.
