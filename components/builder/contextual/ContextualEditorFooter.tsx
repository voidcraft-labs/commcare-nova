...
export function ContextualEditorFooter({ question, builder }: QuestionEditorProps) {
  ...
  const typePicker = useFloatingDropdown<HTMLButtonElement>({ placement: 'top-start', offset: 4 })
  ...
  return (
    <div className="flex items-center justify-between px-2 py-1.5 border-t border-white/[0.06]">
      ...
      <button
        ...
        disabled={!canConvert}
        title={canConvert ? 'Change type' : "Can't change type — remove and add a new question instead"}
        aria-haspopup="true"
      ...
      >
        {typeIcon && <Icon icon={typeIcon} width="14" height="14" className="shrink-0" />}
        <span>{typeLabel}</span>
        {canConvert && <Icon icon={ciChevronDown} width="12" height="12" className="shrink-0 opacity-50" />}
      </button>
      {canConvert && (
