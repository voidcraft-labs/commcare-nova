...
import {
  type QuestionEditorProps, type XPathFieldKey,
  useAddableField, xpathFields, addableTextFields,
} from './shared'
 ...
 */
function XPathSection({ label, value, onSave, getLintContext, autoEdit, children }: {
  label: string
  value: string
...
export function ContextualEditorLogic({ question, builder }: QuestionEditorProps) {
  ...
  const missingXPathFields = xpathFields.filter(f =>
    !question[f.field as keyof Question] && xpathField.activeField !== f.field,
  )
  ...
  const missingValidationMsg = addableTextFields.filter(f =>
    f.field === 'validation_msg' && !question.validation_msg && textField.activeField !== 'validation_msg'
  ...
  const hasContent = question.required || question.validation || question.relevant
    || question.default_value || question.calculate || xpathField.activeField
  ...
  return (
    <div className="space-y-3">
      ...
      {isVisible('validation') && (
        ...
        >
          {(question.validation_msg || textField.activeField === 'validation_msg') && (
            <div className="mt-1">
              <EditableText
                ...
                onEmpty={textField.activeField === 'validation_msg' ? textField.clear : undefined}
              />
            </div>
          )}
        </XPathSection>
