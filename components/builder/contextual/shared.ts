...
export type XPathFieldKey = 'validation' | 'relevant' | 'default_value' | 'calculate'
...
export const addableTextFields: readonly { field: TextFieldKey; label: string }[] = [
...
export function useAddableField(questionPath: QuestionPath) {
  const [pending, setPending] = useState<{ field: string; questionPath: QuestionPath }>()

  /** The field key if it belongs to the current question, undefined otherwise. */
  const activeField = pending?.questionPath === questionPath ? pending.field : undefined
