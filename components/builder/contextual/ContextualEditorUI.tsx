...
import { type QuestionEditorProps, useAddableField, addableTextFields } from './shared'
...
export function ContextualEditorUI({ question, builder }: QuestionEditorProps) {
  ...
  const { activeField, activate, clear } = useAddableField(selected.questionPath!)

  /** Text fields not yet set on this question, available to add. */
  const missingTextFields = addableTextFields.filter(f =>
    f.field === 'hint' && !question[f.field as keyof Question] && activeField !== f.field,
  )
