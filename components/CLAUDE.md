...
- **App chrome inverts the lockup's proportion, on purpose.** Four rungs hold the illustration's ratio (wordmark ~1.3× the mark). The `chrome` rung does not: the mark is 44px because that is the control band every header control stands in, and the type stays near reading size instead of scaling with it. Proportionally the type would be 57px, which makes the lockup taller than the buttons beside it and turns the header into a title card. In chrome the sphere is the brand and the word is its label. The header shows the mark alone until the row can hold the whole lockup, and that breakpoint is the width the row actually measures, not a nominal "phone" one.
- **The lockup aligns on the lowercase INK, not on the two boxes.** The ink band sits 0.132em below the text box's centre, so centring the boxes leaves the mark visibly high. `Logo` moves whichever box has the room: the mark drops when the type is taller, the type rises when the mark is. Moving the wrong one carries it outside the lockup — a mark as tall as its own type has nowhere to drop, and taking the offset anyway hangs the sphere below the header row. Verify this with a baseline probe (a zero-sized `inline-block` on `vertical-align: baseline`, whose bottom edge IS the baseline) plus canvas `actualBoundingBox*`; reconstructing the baseline from half-leading arithmetic gets it wrong by 3px and will send you to fix an alignment that is already correct.
