...
const scenarios: Scenario[] = [
	{
		name: "Sending",
		description:
			"Upward wave — user just hit send, waiting for server response.",
		run: ({ setMode }) => {
			setMode("sending");
			return () => setMode("idle");
		},
	},
	{
		...
		description:
			"Model is deep in thought. Sparse reasoning tokens every ~500ms.",
		run: ({ setMode, inject }) => {
			setMode("reasoning");
	...
	},
	{
		...
		description:
			'"do-do-do-DA" — small trickles punctuated by big reasoning bursts.',
		run: ({ setMode, inject }) => {
			setMode("reasoning");
			let tick = 0;
	...
	},
	{
		...
		description:
			"Model thinks silently for 3s, then dumps a big reasoning block. Repeats.",
		run: ({ setMode, inject }) => {
			setMode("reasoning");
			let phase = 0;
			const id = setInterval(() => {
	...
	},
	{
		...
		description:
			"Build pipeline running, no data parts yet. Just the heartbeat sweep.",
		run: ({ setMode }) => {
			setMode("building");
			return () => setMode("idle");
		},
	},
	{
		...
		description:
			"Sweep + large bursts every ~2s (simulating module/form done events).",
		run: ({ setMode, inject }) => {
			setMode("building");
	...
	},
	{
		...
		description:
			"Forms completing in quick succession — frequent medium bursts.",
		run: ({ setMode, inject }) => {
			setMode("building");
	...
	},
	{
		...
		description:
			"Agent editing with no specific target. Defrag across entire grid.",
	...
	},
	{
		...
		description:
			"Agent editing a specific form. Defrag concentrated in middle zone.",
		run: ({ setMode, injectThink, setFocus }) => {
			setMode("editing");
			setFocus({ start: 0.3, end: 0.6 });
	...
	},
	{
		...
		description:
			"Agent moves between zones: starts at left, jumps to right, then center.",
		run: ({ setMode, injectThink, setFocus }) => {
			setMode("editing");
			...
			timers.push(setTimeout(() => setFocus({ start: 0.7, end: 0.95 }), 3000));
	...
	},
	{
		...
		description:
			"Agent making edits that trigger data parts. Defrag + burst flashes.",
		run: ({ setMode, inject, injectThink, setFocus }) => {
			setMode("editing");
			setFocus({ start: 0.2, end: 0.55 });
			const thinkId = setInterval(
				() => injectThink(25 + Math.random() * 30),
				120,
			);
			const burstId = setInterval(
				() => inject(100 + Math.random() * 100),
				2000,
			);
			return () => {
	...
	},
	{
		name: "Full Lifecycle",
		description:
			"Sending → Reasoning → Scaffolding → Building → Done. All transitions driven by onSettled.",
			...
			onSettled(() => {
				timers.push(
					setTimeout(() => {
						...
						setMode("scaffolding");
						const thinkId = setInterval(
							() => injectThink(15 + Math.random() * 25),
							150,
						);
						intervals.push(thinkId);

						setScaffoldProgress(0.3);
	...
	},
	{
		...
		description:
			"Building completes, then agent starts editing a specific form.",
		run: ({ setMode, inject, injectThink, setFocus }) => {
			setMode("building");
			const timers: ReturnType<typeof setTimeout>[] = [];
			const intervals: ReturnType<typeof setInterval>[] = [];
			...
			timers.push(
				setTimeout(() => {
					...
					const editId = setInterval(
						() => injectThink(25 + Math.random() * 35),
						100,
					);
					...
					timers.push(
						setTimeout(() => setFocus({ start: 0.6, end: 0.85 }), 5000),
					);
	...
	},
	{
		...
		description:
			"Tetris fill with real milestone timing: schema → partial → scaffold → done.",
		run: ({ setMode, setScaffoldProgress, injectThink }) => {
			...
			timers.push(setTimeout(() => setScaffoldProgress(0.3), 2000));
	...
	},
	{
		...
		description:
			"Full theatrics — drip-feeds progress so every piece gets preview → select → slide → lock at natural pace.",
		run: ({ setMode, setScaffoldProgress, injectThink }) => {
			setMode("scaffolding");
			const thinkId = setInterval(
				() => injectThink(15 + Math.random() * 25),
	...
	},
	{
		...
		description:
			"Schema at 30%, then jump straight to 100% after 1.5s. Tests rapid catch-up.",
		run: ({ setMode, setScaffoldProgress, injectThink }) => {
			setMode("scaffolding");
			setScaffoldProgress(0.3);
			const timers: ReturnType<typeof setTimeout>[] = [];
			const thinkId = setInterval(
				() => injectThink(20 + Math.random() * 30),
				120,
			);
	...
	},
	{
		...
		description:
			"Progress reaches 60% then stalls for 8s (breathing front), then completes.",
		run: ({ setMode, setScaffoldProgress, injectThink }) => {
			setMode("scaffolding");
			setScaffoldProgress(0.6);
			const timers: ReturnType<typeof setTimeout>[] = [];
			const thinkId = setInterval(
				() => injectThink(10 + Math.random() * 15),
				200,
			);
	...
	},
	{
		...
		description:
			"Full lifecycle: scaffold fills, transitions to building, then done. All driven by onSettled.",
		run: ({ setMode, setScaffoldProgress, inject, injectThink, onSettled }) => {
			...
			timers.push(setTimeout(() => setScaffoldProgress(0.3), 1000));
	...
	},
	{
		name: "Build → Done",
		description:
			'Building with energy bursts, then "du-du-DONEE" celebration transition.',
		run: ({ setMode, inject }) => {
			setMode("building");
			const timers: ReturnType<typeof setTimeout>[] = [];
			const buildId = setInterval(() => inject(80 + Math.random() * 60), 200);
	...
	},
	{
		...
		description:
			"SA hit an issue but is still working. Reasoning with ~35% warm-hued cells.",
		run: ({ setMode, inject }) => {
	...
	},
	{
		...
		description:
			"Recovering with minimal energy. Slow amber-rose flickers in ambient.",
		run: ({ setMode, inject }) => {
			setMode("error-recovering");
	...
	},
	{
		name: "Error Fatal",
		description:
			"Unrecoverable error. Erratic warm flicker → slow fade → dim rose embers.",
		run: ({ setMode }) => {
			setMode("error-fatal");
			return () => setMode("idle");
		},
	},
	{
		...
		description:
			"Full error lifecycle: building normally, hits an issue, tries to recover, gives up.",
		run: ({ setMode, inject }) => {
			setMode("building");
			const timers: ReturnType<typeof setTimeout>[] = [];
			const intervals: ReturnType<typeof setInterval>[] = [];
		...
		<div className="min-h-screen bg-nova-void text-nova-text p-8">
			<div className="max-w-3xl mx-auto space-y-8">
					...
					<div className="flex gap-6 flex-wrap">
						{PIECES.map((piece) => (
							<div key={piece.id} className="space-y-1.5">
								<span className="text-xs text-nova-text-secondary font-mono">
									{piece.name}
								</span>
                       ...
                       to produce a stable content-derived key instead of the array index. */
										...
										return (
											...
											>
												...
												<span className="text-[9px] text-nova-text-muted font-mono">
													r{rotationIndex}
												</span>
											</div>
										);
				...
				<div className="space-y-2">
					...
					<div className="bg-nova-deep border border-nova-border rounded-xl p-4 flex justify-center">
						<div style={{ width }}>
							<SignalPanel
								active={appliedMode !== "idle"}
								label={appliedLabel}
								error={appliedMode === "error-fatal"}
								recovering={appliedMode === "error-recovering"}
								done={appliedMode === "done"}
							>
						...
						{(
							...
							<button
								...
								onClick={() => {
									...
									scaffoldProgressRef.current = 0;
									setMode(m);
									if (m === "scaffolding") setScaffoldProgress(0.5);
