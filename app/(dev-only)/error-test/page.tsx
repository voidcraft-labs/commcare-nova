...
const errorScenarios: ErrorScenario[] = [
  ...
  },
  {
    ...
    run: ({ setGridMode, setPhase, setStage, setGenerationError, setStatusMessage, injectEnergy }) => {
      ...
      const timers: ReturnType<typeof setTimeout>[] = []
      const energyId = setInterval(() => injectEnergy(30 + Math.random() * 20), 150)
  ...
  },
  {
    ...
    description: 'Non-generation error: compile/download fails. Just a toast, no grid or progress impact.',
    run: () => {
  ...
  {
    name: 'Toast Stack',
    description: 'Multiple toasts in quick succession to test stacking and dismiss behavior.',
    run: () => {
      showToast('error', 'Stream error', 'The connection was interrupted.')
      setTimeout(() => showToast('warning', 'Retrying...', 'Attempting to reconnect.'), 800)
      setTimeout(() => showToast('info', 'Recovered', 'Generation resumed.'), 2000)
      return () => {}
    },
  },
]
...
export default function ErrorTestPage() {
  ...
  const gridCallbackRef = useCallback((el: HTMLDivElement | null) => {
    ...
    const ctrl = new SignalGridController({
      consumeEnergy: () => { const e = energyRef.current; energyRef.current = 0; return e },
      consumeThinkEnergy: () => { const e = thinkEnergyRef.current; thinkEnergyRef.current = 0; return e },
    })
    controllerRef.current = ctrl
    ctrl.attach(el)
    ctrl.powerOn()
    return () => { ctrl.detach(); controllerRef.current = null }
  }, [])
  ...
  const injectEnergy = useCallback((n: number) => { energyRef.current += n }, [])
  ...
  return (
    <div className="min-h-screen bg-nova-void text-nova-text p-8">
      <div className="max-w-3xl mx-auto space-y-8">
        <div>
          <h1 className="text-2xl font-display font-medium mb-1">Error System Test</h1>
          <p className="text-sm text-nova-text-secondary">
            Simulate error scenarios to see toasts, signal grid error modes, and progress error states together.
          </p>
        </div>
