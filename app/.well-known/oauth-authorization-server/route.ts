 ...
 * host and rejects it on the MCP host (see lib/hostnames.ts). Claude Code and
 * other MCP clients read this endpoint to discover the token, authorization,
 * and dynamic-registration endpoints for the OAuth flow.
 *
 ...
 * exposed at `/.well-known/oauth-authorization-server/api/auth`. RFC 8414
 * clients (and Claude Code) probe the BARE `/.well-known/oauth-authorization-server`
 * path first. Mounting the same `oauthProviderAuthServerMetadata` helper here
 * gives us the canonical discovery URL without duplicating logic — the
 * helper generates the correct document regardless of which path it lives at.
 *
