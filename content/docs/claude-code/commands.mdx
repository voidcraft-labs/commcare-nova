...
Name a **project space** as the second argument to upload straight there, useful when your key reaches several spaces and you already know the target. Leave it off and Claude shows you the target before uploading, asking which one if your API key (saved in [Nova settings](https://commcare.app/settings)) reaches more than one, then asks you to confirm; decline to back out. If you name a space the key can't reach, Claude lists the ones it can so you can pick.
...
On success, the response includes the HQ app's URL and any warnings HQ returned. The first upload to a project space creates the app there; uploading again updates that same HQ app in place. Your Nova copy is unaffected either way.
