...
| `generate_schema` | Record the data model on the app: every case type with its properties and parent links. A real write — `create_module` references the recorded types by name, and a form field that writes a recorded property inherits its label, options, and validation. Also how a new case type enters an existing app. A property something outside the app writes can carry an `external` marking (with a note naming what sets it). |
| `mark_property_external` | Record that a case property is written outside this app — another app on the same case type, HQ, or an integration — or clear that marking (`external: null`). The resolution path for a no-writer advisory (below) when the user confirms the data comes from elsewhere; the note is kept so future sessions inherit the fact. |
...
Some problems are about the workflow, not any single piece: a case property that gates behavior — a field's visibility or validation, a form link's condition, a case-list filter — while no form in the app writes it means records can never reach the gated state from within the app. Every piece is individually valid, so nothing is rejected; instead, a mutating call that introduces this state carries an `advisories` note in its result, and `get_app` lists any still open. Resolve one by adding a field that writes the property, or — when another app or system genuinely sets it — by recording that with `mark_property_external`.
