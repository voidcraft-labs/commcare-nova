...
These tools use author identities throughout: module, form, and operation slug ids, plus field paths such as `visits/outcome`. They never ask for or return storage UUIDs. A later operation in one add call can target an earlier create from that call by its operation id; the whole list is checked and saved atomically.
