...
| `update_case_operation` | Replace one operation's author shape by id. Only changed identity-keyed slots are written, so unrelated concurrent edits compose. |
| `move_case_operation` | Move an operation to a new execution index. Refuses dependency-breaking or non-portable order. |
