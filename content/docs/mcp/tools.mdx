...
| Tool | What it does |
|---|---|
| `create_app` | Create a new empty app in **Draft** status and return its ID. Build it up with the other tools, then mark it finished with `complete_build`. |
...
| Tool | What it does |
|---|---|
| `create_module` | Add a new module together with its forms (each with fields) and case-list columns, in one call — a case-managing module lands complete or not at all. Refine the case list afterward via the case-list tools below. |
| `update_module` | Rename a module and/or set its case type — set the case type before adding registration, follow-up, or close forms to a module created without one. |
...
| Tool | What it does |
|---|---|
| `complete_build` | Finish the build: run the full app review (every rule, including unfinished pieces and attached media), mark the app Complete, and ready its case database. Anything still unfinished comes back as a list — your agent finishes that work and calls again. |
...
<Callout type="warn">
Edits are checked as they happen, and exports are checked again at the door. A tool call that would break the app — a bad expression, a duplicate name, a reference to something that doesn't exist — fails with a message naming each problem, and nothing is saved; fix the input and call again. While an app is a Draft, unfinished pieces are allowed between calls — but `create_form` and `create_module` still require each new form to arrive with its fields (and a case-managing module with its case-list columns), so structure always lands whole. `compile_app` and `upload_app_to_hq` refuse (`invalid_input`) while the app still has unresolved findings, listing each one — a Draft whose content already passes that review exports fine. `complete_build` is what marks the app finished and readies its case database; it isn't a gate the exports wait on.
</Callout>
