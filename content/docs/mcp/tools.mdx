...
lookup-table logic is preserved and remains movable by id, but its unreadable
slots are withheld and a full-shape update is refused rather than dropping
logic the author projection cannot represent.

| Tool | What it does |
|---|---|
| `get_case_operations` | List a form's case operations in execution order, using operation ids and field paths. |
