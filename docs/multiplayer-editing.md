...
> contract, the reconciler, and every invariant in this spec are unchanged. The phase
> bodies, appendix, and test plan below are written against the ORIGINAL Firestore
> substrate — where they name Firestore mechanics (the `acceptedMutations` / `batchDedup` /
> `presence` collections and `onSnapshot`; the `getListenDb()` gRPC client and
> `firestoreListen.ts` / `firestore.ts`; `withFirestoreRetry`; out-of-band TTL policies on
> `expireAt`), read them as the built Postgres equivalents: the `accepted_mutations` /
> dedup / presence tables, the `LISTEN`/`NOTIFY` relay (`lib/db/streamListener.ts` +
> `pg_notify`, tailed by `app/api/apps/[id]/stream/route.ts`), `withAppTx`'s bounded
> deadlock/serialization retry, and permanent retention (no TTL). `lib/db/CLAUDE.md` holds
...
- **Authority + gate.** Every blueprint commit — builder auto-save, MCP, and the chat
  ...
  Postgres transaction re-applies the client `Mutation[]` onto the *fresh* stored doc,
...
- **Transport.** A same-origin Next route (`app/api/apps/[id]/stream`) tails the durable
  `accepted_mutations` table (and presence): a dedicated `LISTEN nova_app_stream` /
  `nova_presence` connection (`lib/db/streamListener.ts`) signals new rows, which the route
  SELECTs since the client's cursor and pipes to the browser as Server-Sent Events. The
  browser carries no database client and no second identity; the route authorizes with the
...
seq + `acceptedMutations` coupling in its own Postgres transaction. Mechanics in
Phase 3.
