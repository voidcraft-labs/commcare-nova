...
The interface omitted an existing capability. `FormEngine`
...
Initialization timing exposed a runtime defect that had to be corrected before
releasing the guide. Exact Nova-exported XML with an earlier default feeding a top-level
query produces rows in Core, while the previous Preview captured the query before defaults and
produced none. Native diagnostic variants distinguish earlier and later defaults:
...
its previous `addRepeat` paths also omitted nested bound initialization. Immediate full native
...
The previous `extractPathRefs` omitted relative `current()/../@id` dependencies, so
...
question ended with the loaded case's different value. An exact-export count
variant likewise retained one row from its default although the loaded answer
ended at three. Existing Preview count tests that seed the loaded answer before
the snapshot do not establish native semantics. None of these gaps should become an instruction asking
...
## Scoped initialization correction

The engine now shares one initialization program across synchronous and worker
execution. Defaults and query snapshots retain authored order, dependent
calculations and relevance bindings run between writes, count snapshots follow, and primary preloads
run last. Initial calculations then settle before rows are created. New outer
rows initialize only their own descendants; existing query snapshots and answers
remain intact. Parsed `current()` paths contribute row-attribute dependencies.

Snapshot evaluation uses an expression-local structural context: a query's
wrapper attribute or a count's generated sibling, linked to the real enclosing
instance. A zero-row repeat therefore retains the parent identity in both
execution paths without inventing an answer row. This context crosses the actual
worker protocol and is not included in entry checkpoints or saved answers.

The public container fixture adds matching named-reference and raw-relative
queries with two parent identities and an empty second result, nested count
snapshots inside a group, and user-controlled insertion after an answer changes.
Core's three new `ContainerRuntimeTest` methods passed against those exact current
CCZ forms at `8e9ba8d908e95f4dc71c9ade0467c6ebfbfbd305`. They execute form entry
and inspect retained values; they do not establish HQ regeneration, Android UI,
submission effects, or first-delivery agent quality. Focused engine/worker and
controller checks pass. Playwright exercised the production renderer, controller
and browser worker: Add created the new membership and retained earlier answers.
The final reviewed CI revision remains a release check.

Delayed entry through two authored sections remains separate unfinished work.
Preview still eagerly creates initial nested rows. The guide and public docs
state that boundary instead of claiming native timing parity.

An independent code review found an omitted relevance cascade. Native exported
fixtures confirmed that a default can hide a field before a later snapshot reads
it; hiding a group must also recalculate consumers of its descendants. The shared
initialization program now retains evolving scope visibility and triggers those
bindings between actions. Sync/worker and Core checks cover both form opening
and insertion, including preservation of an older hidden row.
