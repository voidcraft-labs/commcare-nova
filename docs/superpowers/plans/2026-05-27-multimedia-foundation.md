...
- **Select-option media** — below each option label in `SelectOneField` / `SelectMultiField` (a compact `imageClassName` so a list stays scannable).
- **Hint media** — with the hint, above the input, in `FieldRow` (edit) + `InteractiveFormRenderer` (live).
- **Group / repeat label media** — a banner above the container header, mounted identically in the live renderers (`GroupField` / `RepeatField`) and the edit-mode row (`GroupOpenRow`) for flipbook parity.

Not yet displayed in-builder: help and validation-message media (`help_media` / `validate_msg_media`). Help sits behind a disclosure affordance and the validation message shows only on error, so each needs its own conditional mount — a small follow-up. Wire emit + authoring for both already shipped.
...
`expandDoc` / `compileCcz` take an optional `assets` manifest. **Manifest present → media-ON**: itext media, nav-menu media, logo, and `enum-image` columns all emit `jr://` refs. **Manifest absent → media-OFF**: media slots are skipped and image-map columns degrade to plain columns. Media wire artifacts therefore emit ONLY where the bytes also ship: the `.ccz` compile path, the HQ-upload path, and the JSON-export paths (`/api/compile/json`, MCP `compile_app` json) **when the app has media** — those return a media-ON bundle (the app JSON + the bulk `multimedia.zip`, shared via `lib/commcare/multimedia/hqJsonExportArchive.ts`), falling back to plain media-OFF JSON (byte-identical to pre-media output) for a media-free app, so a media-free export never changes shape. Every media-ON entry point runs `collectMediaValidationErrors` before expand, so a stale/pending/foreign/kind-mismatched ref surfaces as an actionable error instead of a broken on-device reference.
