...
| Module menu tile | ✅ `module.icon` / `module.audioLabel` | ❌ tool-only | ✅ `set_module_media` | ✅ nav-menu media |
| App logo | ✅ `blueprint.logo` | ❌ tool-only | ✅ `set_app_logo` | ✅ `hq_logo_web_apps` profile property |
...
- **Module / case-list / logo inline builder chrome** — there is no per-app module-settings or app-properties builder surface to mount these in; they are reachable through the SA/MCP tools, which fully cover them. Building that chrome is net-new surface, out of scope here.
...
One module the HTTP confirm route and the MCP `upload_media_asset` tool both call. Five stages: (1) extension whitelist (`.png .jpg .jpeg .gif .webp .mp3 .mp4 .wav` — SVG and `.m4a`/`.ogg` rejected); (2) per-kind size cap (image 5 MB / audio 10 MB / video 50 MB); (3) magic-bytes sniff (`file-type`) matched against claim + extension; (4) library re-parse (`sharp` re-encode for images — strips EXIF; `ffprobe` for audio/video); (5) SHA-256 from the validated bytes. Deps: `@google-cloud/storage`, `file-type`, `sharp`, `@ffprobe-installer/ffprobe`, `fluent-ffmpeg`.
...
## HQ upload — per-file, synchronous

`lib/commcare/client.ts`: `uploadAppMedia` POSTs each asset's bytes individually to HQ's per-file multimedia endpoint (`uploadMediaFile` is the internal per-file call; `mediaUploadAssetsFromManifest` derives the upload list). There is **no** bulk-ZIP upload and **no** status poll — the original plan's `uploadMultimedia` / `pollMultimediaStatus` were never built; HQ's bulk path is a separate migration surface. The app is imported FIRST (its id goes in the per-file upload URL), then bytes are POSTed; HQ's `create_mapping` overwrites the placeholder `multimedia_id`s with the couch-assigned ones so refs resolve on the device. A media-byte failure never fails the upload — the app already exists, so per-asset failures degrade to warnings on both the chat route (`app/api/commcare/upload/route.ts`) and the MCP tool (`lib/mcp/tools/uploadAppToHq.ts`). Both apply the media-validation gate before expand.
...
In the builder (`npm run dev`, open a form), the `+ Image` pill next to a question's label opens the picker; dropping a PNG shows a thumbnail within ~2s. A second image attaches to a select option, and the image-map column kind appears in the case-list column picker. Module icon and app logo are set through the SA chat or MCP tools (`set_module_media`, `set_app_logo`) — they have no builder chrome. Compile downloads `<app>.ccz`; the label/option images appear as `<value form="image">jr://file/commcare/…` in the form XML, the module icon as `<media_image>` in `suite.xml`, the logo's `jr://` path in `profile.ccpr`, and the bytes at `commcare/<hash>.png` in the archive. Asking the SA "what assets does this app use?" returns them via `list_media_assets`. Upload to HQ imports the app, then POSTs each asset's bytes per-file; a partial media failure surfaces as a warning while the app still lands. On web apps the form renders the image beside the question prompt and the module shows its icon. (The on-device render is the one link that needs a live HQ.)
