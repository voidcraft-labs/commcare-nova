...
- `docs/superpowers/coverage/2026-05-02-foundation-coverage-matrix.md` (new) — coverage matrix as docs artifact: every spec V1-IN operator (12 distinct Predicate kinds + 14 ValueExpression kinds) cited `file:line` against four compilation surfaces (type checker, on-device XPath, CSQL, Postgres compiler), plus Term family + RelationPath family tables, plus cross-arm verification gates.
