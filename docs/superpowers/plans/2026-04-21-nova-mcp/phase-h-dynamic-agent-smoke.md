...
**Goal:** Verify Claude Code picks up an agent file written mid-session (via the Write tool) when a subagent is spawned immediately afterward. The entire plugin design in Phase I depends on this behavior.

**Dependencies:** Phases A–G. This runs before Phase I because a failing outcome here changes how the skills are written.

---

## Task H1: Probe Claude Code's agent discovery
...
**Pass case:** The subagent returns something like `SMOKE TEST OK — hello`. Dynamic discovery works — plugin skills can use the three-step pattern (fetch / write / spawn) as designed. Proceed to Phase I unchanged.
...
4. Document whichever pattern works; all Phase I skill bodies will need to incorporate it.
...
- Outcome: <PASS — subagent boots with written prompt as its system prompt. Phase I three-step pattern valid.>

OR

- Outcome: <FAIL — Agent tool errors "agent not found". Required workaround: <list the probe step that succeeded>. All Phase I skill bodies amended to include that step between Write and Agent.>
```
...
- [ ] **Step 8: If FAIL, update Phase I skills**
...
git commit -m "docs(mcp): amend Phase I skills with agent-refresh step"
```
