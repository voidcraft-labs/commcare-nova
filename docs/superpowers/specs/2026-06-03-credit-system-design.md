...
### 9b. Delete the orphan — guarded
The delete asserts `re-baselined cost_estimate >= live-read orphan value` as a precondition (April 2026 is a closed month, so its re-baseline writes freely) — finite-and-`>=` or it **throws before any write** — then deletes `unadjusted_estimate` from `usage/w4KlwedcG1WijXOK0hVz/months/2026-04`. That precondition (not procedural separation) is the real safety mechanism: the orphan is removed only once `cost_estimate` already records ≥ its magnitude. So the delete may run **either** as a separate `--apply` after the re-baseline is confirmed (the conservative default) **or** combined in one `--apply` with `--rebaseline-cost` — the migrator dispatches `--delete-orphan` LAST, so April's cost folds before the precondition reads it. (The shipped `migrate-actual-cost.ts` permits the combined run for exactly this reason.)
...
Scan → review deltas + cross-month + current-month-over-$50 flags with user → `--apply` re-baseline (closed months auto; current-month writes only for users the user confirms) → capture output → `--apply` orphan-delete (separately, or combined with the re-baseline in one `--apply` — the migrator dispatches it last so the fold precedes its precondition) → `git rm` both migration scripts. Note: today's resets are **manual Firestore hand-edits** (there is no committed reset script to remove); a private uncommitted `scripts/reset-usage.ts` exists on the author's `main` working tree and is deleted there once the admin endpoint ships — it is not in version control and is out of this PR's scope. `.env.example`'s `MONTHLY_SPEND_CAP_USD` block is removed in this PR (no other config depends on it).
