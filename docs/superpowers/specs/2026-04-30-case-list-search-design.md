...
- Claim condition (Predicate AST) — compiles to `additional_relevant` AND-composed with the base guard. CCHQ's authoring-side `additional_relevant` UI is gated `CASE_SEARCH_DEPRECATED`; Nova ships the same runtime-alive field via the structured `claimCondition` predicate editor. No separate "skip already-owned" toggle — the semantic, when desired, is authored as XPath inside `claimCondition`.
