...
The implementation uses **Option B** (direct edges + recursive CTE on read) for write predictability. If profiling shows recursive CTE dominates query cost, switch to Option A — the read-path code is the same; only the materialization trigger changes. Both options are within the same architectural commitment.
