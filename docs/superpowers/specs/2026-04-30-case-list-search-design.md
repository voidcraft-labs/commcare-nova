...
- **Closest-form emission.** AST shape has no exact CCHQ-wire equivalent; the emission is the closest CCHQ form, with the AST's strict semantic preserved on Postgres. The canonical case is `is-null(prop)` and `is-blank(prop)`: both emit as `prop = ''` because CCHQ's `case_property_query` short-circuits absent / cleared / empty alike inside `case_search.py::case_property_query`. The Postgres runtime executes the strict semantic; the CCHQ wire emission collapses to the broader match. This is the wire's lossiness, faithfully emitted.
...
- Search-screen display labels — `searchScreenTitle`, `searchScreenSubtitle`, `emptyListText`, `searchButtonLabel`, `searchAgainButtonLabel`. All free-text strings; the subtitle renders through CCHQ's runtime markdown formatter, the rest as plain text.
...
- Excluded owner ids (advanced cluster) — `excludedOwnerIds`, a `ValueExpression` evaluating to a space-separated list of owner ids whose cases are excluded from the search-results scope. Wire form: `<data key="commcare_blacklisted_owner_ids" ref="..."/>` on `<query>` — the wire token is CCHQ's literal vocabulary (`CASE_SEARCH_BLACKLISTED_OWNER_ID_KEY`); Nova's authoring vocabulary is `excludedOwnerIds`. Niche affordance; Nova's UI hides it behind a collapsed "Advanced" section in the case-search workspace. There is no claim-condition authoring affordance — CCHQ's runtime fires the case-claim step automatically with the default guard `count(...) = 0`, regardless of any author input. The CCHQ field a Nova claim-condition editor would compile to (`additional_relevant`) is gated `CASE_SEARCH_DEPRECATED` for authoring upstream.
...
- **Existing apps that have columns referencing non-existent case properties.** Owned by Plan 7 (`2026-05-07-case-list-schema-reshape.md`) Task 8 — the v0+v1 → v2 migration script that absorbed the original Plan 3 Task 15. The migration's `--dry-run` mode reports references that don't resolve; review before running live.
...
- **Plan 4 (Search authoring):** Module schema for `caseSearchConfig` (display labels + `searchButtonDisplayCondition` predicate + the "Advanced" cluster's `excludedOwnerIds` value-expression — filter and sort reuse `caseListConfig`) + case-search-config workspace UI + 2 wholesale SA tools + platform-aware compilation + wire emission for `<remote-request>` + the hard-coded `<post>` claim element + dual-detail block emission (`m{N}_search_short` / `m{N}_search_long` carrying identical content to the case-list detail blocks).
