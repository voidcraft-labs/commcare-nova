...
ever took effect) across the whole fleet, soft-deleted apps included, because
every app read runs the absolute commit gate and the migration probe audits
every `apps` row. The sequence, each step gated on the one before it, is the
read-only scan, the maintenance image, the Job plan and apply, the Job dry
run, the Job execute (a production write; confirm first), and the scan again
...
`CLEAN` with exit code 0. A blocked app needs its own owned repair. What this
repair unblocks, and when its scripts leave, is
