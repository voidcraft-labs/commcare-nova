...
discriminator is whether the target loses the state silently or refuses it
loudly, because that decides whether an author can ever find out:

> **Silent loss on the primary delivery path ⇒ exclude it from constructible
> state. Loud refusal on a secondary path ⇒ construct it, preflight it as a
> required prerequisite, and say so where the author creates it.**
