...
- [ ] **Step 1: `user-usage.tsx`** — add the four per-period credit columns spec §8b enumerates (Allowance / Credits used / Bonus / Balance), in that order, beside the cost column: **Allowance** (`period.credits_allowance ?? 0`), **Credits used** (`period.credits_consumed ?? 0`), **Bonus** (`period.credits_bonus ?? 0`), **Balance** (derived: `(credits_allowance ?? 0) + (credits_bonus ?? 0) − (credits_consumed ?? 0)`). All `.toLocaleString()` + `tabular-nums`. `UsagePeriod` carries `credits_allowance?`/`credits_consumed?`/`credits_bonus?` (absent for pre-credit periods → render 0); `getAdminUserUsage` projects all three from each credit-month doc; balance is derived in the render (not stored). **Totals row** summing the section's OWN periods: Σ `credits_consumed ?? 0` (= lifetime credits used) and Σ `cost_estimate` (= lifetime cost); the Allowance + Balance totals cells are BLANK (a per-month constant + a non-summable derived figure don't total meaningfully — like the blank Generations/Tokens cells). Source the totals from the periods the table already renders (not a second fetch). *(The four-column §8b set landed via the final whole-branch review — Task 12 originally shipped only consumed+bonus; the review caught the enumerated-spec drift.)*
...
**Files:** Modify `lib/db/usage.ts` + `.env.example` + root `CLAUDE.md`; **Create** `lib/db/CLAUDE.md`. *(Plan corrected against reality: `MONTHLY_SPEND_CAP_USD` is now DEAD code — zero importers after Tasks 7/8 replaced the dollar cap with the credit gate + `ACTUAL_COST_BACKSTOP_USD` — so it's removed, not just the env var. `lib/db/CLAUDE.md` does NOT exist (10 other `lib/*` packages have one) → CREATE it. Root `CLAUDE.md` has NO "spend cap" wording → ADD a credit note, not update. `content/docs/` (the real docs home; `app/(docs)/` only renders it) has ZERO cap/usage/credit mentions — credits gate the builder, not MCP/plugin — so the docs conversion is a verified no-op, nothing to change.)*

- [ ] **Step 1:** Remove the dead `MONTHLY_SPEND_CAP_USD` constant + its doc-comment from `lib/db/usage.ts` (verified: zero importers — the chat gate uses `ACTUAL_COST_BACKSTOP_USD`, the usage route returns `getCreditSummary`). KEEP `getMonthlyUsage` (the backstop still reads it). Remove the `# ── Usage tracking ──` block (`MONTHLY_SPEND_CAP_USD=15` + its comment) from `.env.example`.
- [ ] **Step 2: Create `lib/db/CLAUDE.md`** — match the terseness/voice of the sibling `lib/*/CLAUDE.md` files. Document the **two-ledger credit model**: `usage/{userId}/months/{period}` = ACTUAL dollar cost, accumulate-only, **resets never touch it**, read by the invisible `$50` (`ACTUAL_COST_BACKSTOP_USD`) backstop; `credits/{userId}/months/{period}` = the **resettable** gate (`allowance 2000 + bonus − consumed`; a missing doc reads as full 2000 everywhere); `credits/{userId}/grants/{id}` = append-only admin audit (reset/grant, written in the same transaction as the balance change). Charge signal: `isChargeableTurn` reads the **raw `body.messages`** last role (a continuation ending `assistant` is free), amount `chargeAmount(appReady)` = build 100 / edit 5. Reserve-before-run (transaction debits up front, no overshoot); refund on failed-run OR zero-cost no-op via `UsageAccumulator.flush()`; a failed run STILL accrues actual-$ (backstop sees retry-spam). `creditPolicy.ts` is **client-safe** (pure, no Firestore — the gate, the send-button chip, AccountMenu all import it); `credits.ts` is the **server** ledger (Firestore transactions). The gate's abort-finalization invariant: the execute `finally` is the sole authoritative `usage.flush()`; the disconnect listener flushes only the log writer; `handleRouteError` short-circuits on `req.signal.aborted`. No line numbers, no external-doc references.
- [ ] **Step 3:** Root `CLAUDE.md` — ADD a concise credit-gate line to the Firestore/data-model area: the two-ledger model (accumulate-only `usage/` + resettable `credits/`) + build-100/edit-5 reserved-up-front + the invisible `$50` backstop, cross-referencing `lib/db/CLAUDE.md` for detail. Keep it brief (one entry, not a section). No line numbers / external-doc refs.
