...
> **For agentic workers:** Use the frontend-design skill and subagent-driven development to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**Migration script (`scripts/migrate-case-list-config.ts`):** reads each live, complete Firestore app doc (server-side filter on `deleted_at == null` AND `status == "complete"`), transforms `{ field, header }[]` into `Column[]` with `kind: "plain"`, preserves any pre-existing `filter` / `sort` / `calculatedColumns` / `searchInputs` / `detailColumns` carried by a partially-migrated `caseListConfig`, and routes the rewritten config through `caseListConfigSchema.safeParse` before any Firestore write so a malformed legacy entry is rejected rather than silently round-tripped. Idempotent at both per-module and per-blueprint granularities. `--dry-run` flag prints per-app + per-module diff lines without writing. `--app-id=<id>` flag drives a surgical retry that bypasses the bulk-query filter. Per-app `try / catch` so a single bad doc cannot abort the run; the run summary increments `failedCount` and the process exits non-zero on any failure so CI / automation can detect partial-failure runs. Companion test suite at `scripts/__tests__/migrate-case-list-config.test.ts` (21 tests) covers legacy, already-migrated, never-had-columns, mixed shape, filter preservation, blueprint-level idempotency, schema-parse-of-output guard, deleted-app skip, generating-app skip, malformed-data safe-parse failure path, single-app failure isolation, surgical-retry filter bypass, and the CLI argument parser. Operator-run; Task 15 owns the live execution; the script archives after that.
...
**Tests:** schema parse (12 tests covering empty / populated / per-kind invalid combinations / silent-strip of legacy keys), classifier (14 tests covering every diff branch), migration script (21 tests), saga integration (7 tests using `setupPerTestDatabase` against real Postgres testcontainer — happy-path additive, runId routing to `updateAppForRun`, fast-path on non-case-type mutations, retype-with-quarantine via hint, Firestore-commit-failure compensation on additive change, retype mid-migration rollback proof per spec line 129, case-type-addition compensation via `dropSchema`).
...
**Display arrangement** — the authoring surface renders the configured labels and ordering directly. It does not fetch a random case row to decorate each field; actual values belong to the running Preview, where they retain their case context.
...
**`loadFilterPreviewAction`** — Server Action that resolves session first, Zod-parses both `caseListConfig` and `blueprint`, then queries a truncated row sample plus the full matching count. Discriminated arms: `rows` (with `totalCount: number`, possibly empty `rows` array) / `unauthenticated` / `error` / `invalid-config` / `invalid-blueprint`. The paired sample and count stay internally consistent across renders.
