...
labels on form fields and Search prompts, Connect text, shared media, and recipient-owned automation messages are
...
`fieldValueType` derives a hidden writer's value type from the effective case
property catalog; visible questions retain their field-kind type. Preview and
emission use that projection to preserve datetime calculations and defaults.
It adds no authored type setting. A hidden value without a datetime destination
keeps generic XPath string conversion, which represents a Date without its clock.
...
Naive datetime answers use compatible instant disambiguation: a repeated local clock chooses the earlier occurrence, while a skipped local clock advances by the transition gap. Explicit stored offsets remain authoritative. Test the resulting UTC instant against an independent native resolver; a normalized wall clock's offset alone is not the offset to stamp onto unchanged input text.
...
Each field kind is one file under `fields/`, and the union (`fieldSchema`) discriminates on `kind`. Each kind's schema declares ONLY the slots it actually has — which is the structural reason a wrong-property-for-kind state is unexpressible (it's also why the SA's per-kind tool arms can't carry a slot the kind lacks). `kinds.ts` holds the per-kind metadata table (`FieldKindMetadata` — XForm control + data type, icon, label, `convertTargets`, the three-section editor schema) that the compiler, validator, editor panel, and SA tool-schema generator all read from ONE place: **adding a kind is one `fields/` file + a registry entry; adding a property is one schema field + one editor entry.** `captureFieldKinds` is the one sub-grouping the registry can't express: the five kinds whose answer is an attachment rather than a value (image / audio / video / signature / file). The reference-slot applicability group, the emitter's `mediatype` table, and the capture-only `caseWrite` shape all read that tuple, so a kind cannot be a capture on one surface and an ordinary field on another. Those five extend the ordinary destination with `captureCaseWriteSchema`, whose required `mode` names what reaches the case: a capture's answer is a file name, so it can never BE the scalar case value. `"url"` stores a link built from the published project space; `"attachment"` stores the file in CommCare's case-attachment block and therefore derives the corresponding project-space capability requirement. Requiring the member is what makes a destination-without-a-mode unrepresentable; the same tuple is why an attachment is refused the two standard scalars (`case_name`, `external_id`), which bypass the update map through their own FormActions slots and would point back at the capture question. `saDocs` on those five is load-bearing prose, not a summary: Web Apps has no camera, microphone, or recorder, so every one says "attach" and none says "take" or "record". A `hidden` field carries exactly ONE value source: `calculate` (re-evaluated on every load and whenever a referenced value changes) or `default_value` (a one-shot seed that fires only when a form instance is first opened, so it survives save-and-resume unchanged). JavaRosa evaluates every calculate after the `xforms-ready` seeds, so a default beside a calculate is dead; the validator's `HIDDEN_VALUE_BOTH_SOURCES` refuses the pair at every commit and every authoring surface refuses to write it, and `fields/hidden.ts::hiddenFieldCarriesBothValueSources` is the one recognizer, shared by the rule and the tool boundary. A hidden field with nothing authored holds `HIDDEN_INERT_VALUE` in `default_value`, never in `calculate`: an inert calculation on a writer to the loaded case would write nothing over the case's value on every submission, where an inert default is replaced by that value before anyone reads it. The three containers (`group`, `repeat`, `section`) are kinds, not a parallel tree — their children are the fields whose `fieldOrder` entry names them. A `section` is one page of a form: its own kind rather than a flag on group because it has no slots at all (`structuralFieldBase` + an optional title, strict: no `relevant`, no media), lives only at a form's root, and is always `appearance="field-list"` on the wire; `isContainer` / `isContainerKindName` are the predicates every container site reads; engine section/group/repeat traversal tests exercise the actual runtime consumers. New container kinds still require reviewing dispatch sites.

## Forms and modules

**Four form types** (`forms.ts`): `registration` creates a case, `followup` updates one, `close` loads + closes (a superset of followup), `survey` touches no case. Use the centralized sets — `CASE_FORM_TYPES`, `CASE_LOADING_FORM_TYPES` (`{followup, close}`) — never ad-hoc string comparisons. `casePreload.ts` is the ONE statement of which answers a form seeds from the case it opened: `formOpensWithOneCase(formType, cardinality)` and `writerPreloadsFromLoadedCase(field, module, form)` (a scalar writer to the module's own type on a one-case followup or close form, `case_name` included; never a child-type, worker-record, or capture writer, never on a several-case form). The builder's inspector, its pure guidance helpers, and the preview engine all read it, so the rail and the running form cannot disagree; the compiled XForm reaches the same answer through the case-write inventory (`lib/commcare/deriveCaseConfig.ts`). `isCaseFirstModule` mirrors `commcare-core`'s `getDataNeededByAllEntries` exactly (a module lands on its case list only when every form is case-loading); `defaultPostSubmit(formType, { searchFirst })` is the form-type-aware navigation default, and it is module-aware: a followup/close form in a search-first module (`modules.ts::moduleOpensOnSearch`) defaults to `module`, not `previous`, because the previous screen was the search. `postSubmit.ts::effectivePostSubmit(doc, formUuid)` / `defaultPostSubmitOf` are the doc-level readers; every consumer that needs a form's destination (the expander, the link planners, Preview) goes through them rather than calling `defaultPostSubmit` with the type alone. A form's optional `entry` (`{ kind: "search-no-matches", label? }`) says how it is reached: absent is a menu form; `search-no-matches` is a registration form offered on Results only after a search found nothing (`isNoMatchesForm`), on no menu at all. `menuForms.ts` holds the readers every menu projection uses (`menuFormUuidsOf`, `noMatchesFormOf`, `owningModuleOfForm`, `moduleIsCaseFirst` over MENU forms only, so a no-matches form never turns a search-first module forms-first) and `searchInputNameResolver`, the only binder of the XPath leaf `search-answer-ref { searchInputUuid }` (`#search/<name>`, `xpath/ast.ts`), which resolves inside a no-matches form alone. `translationUnits.ts` carries the `form-entry-label` role (source `entry.label ?? form.name`).
...
A `Module` (`modules.ts`) carries an optional `caseType`, the `caseListConfig` (a `Column[]` of eight kinds + an optional `filter` predicate + `searchInputs` + optional bounded `selection`), and the `caseSearchConfig` (search-screen display + niche filters). `selection` stores only `{kind: "multiple", maximum: 1..100}`; absence is the canonical one-case flow. `caseSelectionCardinality` and `caseSelectionMaximum` are the shared effective projection and stay separate from `isCaseFirstModule`, which answers navigation order rather than datum multiplicity. These structured configs are the single source of truth every case-list surface reads — validator, wire emitters, SA tools, and the case-list workspace UI. Their AST-typed slots (the filter, calculated-column expressions, search-input predicates/defaults) come from `lib/domain/predicate`. Search inputs are an exact stored union: text, date, date-range, barcode, a lookup-backed choice (`select` / `multi-select`), an advanced predicate over any of those widgets, or a hidden system value. The union is seven arms over two independent axes — kind (simple | advanced | hidden) and widget (scalar | date-range | select) — which is what makes a range default, a scalar range mode, and a choice list without a table unrepresentable. Simple inputs require a nonblank case property. A SIMPLE date-range input requires range mode; the ADVANCED date-range arm declares no `mode` slot at all and is `.strict()`, so it carries none. Neither date-range arm has a scalar default. A choice input stores `options: lookupOptionsSourceSchema` verbatim (the same table / value-column / label-column identity a lookup-backed question uses, plus an optional row filter over the table's columns and globals) and admits only `exact` mode; Nova never authors an inline choice list on the Search screen because CommCare's prompt reads choices from a fixture only. Every VISIBLE arm may carry `hint`, `required` (`{}` always, or `{when: Predicate}` with an optional `message`; `searchInputRequiredMessage` supplies Nova's default sentence so no runtime falls back to its own words), and one `validation` (`{rule: Predicate, message}`; Core keeps one per prompt). The `hidden` arm is `{kind: "hidden", uuid, name, label, value: ValueExpression}` on the bare base: it has no property, no widget, no hint, no required, no validation, and never filters. It exists to be read by name (`input(uuid)` in a sibling's condition, `#search/<name>` in the module's no-matches registration form). `searchInputRuntimeValueType(input)` is the ONE reader of an input's runtime value type; a hidden input is text. Three slots resolve GLOBALLY — once, before any case is selected — so property and relationship reads are invalid in them: `caseSearchConfig.excludedOwnerIds`, `caseSearchConfig.searchButtonDisplayCondition`, and each scalar `searchInputs[].default`. Literals, session/current-user values, and pure calculations over them are valid (Search answers additionally so for `excludedOwnerIds` only). Two more slots resolve on the SEARCH SCREEN — after the worker types, before any case is selected: `required.when` and `validation.rule` may read the input itself and its siblings by bare `input(uuid)` (blank while unanswered, so `is-blank(input(sibling))` asks whether it was answered), session/current-user values, and literals, never case data. A hidden `value` resolves when the screen opens, so it may read neither case data nor another input. The shared semantic guards are `expressionReadsCaseData` / `predicateReadsCaseData` in `predicate/walk.ts`, consumed by the validator rules, the SA/MCP boundary schemas, and the builder's global-scope pickers. Owner-only availability is the exact `{ searchActionEnabled: false, excludedOwnerIds }` arm; it cannot carry Search-screen presentation, inputs, or an inferred Never condition. `searchFirst: true` on the ordinary arm (`moduleOpensOnSearch`) opens the module on its Search screen with no browse list; the validator admits it only on a case-first or forms-free case list, and it is the one module fact `defaultPostSubmit` reads. Sort lives per-column (direction + priority); the comparator TYPE is derived at wire emission from the property's `data_type`, never authored.

A multiple selection is meaningful only where a follow-up or close form
consumes it. A form-less case-list-only root may hand the complete set to its
...
Typed `ValueExpression` arithmetic preserves the existing `int`/`int` → `int` contract, including division truncated toward zero; a decimal operand produces decimal arithmetic. Runtime compilers project that admitted type through `predicate/numericType.ts`, using declared field, property and lookup-column types rather than inferring from the current value. Runtime form bindings and folded lookup values preserve those declarations. Raw XPath keeps its independent native division semantics. Zero division, floating-point precision and evaluator range limits are not generalized into a cross-runtime parity promise.

XPath-bearing slots store the typed AST from `xpath/` — **references are identity, text is a projection** (`printXPath`); field and custom-worker-property renames never rewrite stored expressions, and human-authored text remains friendly (`#form/first_name`) rather than exposing UUIDs. The Predicate / ValueExpression AST (`predicate/`) is the boolean + typed-value family behind filters, calculated columns, and search, and carries the parallel UUID-backed custom-worker arm. Connect is a per-form opt-in (`form.connect`) gated by the app-level `connectType` (`learn` | `deliver` | null). Every stored Connect sub-config has a required nonblank id, matches the app's Connect mode, and participates in app-wide id uniqueness across Learn and Deliver sub-kinds. Local editor/tool drafts are separate types; they receive their final id before entering the document, and emitters only assert the already-complete invariant. The media primitives (`MediaAssetId`, `Media`, MIME partitions, size caps, the export ceiling, GCS key derivations) live in `multimedia.ts`; MIME alias and extension lookups accept only their own declared keys, so inherited JavaScript names cannot become MIME values. The verdicts, manifest, and wire emission live in `lib/media`.
...
`entryPoints.ts` owns the optional destination-owned singleton identities:
`Module.entryPoint`, `Module.caseListEntryPoint`, and `Form.entryPoint`.
Every entry point has an immutable global authored UUID and an app-unique
external `id`; the form variant alone permits `ignoreDisplayConditions: true`.
Absence preserves historical documents without backfill. `entryPointInventory`
walks destination order and derives ownership; labels and suggested IDs come
