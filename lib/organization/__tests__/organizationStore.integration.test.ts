...
describe("locations store — the tenant boundary", () => {
	...
	it("carries every place across a Project move, unchanged and still reachable", async () => {
		// The behavioural half. The rows key on `app_id`, so flipping the app's
		// Project moves them by construction — and the scope check is what makes
		// a stale source-Project authorization stop working.
		await seedOrgApp();
		const { facility } = await seedChain();
...
describe("locations store — reference edges", () => {
	...
	it("refuses a commit assigning a persona to a place that does not exist", async () => {
		...
		).rejects.toThrow(/no longer exists in this app's organization/);
	});
});
	...
	it("refuses while a place still stands at it", async () => {
		...
		await expect(
			commitGuardedBatch({
				...
				mutations: [{ kind: "removeOrganizationLevel", uuid: FACILITY }],
				actorUserId: ACTOR_A,
				kind: "autosave",
				expectedProjectId: PROJECT_A,
			}),
