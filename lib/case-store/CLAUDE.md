...
A real Postgres engine boots once when a `*.postgres.test.ts`/`.tsx` test
is selected. Ordinary unit tests in this package do not need Docker. The harness lives entirely
...
1. **One container per selected Postgres project, not per test file.**
