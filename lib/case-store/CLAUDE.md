...
**Two case-type map flavors.** `buildCaseTypeMap` (query/validation — the domain's FULL effective view, standard properties included, so the compiler resolves everything the type checker admits) vs `buildMaterializableCaseTypeMap` (every `applySchemaChange` feeder — the effective view minus the implicit standard entries, so JSON-schema insert validation and the per-property index DDL never grow constraints/indexes for runtime-metadata keys inserts don't carry). Feeding a schema write from the full map is a defect — it mints a GIN index per text-typed standard name on every case type.
