...
**Runtime operation targets are resolved facts, never client descriptors.** `caseOperationTargetRequestSchema` accepts only `{ caseId }`. A future S06/S07 executor loads the row through the tenant-bound pre-submission snapshot and then calls `validateCaseOperationTargetDescriptor(request, resolved, expected)` with server-owned `{ caseId, caseType, projectId }` plus the expected `{ projectId, snapshotCaseType }` from `caseOperationExpressionSnapshotTypes`. That snapshot type is intentionally distinct from the operation's rolling semantic type after an earlier retype. Parse failure, absence, id mismatch, and foreign tenancy intentionally collapse to `not-found-or-out-of-scope`; a type mismatch is reported only after Project authorization succeeds. This seam is pure and preparatory in S04 — no operation executor or new write path is active yet.

**Case ids are opaque CommCare wire identities, not intrinsically UUIDs.** S04 deliberately admits deterministic ids such as `nova-case-v1:<namespace-uuid>:external-123`; `idFrom` is an exact authored key inside that Nova-owned app/form/operation/type namespace, never a raw global CommCare id. Narrowing Nova's domain to the current local table type would still leak an accidental storage choice into authoring and break import/wire fidelity. The current `cases.case_id`, `parent_case_id`, `case_indices.{case_id,ancestor_id}`, and `parked_case_values.case_id` columns are still `uuid`, so the activation gate is also a storage-safety gate. S06 must ship a scan-then-migrate widening of that entire identity/FK/index/default family to `text` (preserving generated ids as `uuidv7()::text`) and remove UUID-array/value casts. That is not sufficient alone: `caseDataBindingHelpers.ts::readCaseData` still rejects non-UUID ids; current default/tie-break paging relies on UUIDv7 lexical insertion order, which authored strings do not carry; and every route/raw path segment must encode and decode opaque ids symmetrically. S06's atomic executor must call the shared `deriveAuthoredCaseId` helper and surface its blank/over-205-key failures before writing, so Preview and the XForm calculation implement the pinned identity vector exactly. Every evaluated create-name, rename, explicit owner, and default owner must also pass through `prepareCaseOperationTextValue` before DML; store only its normalized value and abort the envelope on `blank` or `too-long`, matching S04's XForm calculate and atomic guard rather than relying on the looser current Postgres columns. It must then expand repeats into physical execution order and call `validateResolvedCaseOperationTypeSequence` over the server-authorized descriptors before any write. That fold keys rolling type by concrete opaque id, checks every link's concrete identity (rejecting self-links) and rolling type before each effect installs its result type, catches aliases the AST cannot prove plus duplicate repeat values, and enforces the `nova-case-v1:` type-stability rule mirrored by the XForm retype guard. Validating each descriptor independently is explicitly insufficient after a retype. S06 owns those audits and the explicit durable ordering replacement, with acceptance coverage for URL-significant non-UUID ids through CRUD, relations, paging, parking/review/restore, retenancy moves, and the atomic submission envelope. Until the migration, parser, ordering, path, shared-derivation, and rolling-type-fold work all land and the rescan is clean, no operation-bearing candidate reaches the case store.
...
The dormant lookup-reference migration additionally owns a database-level
rolling-writer cutoff. Its statement triggers read
`nova.writer_version` (unset/empty means version 0) and lock the permanent
compatibility singleton `FOR SHARE` before guarded app/entity/history/lookup
writes. That lock is the cutoff protocol: a write that wins may finish before a
floor raise, while a floor raise that wins makes the stale writer wake and fail
with non-retryable SQLSTATE `55000`. Runtime code declares a version only via
`lib/db/pg.ts::setTransactionWriterVersion` inside the same Kysely transaction;
the helper uses transaction-local `set_config`, so commit and rollback both
clear it on a reused pool connection. Do not replace it with session-scoped
`SET`, weaken the singleton's delete/truncate guards, or read the floor without
the lock.

That migration is intentionally replay-idempotent because the migration
adoption tests erase Kysely's ledger and replay the complete chain over an
existing schema. Keep its guarded constraint addition, `IF NOT EXISTS` tables
and indexes, `ON CONFLICT` singleton seed, replaceable functions, and
