...
When the server supplies a capture intent, the same transaction first claims
the `(app, Project, actor, entry_key)` idempotency row, validates the app
sequence and exact structured attachment references, and moves only those
