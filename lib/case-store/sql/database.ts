...
export interface CaseTypeSchemasTable {
	...
	case_type: string;
	/**
	 * Whether the case type is present in the committed Blueprint. Retiring a
	 * type flips this false instead of deleting the row: ordinary validation
	 * treats an inactive row as absent, while reactivation can still diff from
	 * the last active contract and `synced_seq` fences delayed stale syncs.
	 */
	is_active: GeneratedAlways<boolean>;
	/**
	 * Latest Blueprint sequence that retired this type. `is_active` is generated
	 * as `retired_seq IS NULL OR synced_seq > retired_seq`, which makes a newer
	 * schema sync from either the current or previous application revision a
	 * reactivation while every stale/equal sync remains fenced.
	 */
	retired_seq: ColumnType<
		string | null,
	...
	/**
	 * The `mutation_seq` this schema row was last synced OR retired from — the
	 * monotone guard that makes concurrent additive case-type edits
	 * converge. `applySchemaChange` accepts a greater incoming seq, or an equal
	 * seq only while the row is active. A stale sync therefore cannot clobber a
	 * peer's concurrently-added property or reactivate a retired type.
	 *
...
/**
 * Durable Phase-B work for a maintenance hard purge. Ordinary case-type
 * retirement keeps an inactive schema row and records pending index work on
 * that row; a purge deletes it, so this tombstone remains until every scoped
 * expression index has been observed absent. A later schema recreation clears
 * the tombstone under the same per-type advisory lock and writes its own
