...
export interface CasesTable {
	/**
	 * Stable case identifier. UUID v7 generated by Postgres via the
	 * column's `DEFAULT uuidv7()` clause when the caller omits
	 * `case_id` on insert; v7's millisecond-timestamp prefix means
	 * INSERT order clusters tightly on the B-tree primary-key
	 * page, and pagination by `(opened_on, case_id)` is naturally
	 * close-to-sorted because the timestamp prefix already orders
	 * rows by creation time. Callers MAY supply an explicit value
	 * — useful for cross-system traceability with CCHQ-style case
	 * ids — and the database accepts whatever the caller provides
	 * verbatim.
	 *
	 * The `ColumnType<select, insert, update>` triple expresses
	 * the database-generated default to Kysely:
	 *
	 *   - `string` (select) — every row carries a non-null
	 *     `case_id` by primary-key constraint.
	 *   - `string | undefined` (insert) — `undefined` tells Kysely
	 *     this column is OPTIONAL on insert; omitting it lets
	 *     Postgres's `DEFAULT uuidv7()` clause fire. Supplying a
	 *     string forwards the explicit id verbatim. Without the
	 *     `undefined` branch, Kysely would force every insert
	 *     site to construct a `case_id`, defeating the database-
	 *     generated default and removing the `RETURNING case_id`
	 *     pattern's only payoff.
	 *   - `string` (update) — UPDATE carries the explicit
	 *     case_id; the primary key is not updateable in practice
	 ...
	 * The `case_id` migration shape lives at
