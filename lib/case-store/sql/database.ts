...
//     `CaseType.properties[].data_type` and used as both the
//     write-time validator (defense-in-depth Postgres trigger) and
...
export interface CasesTable {
	 ...
	 * `case_type_schemas[app_id, case_type]` JSON Schema by both
	 * application code (primary) and a Postgres BEFORE
	 * INSERT/UPDATE trigger (defense-in-depth). The Term compiler
...
export interface CaseTypeSchemasTable {
	 ...
	 * `lib/domain/predicate/jsonSchemaGenerator.ts` from the
	 * blueprint's `CaseType.properties[]`. The application layer
	 * validates `cases.properties` against this schema before
	 * every write; a Postgres trigger validates as
	 * defense-in-depth.
	 *
