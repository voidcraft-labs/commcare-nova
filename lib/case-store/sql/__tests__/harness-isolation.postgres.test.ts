// lib/case-store/sql/__tests__/harness-isolation.postgres.test.ts
//
// Sibling smoke file paired with `harness.postgres.test.ts`. Two contracts
...
//      well-formed value, mirroring `harness.postgres.test.ts`. Both files
...
describe("case-store harness — cross-file invariants", () => {
	test("inject() returns the same connection URI as the sibling file", () => {
		...
		// We can't assert "same as harness.postgres.test.ts" without
