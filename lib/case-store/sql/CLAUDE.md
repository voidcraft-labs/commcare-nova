...
- `datetime-coerce` must match CCHQ CSQL rather than Postgres's session defaults: a date-only value means midnight UTC even when the connection `TimeZone` is not UTC. Full datetime values still use `timestamptz` parsing so an explicit `Z` or numeric offset wins. Keep the non-UTC Postgres harnesses for both the coercion and the exact/range calendar-search lowerings.
