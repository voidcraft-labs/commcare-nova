...
typed-builder calls Postgres executes natively.
...
- **`compileRelationPath`** — `case_indices` + `cases` join chains.
  ...
  `(app_id, owner_id)` tenant filter — a missing filter on any
  intermediate JOIN would let a relation walk reach a row outside
  the bound owner's tenant.
