...
- **`compileTerm`** — leaf-level value reads. JSONB property reads
  ...
  column reads for the reserved scalar-column set
  (`RESERVED_SCALAR_COLUMNS`; both tables live in
  `dataTypeTokens.ts`), parameter-bound runtime bindings (`input` /
  `session-user` / `session-context`), and correlated scalar
  subqueries for non-self via reads. Delegates literal emission to
  the sibling `compileLiteral` helper.
...
- **`dataTypeTokens`** — data-only module owning the two
  ...
  compiler reads: `POSTGRES_CAST_FOR_DATA_TYPE` (Kysely-recognised
  `ColumnDataType` literal per arm) and the package-internal
  `JSONB_READ_OPERATOR_FOR_DATA_TYPE` (`->>` text vs `->` jsonb
  per arm). `compileTerm` and `compileLiteral` import from this
  module independently; no edges between any pair of compiler
  modules. Extending the blueprint's `data_type` enum surfaces a
  `Record<...>` exhaustivity error in this one place rather than
