...
// Rehearse the migration on a fresh clone of its actual historical prefix.
// Only the precondition is prepared once; the cutover still runs in every case.
const scratch = setupPerTestDatabase({
	databaseNamePrefix: "canonical_identity_",
...
async function seedLegacyApp(
	...
	// topology. The prepared template installs that exact external schema
	...
	`.execute(db);
	await sql`
		INSERT INTO presence (app_id, user_id, session_id, name, color, location, expire_at)
		VALUES (${appId}, 'fixture-user', 'fixture-session', 'Fixture user', '#123456',
...
describe.sequential("canonical identity database migration", () => {
	test("refuses an unowned app dependency before changing persisted state", async () => {
		...
		await expect(runFrozenCaseStoreMigrations(scratch.db)).rejects.toThrow(
			/unowned apps dependency relation/,
		);
	...
	test("atomically rewrites, archives, horizons, clears, and converts the frozen fixture", async () => {
		...
		// Independent UUIDv5 vectors for the frozen namespace and legacy IDs.
		expect(source?.options.map((option) => option.uuid)).toEqual([
			"da549ae6-255c-5ee9-8e6c-da4cd9334a5a",
			"b797a817-7983-5625-a085-7dcd10a33f19",
		]);
		expect(field.rows[0]?.data).not.toHaveProperty("options");

		const cases = await sql<{ external_id: string; properties: unknown }>`
			SELECT external_id, properties FROM cases WHERE app_id = ${APP_ID}
		`.execute(scratch.db);
		expect(cases.rows).toEqual([
			{
				external_id: "COLUMN01",
		...
		expect(appChanges.rows[0]?.row_text).toBe(beforeAccepted.rows[0]?.row_text);
	...
	// The frozen implementation has four adjacent failure hooks after the same
	// write. One case proves that rollback boundary; the remaining entries
	// each follow a distinct write stage.
	...
	test("reports the exact shared occurrence projection before and after migration", async () => {
		...
		const latestOnly = await captureFrozenStorageSnapshot(scratch.db);
		// A retained older baseline must not duplicate or repartition exact payloads.
		// Admission normally creates only a current marker; this fixture represents
		// the historical baseline that a later compaction leaves behind.
		await sql`ALTER TABLE app_change_fold_baselines DISABLE TRIGGER app_change_fold_baselines_admit_insert`.execute(
			scratch.db,
		);
		await sql`
			INSERT INTO app_change_fold_baselines (app_id, seq, project_id, snapshot, snapshot_digest)
			SELECT app_id, 1, project_id, snapshot, snapshot_digest
			FROM app_change_fold_baselines WHERE seq = 2
		`.execute(scratch.db);
		await sql`ALTER TABLE app_change_fold_baselines ENABLE TRIGGER app_change_fold_baselines_admit_insert`.execute(
			scratch.db,
		);
		const withHistory = await captureFrozenStorageSnapshot(scratch.db);
		expect(withHistory.app_change_fold_baselines?.rows).toHaveLength(2);
		expect(withHistory.__exact_payloads).toEqual(latestOnly.__exact_payloads);
	}, 120_000);
	...
	test("direct applied-state audit strictly replays the app-change suffix", async () => {
		...
		await expect(
			scratch.db
				.transaction()
				.execute((tx) => runFrozenCanonicalIdentityMigration(tx)),
		).rejects.toThrow(/app-change suffix replay does not equal current state/);
	}, 120_000);
});
