...
//
// ## Why the form-bridge has its own `buildBlueprint`
//
// `lib/case-store/form-bridge/__tests__/fixtures.ts` exports a
// `buildBlueprint` whose API is fundamentally different — it
// takes a nested field-tree shape and emits a blueprint with
// modules + forms + fields populated, not just case-types. That
// helper is the right tool for the form-bridge's domain (form
// completion, field traversal, repeat fan-out) and stays
// form-bridge-local.
