...
import { ReferenceAutocomplete, type ReferenceAutocompleteHandle } from '@/components/builder/ReferenceAutocomplete'
 ...
 */
function parseQuery(query: string): { namespace: ReferenceType | null; partial: string } {
  const slashIdx = query.indexOf('/')
  if (slashIdx < 0) return { namespace: null, partial: query }
  const ns = query.slice(0, slashIdx)
  if (!(REFERENCE_TYPES as readonly string[]).includes(ns)) return { namespace: null, partial: query }
 ...
 */
export function createRefSuggestion(provider: ReferenceProvider): Omit<SuggestionOptions, 'editor'> {
  return {
    char: '#',
    allowSpaces: false,
    ...
    items: ({ query }: { query: string }): SuggestionItem[] => {
      ...
      if (!namespace) {
        return REFERENCE_TYPES
          ...
          .map(ns => ({ kind: 'namespace' as const, type: ns, label: `#${ns}/` }))
      }
    ...
    command: ({ editor, range, props }: { editor: Editor; range: Range; props: SuggestionItem }) => {
      if ('kind' in props && props.kind === 'namespace') {
        ...
        editor.chain().focus().deleteRange(range).insertContent(`#${props.type}/`).run()
        return
      }
    ...
    render: () => {
      ...
      return {
        onStart: (props: SuggestionProps) => {
          ...
          container.style.cssText = 'position: absolute; z-index: var(--z-popover, 50);'
      ...
      function updatePopup(props: SuggestionProps) {
        ...
        const items: Reference[] = showNamespaces ? [] : (props.items as Reference[])
      ...
      function updatePosition(props: SuggestionProps) {
        ...
        const rect = (props.decorationNode as HTMLElement).getBoundingClientRect()
