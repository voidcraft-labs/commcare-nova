 ...
 */
export function ReferenceProviderWrapper({ getContext, subscribeMutation, children }: ReferenceProviderWrapperProps) {
  const getContextRef = useRef(getContext)
  getContextRef.current = getContext

  const provider = useMemo(() => new ReferenceProvider(() => getContextRef.current()), [])
  ...
  return <ReferenceCtx.Provider value={provider}>{children}</ReferenceCtx.Provider>
}
