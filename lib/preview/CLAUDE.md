...
- **A module target lands where the home screen lands it.** `moduleLanding.ts` reads the module-URL rule (`moduleScreenNavigation.ts`) from the outside: case-first and bare-case-list modules open on the case list, everything else on the form menu. The home screen's tiles use the same function.
