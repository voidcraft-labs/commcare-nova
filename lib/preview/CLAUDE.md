...
- **Default values apply one-time on init, and never to a writer the loaded case seeds.** In one-case mode every primary writer (the inventory's primary bucket, which `lib/domain/casePreload.ts::writerPreloadsFromLoadedCase` names) takes the case's value and skips its own `default_value` entirely, even when the case holds no such property yet, because on the device both are `xforms-ready` setvalues with the preload spliced after the default (`lib/commcare/xform/caseBlocks.ts`); a default on such a writer never shows anywhere. Defaults fill every other field. An authored several-case follow-up or close form never chooses one selected row to preload, even when its runtime selection contains one id. Its defaults therefore fill the initially blank shared answers.
