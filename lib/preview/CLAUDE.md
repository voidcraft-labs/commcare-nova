...
- **The carried case is the wire's match, valued from the submission.** `carriedCaseFor` asks `targetFrameChildren` for the target's `case_id` selection datum (Nova entries carry exactly one), evaluates a manual `case_id` datum when the link names its datums, and otherwise reads the source datum `matchFrameToSource` picked. `sourceSessionDatums` is the one mapping from source datum ids to values: `case_id` is the case the form loaded, a registration's `case_id_new_<module type>_0` is the case it created, and a subcase datum is the child case of its type the submission created. That last one is exact because the case store inserts `mutation.children` in order and answers `childCaseIds` in that same order (`submissionEnvelope.ts::applyOrdinaryAction`), and a non-repeat child bucket is one per case type (`caseWriteInventory.ts::childBucketKey`); when the form ALSO has a repeat bucket of that type the children cannot be told apart, so the datum stays unvalued rather than guessed. A manual XPath reads the same map, so `instance('commcaresession')/session/data/case_id_new_patient_1` names the created child.
...
- **Menu selection and Form launch are separate session facts.** A case-list/case-first parent selected through its explicit **Cases** entry stores a UUID-keyed menu case and returns to the parent menu; it does not seed a Form target. Same-case-type structural children may reuse that selection. Different-type modules follow the case-type `parent_type` projection to the first matching selector module in authored order, independently of structural menu parentage; Preview records an ordered selector/return chain, selects there, then constrains the child's Results query through the canonical `parent` relation before continuing. A child registration form receives the selected parent's properties as its ancestor preload and carries only the selected parent id in its submission mutation; the committed CaseType supplies the expected parent type plus `child`/`extension` relationship before the case store writes the primary. Never infer case ancestry from `parentModuleUuid`.
