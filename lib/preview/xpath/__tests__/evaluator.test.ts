...
function makeCtx(values: Record<string, string> = {}, caseData: Record<string, string> = {}): EvalContext {
  return {
    getValue: (path) => values[path],
    resolveHashtag: (ref) => {
      if (ref.startsWith('#form/')) return values[`/data/${ref.slice(6)}`] ?? ''
...
describe('XPath evaluator', () => {
  ...
  describe('functions', () => {
    ...
    it('concat()', () => {
      expect(evaluate('concat("hello", " ", "world")', makeCtx())).toBe('hello world')
    })
    ...
    it('coalesce()', () => {
      expect(evaluate('coalesce("", "", "fallback")', makeCtx())).toBe('fallback')
    ...
    it('normalize-space()', () => {
      expect(evaluate('normalize-space("  hello   world  ")', makeCtx())).toBe('hello world')
    })
  })
  ...
  describe('complex expressions', () => {
    ...
    it('if with path comparison', () => {
      const ctx = makeCtx({ '/data/gender': 'female', '/data/pregnant': 'yes' })
      expect(evaluate('if(/data/gender = "female" and /data/pregnant = "yes", "high", "normal")', ctx))
        .toBe('high')
    })

    it('nested function calls', () => {
      expect(evaluate('if(not(false()), concat("a", "b"), "c")', makeCtx())).toBe('ab')
    })
  })
