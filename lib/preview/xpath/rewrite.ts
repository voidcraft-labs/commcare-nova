 ...
 */
export function rewriteXPathRefs(expr: string, oldPath: string, newId: string): string {
  if (!expr) return expr
 ...
 */
export function rewriteHashtagRefs(expr: string, prefix: string, oldName: string, newName: string): string {
  if (!expr) return expr
...
function walkForPaths(
  ...
  if (T.Children.has(node.type) || T.Descendants.has(node.type)) {
    ...
    if (collected.length === targetSegments.length) {
      const matches = collected.every((seg, i) => seg.text === targetSegments[i])
      if (matches) {
...
function collectSegmentsWithPositions(
  ...
  while (child) {
    if (T.Children.has(child.type) || T.Descendants.has(child.type)) {
      ...
      segments.push({ text: source.slice(child.from, child.to), from: child.from, to: child.to })
