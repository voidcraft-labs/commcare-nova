...
export function evaluate(expr: string, context: EvalContext): XPathValue {
  ...
  let hasError = false
  tree.iterate({ enter(n) { if (n.type === T.Error) hasError = true } })
  if (hasError) return ''
...
function evalNode(node: SyntaxNode, source: string, ctx: EvalContext): XPathValue {
  ...
  if (type === T.AddExpr) {
    ...
    return toNumber(evalNode(left!, source, ctx)) + toNumber(evalNode(right!, source, ctx))
  }
  ...
  if (type === T.EqualsExpr) {
    const [left, right] = getBinaryOperands(node)
    return compareEqual(evalNode(left!, source, ctx), evalNode(right!, source, ctx))
  }
  ...
  if (type === T.LessThanExpr) {
    const [left, right] = getBinaryOperands(node)
    return compareRelational(evalNode(left!, source, ctx), evalNode(right!, source, ctx), '<')
  }
  ...
  if (type === T.AndExpr) {
    const [left, right] = getBinaryOperands(node)
    return toBoolean(evalNode(left!, source, ctx)) && toBoolean(evalNode(right!, source, ctx))
  }
  if (type === T.OrExpr) {
    const [left, right] = getBinaryOperands(node)
    return toBoolean(evalNode(left!, source, ctx)) || toBoolean(evalNode(right!, source, ctx))
...
function evalInvoke(node: SyntaxNode, source: string, ctx: EvalContext): XPathValue {
  let fnName = ''
  ...
  while (child) {
    if (child.type === T.FunctionName) {
      ...
      while (arg) {
        if (arg.type !== T.OpenParen && arg.type !== T.CloseParen && arg.type !== T.Comma) {
...
function collectSegments(node: SyntaxNode, source: string, segments: string[]): void {
  let child = node.firstChild
  while (child) {
...
function getBinaryOperands(node: SyntaxNode): [SyntaxNode | null, SyntaxNode | null] {
