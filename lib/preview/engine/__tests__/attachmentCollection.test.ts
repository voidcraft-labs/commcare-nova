 ...
 * submission: everything it returns is promoted, everything else staged
 * under the entry is discarded. So each case here is a rule about whether a
 * worker's file is kept or thrown away.
 *
...
describe("the mutation's attachment slots", () => {
	it("carries an EMPTY list when nothing was attached, given an entry key", () => {
		// Empty means "this submission named nothing" — the instruction to
		// discard every staged attachment. It must not be confused with
		// absent, which means "a client that knows nothing about attachments".
		const engine = new FormEngine(
			input([{ id: "photo", kind: "image", label: "Photo" }]),
		);
	...
	it("omits both slots when no entry key is supplied", () => {
		const engine = new FormEngine(
			input([{ id: "photo", kind: "image", label: "Photo" }]),
		);
		engine.setValue("/data/photo", "att-1.jpg");
