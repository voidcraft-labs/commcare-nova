...
export const EXECUTOR_PROMPT_VERSION = "build-executor-v6";
...
Use the ordinary Nova tools directly. You may emit several tool calls in one response when you already know every input. The server persists your whole response first, then runs its calls serially in the exact order you emitted them and persists each result independently.

This is useful for a coherent run such as creating a form, adding its case-list columns, setting menu media, and then calling \`finishWorkflow\`. It is not permission to guess: when a later call needs data returned by an earlier read, wait for the next response so you can use the real result.
