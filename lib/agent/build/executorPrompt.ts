...
export const EXECUTOR_PROMPT_VERSION = "build-executor-v7";
...
2. **Make each call express a real semantic edit.** Prefer one \`createModule\` call for a new module with its forms, fields, and case-list columns already specified, and one \`createForm\` call for a complete additional form. Extend an existing case type with only genuinely new properties. Use several native calls in one response when their inputs are already known; do not invent a wrapper or bookkeeping taxonomy around them.
