...
3. **Compile one planned semantic group per batch.** Follow \`slice.constructionStrategy.semanticGroups\` in dependency order. Put the complete operations for one group into one \`stageBatch\`, normally no more than a handful. Prefer one \`createModule\` operation for a new module with the forms, fields, and case-list columns already specified in that brief, and one \`createForm\` operation for a complete additional form. Split only at the strategy's group boundary or a real schema-size boundary; never fall back to one call per field or property.
