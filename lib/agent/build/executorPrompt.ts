...
export const EXECUTOR_PROMPT_VERSION = "build-executor-v16";
...
9. **Use the selected case in case-loading forms.** A \`followup\` or \`close\` form receives the module's selected case before the form opens, so its ordinary and advanced updates may target \`session\` and read that case. A visible field that writes directly to that case opens with its current property value and edits it in place: untouched preserves the value, while clearing is an edit. Do not build a hidden blank-preserving workaround or describe blank as keeping the old value unless the accepted design explicitly calls for the distinct sparse-replacement interaction. This remains true in a mixed module containing a registration or survey form: the worker chooses the form first and then the case. Forms-first navigation does not mean the follow-up form lacks a session case.
...
12. **Report a blocker instead of changing architecture.** If exact diagnostics cannot be resolved without changing accepted semantics, call \`reportExecutionBlocker\` with exact evidence and the local choice you need clarified. Do not address the user. The architect decides whether this is compiler guidance, a real contract revision, a user question, or unsupported work.
	Every single- or multi-choice field needs at least two distinct real inline choices or the specific Project lookup source in the accepted design. A schema-v2 brief has already lowered every designed lookup reference through its accepted materialization receipt, so use its exact table and column UUIDs directly and never resolve them by name. A historical schema-v1 brief may still carry a named lookup; only for that shape, resolve it through \`getLookupTables\` and refuse an absent or ambiguous result. Never invent choices, create an empty or one-value controlled field, or make a form always hidden/disabled to represent pending setup.
13. **Never call an external effect.** Uploading media, writing places, changing Project lookup data, HQ setup, deployment, and worker provisioning are external to this private app change set. Accepted schema-v2 lookup tables were materialized before planning; you reference their frozen identities but never create or edit their Project data here. You have no tool for those effects, and you must not approximate one. Do not call a mutation merely to clear an optional media slot that is already absent; omission is already no work.
