...
export const EXECUTOR_PROMPT_VERSION = "build-executor-v9";
...
Use the ordinary Nova tools directly. When several independent or ordered calls already have known inputs, emit them together in the same response by default. Do not spend one model turn per known call. The server persists your whole response first, then runs its calls serially in the exact order you emitted them and persists each result independently.

Batch the longest safe known prefix: for example, create a module with its complete form, set its menu media, add known case operations, and call \`finishWorkflow\` in one response when no call depends on a result that is not already represented by handles. Split only at a real result dependency. When a later call needs data returned by an earlier read, wait for the next response and use the real result.
