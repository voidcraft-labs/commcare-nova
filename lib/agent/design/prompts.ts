...
export const DESIGN_PROMPT_VERSIONS = {
	agent: "design-agent-v18",
	reviewer: "design-reviewer-v12",
	planner: "design-plan-v2",
} as const;
...
The first workflow must be executable without another workflow prerequisite. Every included workflow must be validly authorable, reachable, and useful before the design is accepted. Runtime or deployment setup may remain external only when the app structure can truthfully exist without it. Every controlled-choice field needs at least two distinct real values, inline or in a Project lookup table. Review a designed table's purpose, valid tag, ordered typed columns, source-grounded rows, unique nonblank saved values, nonblank labels, and every consumer. Prefer a shared table when the person requests maintainable Project data or one canonical list serves at least two consumers; reject needless Project data for a one-off list. Existing table and column references must be inspected stable UUIDs, never names or guessed identities. The contract exposes a server-validated, revision-bound attestation for the complete ordered saved-value/label projection: bounded display names plus row, validity, distinctness, duplicate, and blank-label counts, with a digest binding exact row identities, order, values, labels, and missing cells. Use those metrics to review each existing source; the server, not the author, verifies them. Any existing-table operation must carry the same expected revision and evidence of the person's direct request or explicit approval, and its impact statement must make the shared Project effect honest. Reject table deletion, removal of a consumed saved-value/label column, an uncited approval claim, unrelated citation, stale/name-based reference, unused table, empty/one-value/duplicate/placeholder choices, an always-hidden or disabled form, or a claim that a draft already changed Project state as a blocking correction.
