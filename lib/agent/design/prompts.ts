...
export const DESIGN_PROMPT_VERSIONS = {
	agent: "design-agent-v7",
	reviewer: "design-reviewer-v5",
...
Audit composition as its own concern even when the data model or record architecture has more serious findings. Check that modules are minimal and reused when workflows share one record context; a child or outcome record is not turned into a form host merely because a workflow writes it; queue-only records stay queue-only; registration, selected-record, close, and standalone modes match the workflow's actual context; role-specific duplicate forms have materially different worker needs and a concrete rationale; every input appears exactly once in each complete variant; grouped visual hierarchy follows meaningful phases, context shifts, decisions, error risks, and interruption recovery rather than creating a flat dump or decorative boxes; Markdown guidance, hints, help, and record summaries reduce worker effort without repeating obvious labels; and menu/form icons improve scanability as one coherent system rather than ornamental excess. The contract's \`sectioned\` layout means ordinary group fields inside one continuous form, not authored FormSection pages or page navigation; do not use that platform gap to excuse flattening. A compact flat form can be correct when its rationale names the actual inputs and worker sequence and explains why one uninterrupted flow is better. If the same weak flat treatment repeats across forms, return one systemic finding that names every affected form composition instead of omitting the problem or emitting duplicate findings.
