...
export class AutoFixer {
	 ...
	 */
	fix(files: Record<string, string>): {
		...
		if (suiteXml && result[appStringsPath] !== undefined) {
			const { content: fixedStrings, applied } = this.fixAppStrings(
				suiteXml,
				result[appStringsPath],
			);
			if (applied.length > 0) {
	...
	private fixItext(
		path: string,
		xml: string,
	): { xml: string; changed: boolean; applied: string[] } {
		const applied: string[] = [];
		...
		if (entries.length === 0) {
			...
			if (!xml.includes("<itext>")) {
				...
				const refs = [...xml.matchAll(/jr:itext\('([^']+)'\)/g)].map(
					(m) => m[1],
				);
				if (refs.length > 0) {
		...
		if (existingItextMatch) {
			const existingIds = [
				...existingItextMatch[1].matchAll(/<text\s+id="([^"]+)"/g),
			].map((m) => m[1]);
		...
		xml = xml.replace(
			/<h:body>[\s\S]*<\/h:body>/,
			`<h:body>${fixedBody}</h:body>`,
		);

		applied.push(
			`${path}: Converted ${entries.length} inline labels to itext references`,
		);
		return { xml, changed: true, applied };
	}
	...
	private collectLabelEntries(body: string): Array<{
		...
		while (match !== null) {
			...
			const itemRegex =
				/<item>\s*<label>([^<]+)<\/label>\s*<value>([^<]+)<\/value>\s*<\/item>/g;
		...
		const groupRegex =
			/<group[^>]*ref="\/data\/([^"]+)"[^>]*>[\s\S]*?<label>([^<]+)<\/label>/g;
		...
		for (const block of updateBlocks) {
			...
			for (const tagMatch of childTags) {
				...
				if (RESERVED_CASE_PROPERTIES.has(prop.toLowerCase())) {
					const newName =
						RESERVED_RENAME_MAP[prop.toLowerCase()] || `${prop}_value`;
					// Replace in instance data
					result = result.replace(
						new RegExp(`(<update>[\\s\\S]*?)<${prop}\\s*/?>`, "g"),
						`$1<${newName}/>`,
					);
					result = result.replace(
						new RegExp(`</${prop}>`, "g"),
					...
					applied.push(
						`${path}: Renamed reserved case property "${prop}" to "${newName}"`,
					);
					changed = true;
				}
			}
	...
	private fixAppStrings(
		...
		const localeIds = [...suiteXml.matchAll(/<locale\s+id="([^"]+)"/g)].map(
			(m) => m[1],
		);
		...
		applied.push(
			`Added ${missing.length} missing key(s) to app_strings.txt: ${missing.join(", ")}`,
		);
	...
	private inferCaseType(xml: string): string | null {
		...
		const calcMatch = xml.match(
			/nodeset="\/data\/case\/create\/case_type"\s+calculate="'([^']+)'"/,
		);
