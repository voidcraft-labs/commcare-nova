/**
 * Actual workspace admission and tool composition with a controlled host receipt.
 * These tests prove whether the host is called and which admitted candidate it
 * receives. Database atomicity, locks, and durable persistence belong to the
...
function badRelevantMutation(doc: BlueprintDoc): Mutation[] {
	return [
		{
			kind: "updateField",
			uuid: villageAddress(doc).fieldUuid,
			targetKind: "text",
...
describe("guardedMutate", () => {
	it("submits a passing candidate once with the post-batch doc and stage tag", async () => {
		const doc = minDoc();
		...
		expect(prepared?.nextDoc).toBe(outcome.newDoc);
		expectAdmittedDoc(outcome.newDoc);
		expect(outcome.newDoc.fields[villageAddress(doc).fieldUuid]).toMatchObject({
			label: proseText("Home village"),
		});
	});

	it("never calls the host on a gate rejection and returns the findings as prose", async () => {
		const doc = minDoc();
		const h = makeToolWorkspaceHarness(doc);
	...
	it("rejects a completeness introduction — an empty form never lands", async () => {
		...
		if (rejected.ok) throw new Error("expected empty-form refusal");
		expect(rejected.error).toContain("has no fields");
...
describe("tool-level gating (editField through the shared layer)", () => {
	...
	it("a passing ID-and-property edit persists as one canonical update stage", async () => {
		...
		const stages: AdmittedMutationStages =
			h.recordMutationStages.mock.calls[0]?.[1];
		...
		expect(h.currentDoc().fields[villageAddress(doc).fieldUuid]).toMatchObject({
			id: "village_name",
			label: proseText("Home village"),
		});
	});
