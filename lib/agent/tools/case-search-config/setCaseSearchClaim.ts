 ...
 * owns the claim cluster (claim condition / already-owned guard /
 * blacklisted owner ids). Display labels stay untouched and round-trip
 * byte-identically through the patch — the tool reads the existing
 * config, strips the claim keys, and rebuilds with the input. The
 * display tool (`setCaseSearchDisplay`) is the parallel for the
 * other cluster.
 *
 * Wholesale-with-`null`-clears semantic — every optional cluster slot
 * is required-and-nullable on the SA boundary; `null` clears,
 * non-null sets. Mirrors `setCaseListFilter`.
 *
 ...
 * clears path); `dontClaimAlreadyOwned` echoes the boolean the SA
 * supplied so a follow-up read isn't required to confirm it landed.
 */
...
export const setCaseSearchClaimTool = {
	description:
		"Set the claim cluster of a module's case-search config: the claim condition predicate, the already-owned guard, and the blacklisted owner ids expression. Pass `null` for `claimCondition` or `blacklistedOwnerIds` to clear that slot. The display cluster (search-screen labels) is not touched — use setCaseSearchDisplay for that.",
	...
	async execute(
		...
		try {
			...
			const {
				...
				dontClaimAlreadyOwned: _existingFlag,
			...
			return {
				...
				result: {
					message:
						claimCondition === null
							? `Set case-search claim on module "${mod.name}" (index ${moduleIndex}): no claim condition, dontClaimAlreadyOwned=${dontClaimAlreadyOwned}.`
							: `Set case-search claim on module "${mod.name}" (index ${moduleIndex}): claim condition kind=${claimCondition.kind}, dontClaimAlreadyOwned=${dontClaimAlreadyOwned}.`,
					claimConditionKind:
