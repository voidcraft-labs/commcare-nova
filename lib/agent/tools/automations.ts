...
export const updateAutomationTool = {
	description:
		"Replace one existing automation with its complete desired canonical state. Use Nova standard property names and one plain-text or rich-text email body. Recipient filters require only user-account recipient kinds; every triggering case must contain a structurally referenced filter property. After trimming, case-property event-time values must begin with H:MM or HH:MM and be entirely parseable as a time; AM/PM and seconds are accepted, while blank, nonmatching, or unparseable values fall back to 12:00 PM. Host-scoped reads are refused when an advanced case operation can add a second extension relationship to the automation case type. Every host-scoped read also requires exactly one live extension at runtime; retained extra extension indices make the current-match count unavailable when a criterion reads the host, and HQ's host choice is undefined. Message case-property parts cannot use owner, host, or last_modified_by because CommCare HQ's formatter context shadows those names. The automation kind and UUID are create-once; nested UUIDs preserve identity across edits.",
