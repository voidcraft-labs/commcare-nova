...
export const askQuestionsTool = {
	description:
		"Ask the user clarifying questions (max 5) when something is genuinely ambiguous — execution pauses for the answers. Give a question 2-4 concrete answer options whenever real candidates or sensible defaults exist, your recommended option first with its label ending in ' (Recommended)' when you have one. The user can always type a free-text answer instead of picking an option, so never add an option that means 'let me type it'; an empty options list is only for questions with no concrete candidates. Decide everything you can decide well yourself.",
