...
- A field can SAVE into worker information, on any form including a survey: \`caseWrite: { caseType: "commcare-user", property: "<declared name>" }\`, read back as \`#user/<name>\`. Records something about the worker rather than the case. Never inside a repeat, and never a built-in worker field (Nova keeps those in step with the profile).
