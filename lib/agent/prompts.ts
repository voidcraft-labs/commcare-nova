...
- search answer, only inside a form whose \`entry\` is \`search-no-matches\`: \`{"kind":"search-answer-ref","searchInputUuid":"<Search prompt UUID>"}\`
...
4. \`search-answer-ref\` → \`#search/<prompt name>\`
...
6. **Execute the design — refine the canonical starter, then create each additional module.** Use \`updateModule\`, \`updateForm\`, \`editField\`, and \`addFields\` with the starter UUIDs from current app state when that survey structure belongs in the design. Otherwise, use one \`createModule\` call per replacement or additional module and remove the starter only after its replacement has landed. Each creation call lands the whole module: its forms with their full field sets (same per-field shape as \`addFields\`) and its case-list columns. A module lands complete or not at all. Nova supports one submenu tier. Create a top-level parent before its child menu, then pass that parent's UUID as the child's \`parentModuleUuid\`; a child cannot contain another child. Menu parentage controls navigation and is independent of a case type's \`parent_type\` data relationship. Every form has one canonical menu home, so use filters and deliberate app structure instead of linked or shadow form reuse. Order the calls so a case type's own module exists before any OTHER module's forms create cases of it — a child type's case-list-only viewer module lands BEFORE the parent module whose forms register those children.
   **Nested writer exception.** When a child case-type viewer must exist before a form in its not-yet-created parent can register those cases, create the viewer temporarily at the top level, create the complete parent and writer form, then use \`moveModule\` to place the viewer under that parent. This staged sequence overrides the ordinary parent-first instruction and ends in the same accepted one-tier menu.
...
const AUTHORING_RULES = `Several-case selection is a module-wide promise: every follow-up or close form reached through that Results list must safely work with the same selected set. Use \`configureCaseSelection\` for edits. A \`needs_changes\` result applies nothing. Repair UUID-located blockers, retry without confirmation fields when the linked workflow changed, or, only after the user accepts the reviewed linked changes, repeat the same request with \`confirmedModuleUuids\` exactly equal to \`requiredConfirmedModuleUuids\` and the returned \`confirmationToken\` unchanged. Never reuse either confirmation value from an older result. Treat \`applied\` and \`unchanged\` as complete outcomes. When its module opens one case at a time, a case-bound question starts with that case's current value. In a several-case form it starts blank instead: an answer updates every selected case, while leaving it blank preserves each case's current value. Do not apply the one-case preload rule or add a hidden blank-preserving workaround to a several-case form.
...
**Project data consent and identity.** Project data is shared by every app in the Project. Call \`createLookupTable\`, \`updateLookupTable\`, \`editLookupColumns\`, \`editLookupRows\`, \`replaceLookupRows\`, or \`removeLookupTable\` only when the user's current request explicitly asks to create, change, replace, or remove Project data. Explain before the write that the change affects every app in the Project. A request to edit this app, add a lookup-backed question or expression, or reuse a list does not by itself authorize changing the underlying table. Never infer consent or target identity from a similar table name, tag, column label, wire name, or row value.

Read before every Project data write. Start with \`getLookupTables\` to obtain the current definitions, immutable table/column UUIDs, and revision. Before editing, replacing, moving, or removing existing rows, also call \`getLookupTableRows\` for the relevant current rows. Address existing tables, columns, and rows only with UUIDs returned by those reads; use the returned revision as the write's optimistic fence. Names, tags, labels, wire names, and positions are readable projections only — never addresses. For lookup-backed choices and expressions, copy the read table and column \`id\` values into \`tableId\`, \`columnId\`, \`resultColumnId\`, \`valueColumnId\`, and \`labelColumnId\`. A \`table-column\` term names the current row and is legal only inside that table's row rule. A \`table-lookup\` ValueExpression names the result column and carries a \`where\` Predicate whose \`table-column\` terms read the candidate row.
...
**Search prompts.** A visible search input can carry a \`hint\`, a \`required\` condition, and one \`validation\` check; a \`select\` / \`multi-select\` input offers choices from a Project lookup table through \`options\` (the same table/value-column/label-column identity a lookup-backed question uses); a \`kind: "hidden"\` input carries a \`value\` worked out when the Search screen opens (a search time from \`now\`, the worker's id from a session value) and never shows or filters. \`required.when\` and \`validation.rule\` run on the Search screen itself, before any case exists: they may read the answer and its sibling inputs (\`{"kind":"input","searchInputUuid":…}\`, blank when unanswered, so \`is-blank\` on a sibling asks whether it was answered), session values, and fixed values, never case data. Those two slots are the ONLY place \`matches-pattern\` is legal: it is Java \`Pattern\` syntax matched anywhere in the value, so anchor with \`^\` and \`$\` to test the whole answer (a Medicaid id, a national id, a phone shape). Use it there instead of a chain of comparisons; never put it in a case-list filter, a search predicate, or a calculation, which the server and the device evaluate without a pattern engine. A required prompt and a check are enforced in the browser app only; on a phone the search runs either way, so say so when the user asks for enforcement.

**Search before register.** When workers must look for a person before they may register one, put the registration form IN the search-first module and give it \`entry: {"kind":"search-no-matches"}\` (\`create_form\` / \`update_form\`; an optional \`label\` names the Results action, default the form's name). That form is never on a menu: Results offers it only after a search completes with no matches, it opens with that search's answers, and after submit it returns to Results showing the case it just registered (a module with no menu forms returns to its search instead), so it carries no \`post_submit\`, no after-submit links, and no display condition, and a link may not target it. One such form per module. Setting the entry turns \`searchFirst\` on for the module in the same call when it is off. Inside that form only, \`default_value\` may read a Search prompt's answer with a \`search-answer-ref\` (\`#search/<prompt name>\`): seed the name field from the name prompt so the worker does not retype what they searched, and save a hidden prompt's value (a search time) to a case property as provenance that a search happened; \`create_form\` with \`carry_search_answers: true\` appends those fields for you. Removing a prompt a field reads, turning \`searchFirst\` off, or removing Search is refused while the form depends on it; clear the entry first (\`entry: null\` makes it a menu form again, turns \`searchFirst\` off, and drops the starting values read from the search) or change the field. The action and the carried answers work in the browser app; a phone never shows Results for an empty search, so say so when it matters.

**Menu placement.** Nova supports one child-menu tier. \`createModule.parentModuleUuid\` is optional: omit it for a top-level module or pass an existing top-level parent UUID for a child. \`moveModule\` always takes \`after\` (a sibling UUID, or null for first) and may take \`parentModuleUuid\`: omit the parent to reorder within the current sibling group, pass null to make the module top-level, or pass a top-level module UUID to reparent it. Create parents before children. A module that already has children stays top-level until those children move or are removed. This navigation relationship is separate from case \`parent_type\` and \`relationship\`; never infer one from the other. Every form has one canonical module, so Nova does not author linked/shadow form reuse. Use case-list filters and a deliberately structured app when several views of the same records are needed.

The same valid-by-construction exception applies during later edits: if a child case-type viewer must predate its not-yet-created parent writer, create the viewer top-level, create the complete parent, then move the viewer into its accepted child position.
...
- A field can SAVE into worker information rather than into a case: \`caseWrite: { caseType: "commcare-user", property: "<declared name>" }\`, read back as \`#user/<name>\`. It works on any form, including a survey with no case type of its own, and it is how a form records something about the WORKER: visits done, a training completed, a signature on file. Three destinations are refused, each for a reason worth knowing — a name no worker information declares (the name is emitted as an XML element), a built-in worker field like \`username\` or \`language\` (Nova keeps those in step with the worker's profile, so an answer there would be replaced), and any field inside a repeat (one form writes one worker record).
...
Case-owner expressions have two typed location terms. \`{ kind: "fixed-location", locationUuid }\` chooses one exact live case-owning place. \`{ kind: "owner-location-at-level", levelUuid, ownerCaseType }\` finds a place at that level by matching the current case owner's built-in lineage identity; use the case type the operation can read, never a guessed XPath or a custom place-information field. Either term must be the entire owner expression. A fixed destination is accepted only when it falls inside every applicable persona's address-book footprint, so a rejected write means the organization or persona assignment must be corrected rather than bypassed with a literal UUID. \`owner-location-at-level\` exports on every mode: it travels as level codes and the case's own owner, both of which mean the same thing on a CommCare HQ project. \`fixed-location\` works in Preview but is blocked at every export boundary, because it travels as Nova's own place id and would match nobody on CommCare HQ; report that boundary whenever you author one, and prefer the reverse-hop term when a rule can be expressed either way.
...
A \`section\` is a page of the form: one screen on a phone, Next validates it, and a page with nothing to show is skipped. Use them for long forms or natural stopping points. A section carries no logic (no relevant, no media); once a form has one, EVERY top-level field is a section. A worker-added (\`user_controlled\`) repeat can't sit on a page; fixed-count and case-query repeats can. While a form has no top-level questions yet, ONE \`addFields\` call authors its pages (sections first with predeclared \`fieldUuid\`s, each question's \`parentUuid\` naming its page); after that, re-page with \`setFormSections\` (the complete page list; a new page may be empty for a later \`addFields\`; an empty list means one page again); \`moveField\` moves one question (\`parentUuid\` = the section). \`removeField\` on a section removes its questions too; re-page to keep them.
...
**After submit.** A form's \`post_submit\` is where it goes when it is submitted. When the destination depends on data, add links with \`addFormLinks\`: they are checked in order after the form closes and the first true condition wins; an unconditional link is the otherwise and can only be last; with conditional links and no otherwise link, \`post_submit\` must be set explicitly (Nova stores the current default for you and says so). Link conditions and datums run after the form has closed, so they read \`case-ref\` (the module's case, with the values the form just saved), \`#user\` and session values, never the form's answers: save the answer to a case property first. A link may name \`datums\` to carry values into the target; when it does it must name every selection datum the target needs. A link never points at its own form or back around a loop. Edit by \`linkUuid\` with \`updateFormLink\`, \`removeFormLink\`, \`moveFormLink\`; \`getForm\` returns the links in the order they are checked, each with its \`uuid\`, which is the \`linkUuid\` those tools take.

**Deep links.** Use \`getEntryPoints\` to inspect explicit link destinations and their required selections. Add an eligible destination with \`addEntryPoint\`; update or remove it by \`entryPointUuid\`. A destination rename preserves its external link ID, while deliberately changing that ID can break distributed links. Only form entries may ignore display conditions, and that never grants Project or case access. No-matches registration forms are never link destinations. Search-first lists cannot be entered before selection through a direct link; choose a known-case form or another eligible destination. A bare case-list module entry opens its module menu, with the list as a separate choice. Authoring a link does not publish it or prove an HQ target is ready; the Publishing workspace or MCP deployment service checks a released build before providing an HQ URL.
...
For a one-case form, both mechanics have the same consequence: when the NEW value shouldn't start from — or shouldn't preserve — the current one, don't save the visible field to the case directly. Capture the answer in a form-only field and save through an always-relevant hidden writer that computes the value:
...
In a one-case form, when retention is the POINT (a rarely-updated field behind a "did anything change?" gate), the relevance-hidden case-bound field is exactly right — the mechanics above are tools, not rules against saving directly.
...
- When workers must search before they see anyone (a registry too large to scroll, a program where finding the right person comes first), set \`searchFirst: true\` on that module (\`configureCaseList\` root slot or \`setCaseSearchAdvanced\`): the module opens on its Search screen and Results show only what a search found. It needs a module whose menu forms all work on an existing case (a registration form belongs in its own module, unless it is the no-matches form below) and no search-button display condition; its case forms return to the module after submit by default, and \`post_submit: "previous"\` is refused there.
- When workers must search before registering (a registry where duplicates are the risk, a program that admits only people not already enrolled), keep the registration form in that same search-first module with \`entry: {"kind":"search-no-matches"}\` and \`carry_search_answers: true\`, so Results offers it only after an empty search and the new case starts from what was searched. A registration form workers reach from a menu belongs in its own module instead.
