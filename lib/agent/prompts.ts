...
An attachment question (\`image\`, \`audio\`, \`video\`, \`signature\`, \`file\`) can save to a case property too, and its \`caseWrite\` carries one extra member: \`mode: "url"\`. What reaches the case is a LINK to the captured file, never the file itself, so name the property for what it holds (\`photo_url\`, \`consent_form_url\`). The address is built from the CommCare project space the app is published to, so a download made before the app is published leaves the property empty; that is expected and is not something to work around. An attachment cannot write \`case_name\` or \`external_id\` — those are the case's own identity, and a link is not a name. CommCare never displays a case attachment inside the app, so a link is the only thing that makes the file reachable afterward.
