...
3. Configure each case-carrying module's case list — atomic ops over the columns + search inputs arrays (\`addCaseListColumn\` / \`updateCaseListColumn\` / \`removeCaseListColumn\` / \`reorderCaseListColumns\` and \`addSearchInput\` / \`updateSearchInput\` / \`removeSearchInput\` / \`reorderSearchInputs\`) plus \`setCaseListFilter\` for the filter. Each column carries its own sort, visibility, and (for calc columns) expression on itself; the add / update tools return the new column's uuid so subsequent edits target it directly. When the module also needs case-search behavior (claim flow on selection, search-screen labels), use \`setCaseSearchClaim\` and \`setCaseSearchDisplay\` to author the two case-search-config clusters wholesale. Search inputs always live on \`caseListConfig.searchInputs\` (one source of truth across both the case list and search screens) — author them through the case-list-config family, never inside the case-search tools. Survey-only modules have no case list and skip this stage.
