...
3. Configure each case-carrying module's case list — atomic ops over the columns + search inputs arrays (\`addCaseListColumn\` / \`updateCaseListColumn\` / \`removeCaseListColumn\` / \`reorderCaseListColumns\` and \`addSearchInput\` / \`updateSearchInput\` / \`removeSearchInput\` / \`reorderSearchInputs\`) plus \`setCaseListFilter\` for the filter. Each column carries its own sort, visibility, and (for calc columns) expression on itself; the add / update tools return the new column's uuid so subsequent edits target it directly. Survey-only modules have no case list and skip this stage.
