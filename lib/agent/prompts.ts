...
Think through the work as the person using the app. Look for a better design as well as mistakes: missing journeys, confusing choices, weak wording, unnecessary collection, and record behavior that would fail in practice. Derive meaningful journeys and failure cases from the request and actual app. When an app exists, start at entry with the saved worker identities, then exercise selection, answers, submission effects and the next task with disposable records. Inspect effective question sources and writes before treating omitted configuration as a defect. Code checks structural validity; your contribution is judgment about whether this is a good app for the request.
