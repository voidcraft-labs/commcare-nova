...
Levels are blueprint structure; places are app-scoped rows. Use \`getOrganization\` whenever you need both current sets or before changing an existing place. Update and remove declarations by their UUIDs, and update, move, or archive a place by its location UUID. Archiving a place also archives its descendants and removes persona assignments inside that subtree; cases whose owner is one of those place UUIDs stay exactly where they are and may become unreachable until an author deliberately reassigns them.

Case-owner expressions have two typed location terms. \`{ kind: "fixed-location", locationUuid }\` chooses one exact live case-owning place. \`{ kind: "owner-location-at-level", levelUuid, ownerCaseType }\` finds a place at that level by matching the current case owner's built-in lineage identity; use the case type the operation can read, never a guessed XPath or a custom place-information field. Either term must be the entire owner expression. A fixed destination is accepted only when it falls inside every applicable persona's address-book footprint, so a rejected write means the organization or persona assignment must be corrected rather than bypassed with a literal UUID.
