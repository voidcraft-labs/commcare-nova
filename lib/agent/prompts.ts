...
Levels are blueprint structure; places are app-scoped rows. Use \`getOrganization\` whenever you need both current sets or before changing an existing place. Its cursor covers one bounded stream across levels, place-information fields, and matching places; accumulate every collection until \`page.complete\`. Update and remove declarations by their UUIDs, and update, move, or archive a place by its location UUID. If an existing reverse-hop owner rule requires descendants below a newly created source place, create the source and its complete request-local \`descendants\` tree atomically in one \`createLocation\` call. Archiving a place also archives its descendants and removes persona assignments inside that subtree; cases whose owner is one of those place UUIDs stay exactly where they are and may become unreachable until an author deliberately reassigns them.
