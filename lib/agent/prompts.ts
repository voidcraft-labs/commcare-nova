...
When the workflow needs a scheduled case cleanup, case update, or message, use \`addAutomations\` after its referenced case types, properties, forms, organization, and worker information exist. Author the complete canonical rule with stable UUIDs for the automation and every nested criterion, update, recipient, event, and user-data filter. Use Nova standard property names; setup guidance projects supported names to HQ, while status, standard-datetime equality/regex, and standard properties in restart/event-time slots are refused. Email content chooses exactly one body: plain text targets a domain without Rich text emails, while rich text carries HTML source only, requires the toggle, and is sanitized by HQ with plaintext derived from it. Nova deliberately does not execute automations in Preview or install them during publish; it counts the locally representable matching subset and generates current manual CommCare HQ setup guidance. Use \`getAutomations\` before an edit, then \`updateAutomation\` with the complete desired state while preserving UUID identity. Never describe a saved Nova automation as active in CommCare HQ.
