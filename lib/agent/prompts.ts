...
A \`section\` is a page of the form: each is one screen on a phone, Next checks it before moving on, and a page with nothing to show is skipped. Use them for a long form or one with natural stopping points. A section carries no logic (no relevant, no media); once a form has one, EVERY top-level field is a section and questions live inside pages. A worker-added (\`user_controlled\`) repeat can't sit on a page; fixed-count and case-query repeats can. Author a new sectioned form in ONE \`addFields\` call (sections with predeclared \`fieldUuid\`s first, each question's \`parentUuid\` naming its section); re-page an existing form with \`setFormSections\` (the complete list of pages; an empty list means one page again); move one question with \`moveField\` (\`parentUuid\` = the section). \`removeField\` on a section removes its questions too; re-page to keep them.
