...
3. **Declare custom worker information — \`addUserProperties\`.** When the design uses custom worker information, declare every property now, immediately after naming the app and before any condition, calculation, module, or form can reference it. Predicate / ValueExpression references, roles, and personas all use its returned uuid. Roles and personas may follow after the reference-bearing app structure; the properties themselves may not.
...
Display conditions, Project-data row rules, case-list filters and calculated columns, search-input predicates and defaults, case-change expressions, and \`excludedOwnerIds\` take a structured AST — a tool slot described as a "Predicate" or "ValueExpression" takes exactly these shapes:
...
\`setFieldOptionsSource\` replaces a single- or multiple-choice field's complete source. An \`inline\` source owns at least two UUID-identified options. A \`lookup\` source owns its table/value-column/label-column UUIDs and optional \`filter\`; the filter may compare columns from that same table with fixed/session/current-user values and eligible earlier form answers by field UUID. It cannot read a case, Search input, later answer, or answer from a child/sibling repeat. Switching kinds discards the previous source; there is no inactive fallback or null clear.

Module and form \`displayCondition\` slots follow the normal edit rule: omission keeps the current condition, a Predicate replaces it, and \`null\` removes it. Structured-AST field leaves use field UUIDs, case-operation references use operation UUIDs, Search answers use search-input UUIDs, and custom worker information uses its property UUID. Textual XPath remains authored text and uses its documented \`#form/\` and \`#user/\` projections.
...
\`{ kind: "session-user-property", userPropertyUuid }\`. Use
\`{ kind: "session-user", field }\` only for
...
**Place a field in its group as you add it.** Within one creation call, an item may use \`parentId\` only as a construction-local handle naming a container item created earlier in that same call. To add under an EXISTING group/repeat, pass its immutable \`parentUuid\` at the batch level. A field with neither lands at the form root. Give a field its parent up front. An EXISTING field that's in the wrong place moves with \`moveField\` — the move and all anchors use UUIDs, preserving its identity and every reference, so never remove and re-add a field to reposition it.
...
**Case operations.** Ordinary case-bound fields remain the simplest way for a form to save its answers onto its primary case. Use \`addCaseOperations\` when one submission has an additional ordered effect: creating another case, updating or closing a known case, linking cases, renaming/retyping, assigning an owner, or running an effect once per repeat entry. Read the current sequence with \`getCaseOperations\`; update, remove, and move by \`operationUuid\`. Every one of these tools addresses its form by \`moduleUuid\` + \`formUuid\` — take all UUIDs from a read result, and never derive them from order or names. The operation's \`id\` remains its authored wire name, not its address. Structured expressions use \`{"kind":"field","uuid":"<field uuid>"}\` and \`{"kind":"id-of","opUuid":"<operation uuid>"}\`. Each \`addCaseOperations\` item may predeclare \`operationUuid\`, so a later item in the same atomic call can target an earlier create by that UUID; keep producer before consumer. Every action has a closed shape: create uses a new target and requires \`name\`, update targets an existing case and may rename/retype, and close targets an existing case and may carry only final writes.
