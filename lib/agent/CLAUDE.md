...
- **Two library tools** (`ReadToolResult`-shaped — no doc mutation): `listMediaAssets` (load-bearing — how the SA discovers the asset ids the attach tools need; reuses `listReadyAssetsForOwner` + `toWireMediaAsset`) and `removeMediaAsset` (deletes the GCS object + Firestore row, but refuses with an Elm-shape error naming the carrier(s) when `walkAssetRefs` finds the asset still referenced in the doc — the reference guard).
