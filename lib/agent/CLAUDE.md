...
The editor and shared MCP tools accept authored wording and expressions.
...
The build executor still uses its durable handle interface. The isolated pilot
under `authoring/experimental` remains research evidence, not a second production
path. Read `authoring/CLAUDE.md` and the delivery plan before extending the boundary.
...
- `prompts.ts` composes static editor and MCP build prompts around purpose, collaboration, and app-quality judgment. Detailed syntax is available through `getAuthoringGuide`, not injected into every turn. `buildAppStateMessage` appends current state separately; MCP edit rendering includes that state in its one-shot prompt and keeps `NOVA-PROMPT-END` as its final delivery marker.
- `wireSchemas.ts` still projects the build executor's canonical handle grammar and the question tool. Shared editor/MCP tools use `authoring/toolSchema.ts`: authored content families, scoped addresses, optional creation identities, and reusable JSON Schema definitions. The full canonical Zod schema runs after binding; no stored AST input alternative is accepted on the new interface.
- `expressionReference.ts` renders the canonical query grammar for the build executor while it still uses the handle interface. The editor and MCP use authored expressions and `authoring/reference.ts`.
