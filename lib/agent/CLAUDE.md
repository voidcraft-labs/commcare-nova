...
- **Definitions are split from bindings so the page can hold them.** `designAgentToolDefinitions(designLoopToolDefinitions())` and `solutionsArchitectToolDefinitions()` return the wire records without `execute`; the loops spread them and bind. `__tests__/toolDefinitions.test.ts` pins both toolset digests and tool orders in `__tests__/fixtures/promptDigests.json`; `__tests__/segments.test.ts` proves every segmented prompt equals its builder, and records each VERSIONED prompt's digest beside its version so a prompt edit that leaves the version alone (which would keep live sessions on text their recorded version no longer names) fails with the version to bump.
