...
- `toolSchemaGenerator.ts` — generates the three field-mutation tool schemas (`addFieldsItemSchema`, `addFieldSchema`, `editFieldUpdatesSchema`) from `fieldRegistry` + `fieldKinds`. Per-kind `saDocs` flows through into the `kind` enum description. These run as tool-use input (not grammar-constrained `Output.object`), so the structured-output optional-field ceiling never applied — the batch item requires only `id`/`kind`/`label` and carries the rest (including `parentId`/`required`) as plain optionals the SA omits when unset. `label` stays required-with-sentinel (`""` = no label) as a conscious-choice guard for the empty-label kinds (hidden / transparent group / titleless repeat), not a budget device. Feature configs still group under nested objects (`validate: { expr, msg? }`, `repeat: { mode, count?, ids_query? }`) so each reads as one slot.
...
- `attachments.ts` — `prepareAttachments` rewrites a turn's file attachments into model-ready content BEFORE the SA runs (the chat route calls it after `ctx` is built, before `createSolutionsArchitect`). Large text/office docs are condensed to a faithful requirements extract by Haiku; large PDFs go to Haiku as a native document block via `generationContext.ts::extractFromContent`; images pass through to Opus vision; small files inline raw. The why is cost, not cleanup: an un-condensed doc rides the SA's tool loop (up to 80 steps) and is re-read at full input rate every step, so condensing once is a large recurring saving. Every failure path inlines the raw/converted text — an attachment is never dropped — and a recovered extraction passes `emitErrors: false` so it doesn't surface a user-facing error. The extract-vs-inline threshold is a code constant, not user-configurable.
