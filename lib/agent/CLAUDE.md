...
- `prompts.ts` — composes the SA's ONE edit system prompt (`buildSolutionsArchitectPrompt`, zero-arg: core + edit preamble + the shared `AUTHORING_RULES` + tail) plus the MCP-only build composition (`buildMcpAgentBuildPrompt` — the plugin's client-side build agent still drives direct canonical tools including `create_app`, an immediate unreviewed surface the chat design pipeline never touches). Every composition is STATIC — byte-identical across turns and apps, the property provider prompt caching keys on. The volatile blueprint summary ships separately as `buildAppStateMessage`, the per-turn message the chat route appends after the full history. Edit-mode MCP rendering inlines the same complete block in its one-shot prompt: there is no server-side character cap or fallback summary, and `NOVA-PROMPT-END` remains the final delivery marker so an MCP host's own truncation is detectable. `isEditableDoc` remains the shared persisted-state predicate for edit framing and the state block; a real persisted app always satisfies it because empty app birth does not exist. CommCare XForm terms (e.g. `jr:`, `#form/`, XPath function names) still appear where the SA genuinely needs them. The XPath quick reference is a closed authoring menu backed by `lib/commcare/xpath/functionCapabilities.ts`; never invite the model to reach for an unlisted XPath-standard function that JavaRosa does not implement.
- `wireSchemas.ts` — the chat-surface wire projection (`wireToolSchema`): every SA tool's JSON schema ships with the Predicate/ValueExpression family flattened into self-contained merged root objects — the complete `kind` vocabulary and every identity slot with its exact pattern, nested expression slots collapsed to `{}` — and a generic pass cuts any remaining definitions-graph cycle (`createLocation`'s descendant tree), with unreferenced definitions pruned. The projection admits direct Term leaves in value slots; `modelAstInput.ts` contextually lifts those leaves before the original full Zod schema parses them, while stored Blueprint and raw MCP inputs remain canonical. WHY: OpenAI re-renders tool schemas server-side into a TypeScript-like namespace and bills the rendering, and how it renders a RECURSIVE definitions graph is not observable offline — live step usage showed billed tool rendering far past the emission's own token mass while the graph was cyclic (the ~540k-token SA request regression), so the wire is cycle-free BY LAW: no definition body may reference a definition that reaches back to it, and the AST bodies carry no refs at all. `__tests__/wireSchemas.test.ts` pins the law plus a hard whole-registry content budget over every registered tool. The complete AST grammar lives ONCE in the system prompt instead of on each expression-bearing tool; the wire teaches vocabulary and identity constraints at the slot, the prompt owns nesting.
...
recovery of that attempt reopens it. Every generation starts with three
separately durable messages — the exact accepted brief, a full handle-projected
Blueprint checkpoint, and short slice focus plus compact inventory — and
appends all three again after each compaction boundary. The executor therefore
gets exact intent and current state without inheriting a previous slice's tool
transcript or spending turns rediscovering the app.
...
Every mode gets the same tools: `askQuestions`, `generateSchema` (the data-model tool — after app naming and any referenced custom worker properties in a build, and how a NEW case type enters an existing app), `searchBlueprint`, `getModule`, `getForm`, `getField`, `addFields`, `editField`, `moveField`, `removeField`, `setFormSections`, `updateApp`, `configureConnect`, `updateModule`, `updateForm`, `createForm`, `removeForm`, `createModule`, `removeModule`, `moveModule`, `getCaseOperations`, `addCaseOperations`, `updateCaseOperation`, `moveCaseOperation`, `removeCaseOperation`, `addFormLinks`, `updateFormLink`, `removeFormLink`, `moveFormLink`, `renameCaseProperties`, `getAutomations`, `addAutomations`, `updateAutomation`, `removeAutomation`, `configureCaseList`, `addCaseListColumns`, `updateCaseListColumn`, `removeCaseListColumn`, `reorderCaseListColumns`, `addSearchInputs`, `updateSearchInput`, `removeSearchInput`, `reorderSearchInputs`, `setCaseListFilter`, `setCaseListTile`, `setCaseSearchAdvanced`, `setCaseSearchDisplay`, `getLookupTables`, `setFieldOptionsSource`, `getLanguages`, `addLanguage`, `updateLanguage`, `removeLanguage`, `getTranslatableContent`, `updateTranslations`, `getUsers`, `addUserProperties`, `updateUserProperty`, `removeUserProperty`, `addUserTypes`, `updateUserType`, `removeUserType`, `addPersonas`, `updatePersona`, `removePersona`, `getOrganization`, `addOrganizationLevels`, `updateOrganizationLevel`, `removeOrganizationLevel`, `addLocationProperties`, `updateLocationProperty`, `removeLocationProperty`, `createLocation`, `updateLocation`, `moveLocation`, `setLocationArchived`, `attachFieldMedia`, `attachOptionMedia`, `setMenuMedia`, `setAppLogo`, `listMediaAssets`, `removeMediaAsset`. Build vs edit still picks the prompt and the model — just no longer the tool set.
...
**Machine tools never parse XPath text.** Every field-expression, Connect, Predicate, ValueExpression, and prose slot is typed structure. Raw MCP calls and stored Blueprint use the exact canonical AST/template. The chat and build-executor provider boundaries additionally accept a direct Term wherever a ValueExpression is expected and contextually normalize it through `modelAstInput.ts` before the original schema parses; true Term slots remain direct, and the document still stores `{ kind: "term", term: ... }`. `fieldAssembly` and `editField` validate and carry typed structure directly; `tools/shared/connectInput.ts::buildConnectConfig` performs only the null/omission merge for already-typed `assessment.user_score` and `deliver_unit.entity_id`/`entity_name` values. Same-call references resolve against the complete predeclared-UUID overlay, with runtime effect dependencies still ordered. The only text → AST boundary is the builder's human XPath editor in `lib/doc/expressionText.ts`. Never cast a string into an AST-typed slot: the strict tool schema rejects it, and the construction fuzz's per-op `blueprintDocSchema` parse plus `atomicCreation.test.ts`'s persisting mock remain the tripwires.
