 ...
 * replay stages.
 *
 * Replaces `BuilderEngine.reset()`. Each store owns its own reset, and
 * the signal grid has a module-level reset too — this helper just fans
 * out the call so `ReplayController` doesn't need to know the individual
 * store shapes.
 *
...
export interface ResetBuilderInputs {
	/** Legacy session store — phase, agentActive, generationData, etc. */
 ...
 *   3. Reset the legacy session store (phase, generation metadata, appId).
 *   4. Reset the ephemeral session store (cursor mode, sidebars, focus hints).
 *   5. Drain any queued signal grid energy so stale accumulation from the
...
export function resetBuilder(inputs: ResetBuilderInputs): void {
	const { store, sessionStore, docStore, engineController } = inputs;
	...
	/* 3. Legacy store back to Idle (the generation-lifecycle fields). */
	store.getState().reset();

	/* 4. Ephemeral UI session state back to defaults. */
	sessionStore.getState().reset();

	/* 5. Signal grid energy baseline — prevents a leftover burst when the
