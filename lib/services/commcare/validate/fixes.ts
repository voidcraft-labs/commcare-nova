...
function renameReservedProperty(questions: Question[], reserved: string): boolean {
  let changed = false
  for (const q of questions) {
...
function findForm(blueprint: AppBlueprint, error: ValidationError): BlueprintForm | undefined {
...
function findModuleCaseType(blueprint: AppBlueprint, error: ValidationError): string | undefined {
  const { location: loc } = error
...
const fixUnquotedStringLiteral: FixFn = (error, blueprint) => {
  ...
  const q = error.location.questionId ? findQuestionById(form.questions, error.location.questionId) : undefined
  if (!q) return false
...
const fixUnknownFunction: FixFn = (error, blueprint) => {
  ...
  const match = error.message.match(/Unknown function "(\w[\w-]*)[\w-]*\(\)" — did you mean "(\w[\w-]*)[\w-]*\(\)"/)
