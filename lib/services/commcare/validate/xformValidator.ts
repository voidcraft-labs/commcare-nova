...
import { findAll, getAttributeValue, getElementsByTagName, getChildren, getText, isTag } from 'domutils'
...
function collectInstancePaths(el: Element, prefix: string, paths: Set<string>): void {
 ...
 */
export function validateXFormXml(xml: string, formName: string, moduleName: string): ValidationError[] {
  ...
  try {
    ...
    errors.push(validationError('XFORM_PARSE_ERROR', 'form',
  ...
  const instances = findAll(el => el.name === 'instance' && !getAttributeValue(el, 'src'), doc.children)
  if (instances.length === 0) {
    errors.push(validationError('XFORM_NO_INSTANCE', 'form',
  ...
  const dataEl = getChildren(mainInstance).find(c => isTag(c)) as Element | undefined
  if (!dataEl) {
    errors.push(validationError('XFORM_NO_INSTANCE', 'form',
  ...
  for (const bind of binds) {
    ...
    if (!nodeset) {
      errors.push(validationError('XFORM_BIND_NO_NODESET', 'form',
    ...
    if (!validPaths.has(nodeset)) {
      errors.push(validationError('XFORM_DANGLING_BIND', 'form',
  ...
  const controlTags = ['input', 'select', 'select1', 'trigger', 'upload', 'group', 'repeat']
  const controls = findAll(el => controlTags.includes(el.name), doc.children)
  for (const ctrl of controls) {
    const ref = getAttributeValue(ctrl, 'ref') || getAttributeValue(ctrl, 'nodeset')
    if (!ref) continue
    if (!ref.startsWith(`/${dataEl.name}`)) continue
    if (!validPaths.has(ref)) {
      errors.push(validationError('XFORM_DANGLING_REF', 'form',
  ...
  for (const sv of setvalues) {
    ...
    if (!ref) {
      errors.push(validationError('XFORM_SETVALUE_NO_TARGET', 'form',
  ...
  if (itextIds.size > 0) {
    ...
    for (const el of itextRefs) {
      ...
      if (!itextIds.has(textId)) {
        errors.push(validationError('XFORM_MISSING_ITEXT', 'form',
