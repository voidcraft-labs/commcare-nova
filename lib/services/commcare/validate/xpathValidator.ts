  ...
  tree.iterate({
    enter(node) {
      ...
      if (node.type === T.Error) {
        const context = expr.slice(Math.max(0, node.from - 10), Math.min(expr.length, node.to + 10))
        errors.push({
          code: 'XPATH_SYNTAX',
...
function validateFunctionCall(node: SyntaxNode, source: string, errors: XPathError[]): void {
