...
export function selectNoOptions(q: Question, ctx: QuestionContext): ValidationError[] {
  if ((q.type === 'single_select' || q.type === 'multi_select') && (!q.options || q.options.length === 0)) {
    const typeName = q.type === 'single_select' ? 'single-select' : 'multi-select'
    return [validationError('SELECT_NO_OPTIONS', 'question',
      ...
      { moduleIndex: ctx.moduleIndex, moduleName: ctx.moduleName, formIndex: ctx.formIndex, formName: ctx.formName, questionId: q.id })]
  }
  return []
}

export function hiddenNoValue(q: Question, ctx: QuestionContext): ValidationError[] {
  if (q.type === 'hidden' && !q.calculate && !q.default_value) {
    return [validationError('HIDDEN_NO_VALUE', 'question',
...
export function unquotedStringLiteral(q: Question, ctx: QuestionContext): ValidationError[] {
  ...
  for (const field of XPATH_FIELDS) {
    ...
    if (bare) {
      ...
      errors.push(validationError('UNQUOTED_STRING_LITERAL', 'question',
        ...
        { moduleIndex: ctx.moduleIndex, moduleName: ctx.moduleName, formIndex: ctx.formIndex, formName: ctx.formName, questionId: q.id, field },
        { bareWord: bare, field }))
    }
  }
  return errors
...
export function invalidQuestionId(q: Question, ctx: QuestionContext): ValidationError[] {
  if (!XML_ELEMENT_NAME_REGEX.test(q.id)) {
    return [validationError('INVALID_QUESTION_ID', 'question',
...
const QUESTION_RULES = [selectNoOptions, hiddenNoValue, unquotedStringLiteral, invalidQuestionId]

export function runQuestionRules(questions: Question[], ctx: QuestionContext): ValidationError[] {
  const errors: ValidationError[] = []
