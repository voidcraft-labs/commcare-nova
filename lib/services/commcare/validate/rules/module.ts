...
import type { AppBlueprint, BlueprintModule, Question } from '@/lib/schemas/blueprint'
import { CASE_TYPE_REGEX, MAX_CASE_TYPE_LENGTH, STANDARD_CASE_LIST_PROPERTIES } from '../../constants'
import { type ValidationError, validationError } from '../errors'

export function caseFormsNoCaseType(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  const hasCaseForms = mod.forms?.some(f => f.type !== 'survey')
  if (hasCaseForms && !mod.case_type) {
    return [validationError('NO_CASE_TYPE', 'module',
...
export function caseListOnlyHasForms(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (mod.case_list_only && mod.forms.length > 0) {
    return [validationError('CASE_LIST_ONLY_HAS_FORMS', 'module',
...
export function caseListOnlyNoCaseType(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (mod.case_list_only && !mod.case_type) {
    return [validationError('CASE_LIST_ONLY_NO_CASE_TYPE', 'module',
...
export function noFormsOrCaseList(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (!mod.case_list_only && mod.case_type && mod.forms.length === 0) {
    return [validationError(
      'NO_FORMS_OR_CASE_LIST', 'module',
...
export function invalidCaseTypeFormat(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (mod.case_type && !CASE_TYPE_REGEX.test(mod.case_type)) {
    return [validationError(
      'INVALID_CASE_TYPE_FORMAT', 'module',
...
export function caseTypeTooLong(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (mod.case_type && mod.case_type.length > MAX_CASE_TYPE_LENGTH) {
    return [validationError(
      'CASE_TYPE_TOO_LONG', 'module',
...
export function missingCaseListColumns(mod: BlueprintModule, index: number, _bp: AppBlueprint): ValidationError[] {
  if (mod.case_type && !mod.case_list_only && mod.forms.length > 0 && (!mod.case_list_columns || mod.case_list_columns.length === 0)) {
    return [validationError(
      'MISSING_CASE_LIST_COLUMNS', 'module',
...
function collectKnownCaseProperties(bp: AppBlueprint, caseType: string): Set<string> {
...
export function invalidColumnField(mod: BlueprintModule, index: number, bp: AppBlueprint): ValidationError[] {
  if (!mod.case_type || !mod.case_list_columns || mod.case_list_columns.length === 0) return []
