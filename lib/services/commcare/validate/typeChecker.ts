...
for (const op of [T.AddExpr, T.SubtractExpr, T.MultiplyExpr, T.DivideExpr, T.ModulusExpr, T.UnaryNegativeExpr]) {
  OPERATOR_TYPES.set(op, { operands: 'number', returns: 'number' })
}
// Relational comparison: expects number, returns boolean
for (const op of [T.LessThanExpr, T.LessEqualExpr, T.GreaterThanExpr, T.GreaterEqualExpr]) {
...
const PATH_TYPES = new Set<NodeType>([
  T.RootPath, T.SelfStep, T.ParentStep, T.NameTest,
  ...T.Children, ...T.Descendants,
])
...
function checkFunctionParamTypes(node: SyntaxNode, source: string, errors: TypeError[]): void {
  const nameNode = node.getChild(T.FunctionName.id)
  if (!nameNode) return
