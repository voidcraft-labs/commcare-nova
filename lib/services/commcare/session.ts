...
import type { AppBlueprint, BlueprintForm, PostSubmitDestination } from '../../schemas/blueprint'
...
export function derivePostSubmitStack(
  ...
  switch (postSubmit) {
    ...
    case 'root':
      return [{ op: 'create', children: [{ type: 'command', value: "'root'" }] }]

    case 'module':
      return [{ op: 'create', children: [{ type: 'command', value: `'m${moduleIndex}'` }] }]
...
export function deriveFormLinkStack(
  ...
  for (const link of links) {
    ...
    if (link.target.type === 'form') {
      children.push({ type: 'command', value: `'m${link.target.moduleIndex}'` })
      children.push({ type: 'command', value: `'m${link.target.moduleIndex}-f${link.target.formIndex}'` })
    } else {
      children.push({ type: 'command', value: `'m${link.target.moduleIndex}'` })
    }
  ...
  if (conditions.length > 0) {
    ...
    const fallbackOps = derivePostSubmitStack(fallback, sourceModuleIndex, sourceFormType, sourceCaseType)
...
export function deriveEntryDefinition(
  ...
  if (formLinks && formLinks.length > 0) {
    ...
    operations = deriveFormLinkStack(formLinks, postSubmit, moduleIndex, formType, caseType)
  } else if (postSubmit !== 'default') {
    operations = derivePostSubmitStack(postSubmit, moduleIndex, formType, caseType)
  }
...
export function renderEntryXml(entry: EntryDefinition): string {
  ...
  if (entry.session) {
    ...
    for (const d of entry.session.datums) {
      const detailAttr = d.detailSelect ? ` detail-select="${d.detailSelect}"` : ''
      const confirmAttr = d.detailConfirm ? ` detail-confirm="${d.detailConfirm}"` : ''
      parts.push(`      <datum id="${d.id}" nodeset="${d.nodeset}" value="${d.value}"${detailAttr}${confirmAttr}/>`)
    }
    parts.push(`    </session>`)
  }
