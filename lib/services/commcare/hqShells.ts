...
export function detailColumn(field: string, header: string | Record<string, string>): DetailColumn {
  const headerRecord: Record<string, string> = typeof header === 'string' ? { en: header } : header
  return {
...
export function detailPair(shortColumns: DetailColumn[], longColumns?: DetailColumn[]): DetailPair {
  return {
    doc_type: 'DetailPair',
    short: { doc_type: 'Detail', display: 'short', columns: shortColumns, ...detailBase() },
    long: { doc_type: 'Detail', display: 'long', columns: longColumns ?? [], ...detailBase() },
  }
}
...
export function applicationShell(
  ...
  return {
    ...
    build_signed: true,
    built_on: null,
    built_with: { signed: true, datetime: null, doc_type: 'BuildRecord', version: null, build_number: null, latest: null },
    cached_properties: {},
    comment: '',
    comment_from: null,
    copy_history: [],
    ...
    user_type: null,
    add_ons: {
      advanced_itemsets: true, calc_xpaths: true, case_detail_overwrite: true,
      case_list_menu_item: true, conditional_enum: true, conditional_form_actions: true,
      display_conditions: true, enum_image: true, subcases: true, register_from_case_list: true,
    },
    modules,
