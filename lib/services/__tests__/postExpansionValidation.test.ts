...
<h:html xmlns:h="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/2002/xforms">
  ...
  <h:body>
    ...
    expect(errors.some(e => e.code === 'XFORM_DANGLING_BIND' && e.message.includes('/data/ghost'))).toBe(true)
  })
...
<h:html xmlns:h="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/2002/xforms">
  ...
  <h:body>
    ...
    const errors = validateXFormXml(xml, 'F', 'M')
    expect(errors.some(e => e.code === 'XFORM_DANGLING_REF' && e.message.includes('/data/missing'))).toBe(true)
  })
...
<h:html xmlns:h="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/2002/xforms">
  ...
  <h:body>
    ...
    const errors = validateXFormXml(xml, 'F', 'M')
    expect(errors.some(e => e.code === 'XFORM_DANGLING_REF' && e.message.includes('/data/phantom'))).toBe(true)
  })
...
<h:html xmlns:h="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/2002/xforms">
  ...
  <h:body>
    ...
    const errors = validateXFormXml(xml, 'F', 'M')
    expect(errors.some(e => e.code === 'XFORM_MISSING_ITEXT' && e.message.includes('missing-label'))).toBe(true)
  })
...
describe('case list column validation', () => {
  it('catches column field not matching any case property', () => {
    ...
    expect(errors.some(e => e.code === 'INVALID_COLUMN_FIELD' && e.message.includes('nonexistent_prop'))).toBe(true)
  })
  ...
  it('allows custom properties defined by forms', () => {
    const bp: AppBlueprint = {
      ...
      case_types: [{ name: 'patient', properties: [{ name: 'case_name', label: 'Name' }, { name: 'age', label: 'Age' }] }],
    }
    const errors = runValidation(bp)
    expect(errors.some(e => e.code === 'INVALID_COLUMN_FIELD')).toBe(false)
  })
...
describe('expanded XForm validation', () => {
  it('our generator produces valid XForms for a standard blueprint', () => {
    const bp: AppBlueprint = {
      ...
      modules: [{
        ...
        forms: [{
          ...
          questions: [
            { id: 'case_name', type: 'text', label: 'Full Name', case_property_on: 'patient' },
            { id: 'age', type: 'int', label: 'Age', case_property_on: 'patient' },
    ...
    const xmlKey = Object.keys(hqJson._attachments).find(k => k.endsWith('.xml'))!
      ...
      modules: [{
        ...
        forms: [{
          ...
          questions: [
            ...
            { id: 'visits', type: 'repeat', label: 'Visits', children: [
    ...
    const bp: AppBlueprint = {
      ...
      modules: [{
        ...
        forms: [{
          name: 'F', type: 'survey',
          questions: [
            { id: 'color', type: 'single_select', label: 'Color', options: [
              { value: 'red', label: 'Red' },
