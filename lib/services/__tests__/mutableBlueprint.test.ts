...
describe("MutableBlueprint", () => {
	describe("search", () => {
		it("finds questions by id", () => {
			...
			expect(results.some((r) => r.questionPath === "email_address")).toBe(
				true,
			);
		});
		...
		it("finds modules by name", () => {
			...
			expect(
				results.some((r) => r.type === "module" && r.field === "name"),
			).toBe(true);
		});
	...
	describe("updateQuestion", () => {
		it("updates validation and validation_msg", () => {
			...
			expect(updated.validation).toBe(
		...
		it("updates case_property_on on a question", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			const updated = mb.updateQuestion(0, 0, qpath("client_phone"), {
				case_property_on: "client",
			});
			expect(updated.case_property_on).toBe("client");
		});

		it("throws for nonexistent question", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			expect(() =>
				mb.updateQuestion(0, 0, qpath("nonexistent"), { label: "X" }),
			).toThrow();
		});
	});
	...
	describe("addQuestion", () => {
		...
		it("adds question with case_property_on", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			mb.addQuestion(0, 0, {
				id: "age",
				type: "int",
	...
	describe("structural mutations", () => {
		...
		it("updateModule changes case_list_columns", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			mb.updateModule(0, {
				case_list_columns: [{ field: "full_name", header: "Full Name" }],
			});
	...
	describe("renameCaseProperty", () => {
		it("renames question IDs (property name = question ID) across forms", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			const result = mb.renameCaseProperty(
				"client",
				"email_address",
				"contact_email",
			);
		...
		it("renames field in case_list_columns", () => {
			...
			expect(
				mod.case_list_columns?.some((c) => c.field === "email_address"),
			).toBe(false);
	...
	describe("case type access", () => {
		...
		it("updateCaseProperty updates property metadata", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			mb.updateCaseProperty("client", "case_name", {
		...
		it("updateCaseProperty throws for nonexistent case type", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			expect(() =>
				mb.updateCaseProperty("bad", "case_name", { label: "X" }),
			).toThrow();
		});
		...
		it("updateCaseProperty throws for nonexistent property", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			expect(() =>
				mb.updateCaseProperty("client", "bad", { label: "X" }),
			).toThrow();
		});
	});
	...
	describe("deep clone isolation", () => {
		it("does not mutate original blueprint", () => {
			...
			expect(original.modules[0].forms[0].questions[0].label).toEqual(
				originalName,
			);
		});
	});
	...
	describe("renameQuestion", () => {
		...
		it("updates close_case.question reference", () => {
			const bp = makeBlueprint();
			bp.modules[0].forms[1].close_case = {
				question: "case_name",
				answer: "close",
			};
			const mb = new MutableBlueprint(bp);
	...
	describe("moveQuestion", () => {
		...
		it("throws for nonexistent question", () => {
			const mb = new MutableBlueprint(makeBlueprint());
			expect(() =>
				mb.moveQuestion(0, 0, qpath("nonexistent"), {
					afterPath: qpath("case_name"),
				}),
			).toThrow();
		});
		...
		it("moves a question from a group to root level", () => {
			...
			mb.moveQuestion(0, 0, qpath("child1", qpath("grp")), {
				afterPath: qpath("q2"),
