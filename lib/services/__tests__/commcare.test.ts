...
import {
  ...
  CASE_PROPERTY_REGEX, CASE_TYPE_REGEX, XML_ELEMENT_NAME_REGEX, XFORM_PATH_REGEX,
  escapeXml, escapeRegex,
  expandHashtags, hasHashtags, extractHashtags,
  neverCondition, alwaysCondition, ifCondition, emptyFormActions,
  detailColumn, detailPair, applicationShell, formShell, moduleShell,
  validateCaseType, validateXFormPath, validatePropertyName, isReservedProperty,
} from '../commcare'

describe('RESERVED_CASE_PROPERTIES', () => {
  it('contains all known reserved words', () => {
    const expected = [
      'case_id', 'case_name', 'case_type', 'name', 'owner_id',
      'closed', 'status', 'type', 'date', 'index', 'parent',
      'external-id', 'xform_id', 'xform_ids',
    ]
...
describe('shell factories', () => {
  ...
  it('formShell produces correct structure', () => {
    const f = formShell('id1', 'My Form', 'urn:x', 'none', emptyFormActions(), {})
