...
import type { AppBlueprint, PostSubmitDestination, FormLink } from '../../schemas/blueprint'
...
describe('derivePostSubmitStack', () => {
  ...
  describe('parent_module (stub)', () => {
    it('falls back to module behavior', () => {
      const parentOps = derivePostSubmitStack('parent_module', 1, 'followup', 'patient')
      const moduleOps = derivePostSubmitStack('module', 1, 'followup', 'patient')
...
describe('deriveFormLinkStack', () => {
  ...
  it('includes manual datums', () => {
    const links: FormLink[] = [{
      ...
      datums: [{ name: 'case_id', xpath: "instance('commcaresession')/session/data/case_id" }],
    }]
  ...
  it('generates fallback when links have conditions', () => {
    const links: FormLink[] = [
      { condition: 'age > 18', target: { type: 'form', moduleIndex: 1, formIndex: 0 } },
    ]
  ...
  it('generates compound fallback condition for multiple conditional links', () => {
    const links: FormLink[] = [
      { condition: 'x = 1', target: { type: 'form', moduleIndex: 1, formIndex: 0 } },
      { condition: 'x = 2', target: { type: 'form', moduleIndex: 2, formIndex: 0 } },
    ]
    const ops = deriveFormLinkStack(links, 'default', 0, 'registration')
...
describe('deriveEntryDefinition', () => {
  it('builds complete entry for followup form with previous navigation', () => {
    const entry = deriveEntryDefinition(
      'http://openrosa.org/formdesigner/abc', 0, 1, 'followup', 'previous', 'patient',
    )
  ...
  it('omits stack for default destination', () => {
    const entry = deriveEntryDefinition(
      'http://openrosa.org/formdesigner/abc', 0, 0, 'registration', 'default',
    )
  ...
  it('uses form_links when provided', () => {
    ...
    const entry = deriveEntryDefinition(
      'http://openrosa.org/formdesigner/abc', 0, 0, 'registration', 'module', undefined, links,
    )
  ...
  it('form_links override post_submit', () => {
    ...
    const entry = deriveEntryDefinition(
      'http://openrosa.org/formdesigner/abc', 0, 0, 'registration', 'default', undefined, links,
    )
...
describe('detectFormLinkCycles', () => {
  ...
  it('detects simple A→B→A cycle', () => {
    const bp = mkBlueprint([{
      name: 'M0', forms: [
        { name: 'F0', type: 'survey', questions: [{ id: 'q', type: 'text' }],
          form_links: [{ target: { type: 'form', moduleIndex: 0, formIndex: 1 } }] },
        { name: 'F1', type: 'survey', questions: [{ id: 'q', type: 'text' }],
...
describe('renderStackXml', () => {
  ...
  it('renders conditional clear', () => {
    const xml = renderStackXml([{ op: 'clear', ifClause: 'true()', children: [] }])
  ...
  it('renders multiple operations', () => {
    const ops: StackOperation[] = [
      { op: 'create', ifClause: 'x = 1', children: [{ type: 'command', value: "'m0-f0'" }] },
...
describe('renderEntryXml', () => {
  ...
  it('renders followup entry with session and stack', () => {
    const entry = deriveEntryDefinition(
      'http://openrosa.org/formdesigner/xyz', 1, 2, 'followup', 'previous', 'patient',
    )
    const xml = renderEntryXml(entry)
...
describe('fromHqWorkflow', () => {
  ...
  it('round-trips with toHqWorkflow', () => {
    const destinations: PostSubmitDestination[] = ['default', 'root', 'module', 'parent_module', 'previous']
