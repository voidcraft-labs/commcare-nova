...
export function classifyError(error: unknown): ClassifiedError {
  ...
  if (APICallError.isInstance(error)) {
    ...
    if (status === 401 || status === 403) {
      return { type: 'api_auth', message: MESSAGES.api_auth, recoverable: false, raw }
    }
    if (status === 429) {
      return { type: 'api_rate_limit', message: MESSAGES.api_rate_limit, recoverable: false, raw }
    }
    if (status === 529) {
      return { type: 'api_overloaded', message: MESSAGES.api_overloaded, recoverable: false, raw }
    }
    if (status === 408) {
      return { type: 'api_timeout', message: MESSAGES.api_timeout, recoverable: false, raw }
    }
    // 400-level errors with "input" in message are usually malformed requests (model_error)
    if (status === 400) {
      return { type: 'model_error', message: MESSAGES.model_error, recoverable: false, raw }
    }
    // 5xx server errors
    if (status && status >= 500) {
      ...
      return { type: 'api_server', message: MESSAGES.api_server, recoverable: false, raw }
    }
  ...
  if (error instanceof TypeError && raw.includes('fetch')) {
    return { type: 'stream_broken', message: MESSAGES.stream_broken, recoverable: false, raw }
  }
  ...
  return { type: 'internal', message: MESSAGES.internal, recoverable: false, raw }
}
