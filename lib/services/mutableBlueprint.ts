...
export class MutableBlueprint {
	...
	switchConnectMode(type: ConnectType | null | undefined): void {
		...
		const resolved =
			type === undefined ? (this.lastConnectType ?? "learn") : type;
	...
	updateApp(updates: { app_name?: string }): void {
		if (updates.app_name !== undefined)
			this.blueprint.app_name = updates.app_name;
	}
	...
	removeModule(mIdx: number): void {
		if (mIdx < 0 || mIdx >= this.blueprint.modules.length)
	...
	renameCaseProperty(
		...
		for (let mIdx = 0; mIdx < this.blueprint.modules.length; mIdx++) {
			...
			for (let fIdx = 0; fIdx < mod.forms.length; fIdx++) {
				...
				formChanged =
					this.renamePropertyInQuestions(form.questions, oldName, newName) ||
					formChanged;
	...
	renameQuestion(
		...
		xpathFieldsRewritten += this.rewriteXPathInQuestions(
			form.questions,
			oldXPathPath,
			newId,
		);
	...
	private newQuestionToBlueprint(nq: NewQuestion): Question {
		return {
			...
			...((nq.type === "group" || nq.type === "repeat") && {
				children: (nq.children ?? []).map((c) =>
					this.newQuestionToBlueprint(c),
				),
			}),
		};
	}

	private rewriteXPathInQuestions(
		questions: Question[],
		oldPath: string,
		newId: string,
	): number {
	...
	private renamePropertyInQuestions(
		questions: Question[],
		oldName: string,
		newName: string,
	): boolean {
		...
		for (const q of questions) {
			...
			if (q.children) {
				changed =
					this.renamePropertyInQuestions(q.children, oldName, newName) ||
					changed;
			}
		}
		return changed;
