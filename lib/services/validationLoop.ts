...
function validateExpansion(hqJson: HqApplication, blueprint: AppBlueprint): ValidationError[] {
...
export async function validateAndFix(
...
): Promise<{ success: boolean; blueprint: AppBlueprint; hqJson?: Record<string, any>; errors?: ValidationError[] }> {
  ...
  while (true) {
    ...
    // Stuck detection
    const sig = errors.map(e => `${e.code}:${e.location.formName ?? ''}:${e.location.questionId ?? ''}`).sort().join('|||')
    recentSignatures.push(sig)
    if (recentSignatures.length > MAX_STUCK_REPEATS) recentSignatures.shift()
    if (recentSignatures.length === MAX_STUCK_REPEATS && recentSignatures.every(s => s === sig)) {
      try {
        const hqJson = expandBlueprint(blueprint)
    ...
    ctx.emit('data-fix-attempt', { attempt, errorCount: errors.length, errors: errors.map(errorToString) })
