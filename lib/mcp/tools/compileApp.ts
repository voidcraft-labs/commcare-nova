...
export function registerCompileApp(server: McpServer, ctx: ToolContext): void {
	server.registerTool(
		...
		{
			description:
				'Compile an owned app to CommCare HQ format. `format: "json"` returns the HQ JSON as text, or, when the app has media, a base64-encoded zip bundle (JSON + an HQ multimedia upload) so the media round-trips. `format: "ccz"` returns the binary archive base64-encoded. When the app uses HQ feature flags, a second text block lists the requirements; because a downloaded artifact has no known destination, these are requirements, not flags Nova has confirmed missing.',
			inputSchema: {
				app_id: z
					.string()
					.describe(
