...
- **`ReconcilerProvider.tsx`** — owns the SINGLE reconciler + SINGLE `EventSource`, wires the real network deps, re-opens the stream at the reload cursor, and exposes the reconciler + `subscribePresence` (P7 rides the same stream) + `activate` via `context.ts`. Guards `EventSource` presence so a non-browser render (SSR / a jsdom test mounting the builder) mounts the state machine without a live stream. The browser's own auto-reconnect covers only TRANSPORT drops; an HTTP-error response (a 502/503 from the LB mid-deploy) fails the `EventSource` permanently, so the provider's `error` listener reopens a CLOSED stream at the reconciler's confirmed cursor with backoff — without it a tab keeps saving but silently stops receiving peer frames.
