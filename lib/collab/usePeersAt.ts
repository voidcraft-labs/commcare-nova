 ...
 *
 *  - `home` / `app-setup` / `project-data` → `null` (roster-only). None of
 *    the three names a blueprint entity: app administration is not app
 *    content, and a Project's shared lookup tables are not this app's content
 *    at all — so there is no tree row to mark.
 ...
 *    screen.
 *  - `form-condition` / `form-operations` / `form-links` → the FORM they
 *    configure.
...
export function peerTarget(location: Location): PeerTarget | null {
	switch (location.kind) {
		...
		case "form-links":
			// An operation or a link is not a blueprint ENTITY with a marker
			// surface — it lives inside the form record — so a peer editing one
			// marks the form, which is the row a teammate can actually see.
			return { kind: "form", uuid: location.formUuid };
		case "form":
