...
import { type ChangeSpec, EditorState, type Extension, StateEffect } from '@codemirror/state'
...
const revertAutocomplete = ViewPlugin.fromClass(class {
  update(update: ViewUpdate) {
...
export function xpathChips(provider: ReferenceProvider): Extension {
  ...
  return [plugin, buildAtomicRanges(plugin), backspaceRevert, revertAutocomplete]
}
