...
function buildFunctionTemplate(name: string, minArgs: number, paramTypes?: string[]): string {
...
const functionCompletions: Completion[] = (() => {
  ...
  for (const [name, spec] of FUNCTION_REGISTRY) {
    ...
    const arityLabel = spec.maxArgs === -1
      ? `${spec.minArgs}+`
...
function findDataPathRoot(node: SyntaxNode): SyntaxNode | null {
  ...
  while (pathRoot.parent && isPathNode(pathRoot.parent)) pathRoot = pathRoot.parent
...
function hashtagSource(
  ...
  return (ctx) => {
    ...
    if (hashtagAncestor) {
      ...
      let left = pathRoot
      while (left.firstChild && isPathNode(left.firstChild)) left = left.firstChild
...
function dataPathSource(
  ...
  return (ctx) => {
    ...
    if (state.doc.sliceString(firstNameTest.from, firstNameTest.to) !== 'data') return null
...
export function xpathAutocomplete(getContext: () => XPathLintContext | undefined): Extension {
  // Suppress functions inside hashtag refs (#case/prop) and path steps (/data/step)
  const wrappedFunctionSource = ifNotIn(['StringLiteral', 'HashtagRef', 'Child', 'Descendant'], functionSource)
  const wrappedHashtagSource = ifNotIn(['StringLiteral'], hashtagSource(getContext))
  const wrappedDataPathSource = ifNotIn(['StringLiteral'], dataPathSource(getContext))

  return [
    autocompletion({
      override: [wrappedFunctionSource, wrappedHashtagSource, wrappedDataPathSource],
