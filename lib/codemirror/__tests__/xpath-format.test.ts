...
describe('prettyPrintXPath', () => {
  ...
  it('does not expand empty function calls', () => {
    const expr = "if(today() > date(#case/some_really_long_property_name), concat(#case/first_name, ' ', #case/last_name), 'N/A')"
  ...
  it('does not expand grouping parens', () => {
    const expr = "(#case/age + #case/bonus_years) * #case/multiplier_for_some_really_long_calculation_property > #case/threshold_value"
    const result = prettyPrintXPath(expr)
    expect(result).toContain('(#case/age + #case/bonus_years)')
  })

  it('preserves string literals containing parens', () => {
    const expr = "if(#case/status = 'active (current)', concat(#case/first_name, ' (', #case/last_name, ')'), 'Inactive (closed)')"
    const result = prettyPrintXPath(expr)
  ...
  it('expands predicates with newlines after [ and before ]', () => {
    const expr = "instance('casedb')/casedb/case[@case_type = 'mother' and @status = 'open'][last()]/case_name"
    const result = prettyPrintXPath(expr)
    expect(result).toBe(
      "instance(\n" +
      "\t'casedb'\n" +
  ...
  it('does not break and/or at top level', () => {
    const expr = "#case/status = 'active' and #case/enrolled = 'yes' and #case/age > 18 and #case/consent = 'yes'"
    const result = prettyPrintXPath(expr)
