...
const DIRS: readonly Cell[] = [[-1, 0], [1, 0], [0, -1], [0, 1]]
...
function hasNewIsolatedCell(board: boolean[][], cols: number, placedCells: Cell[]): boolean {
  for (const [pr, pc] of placedCells) {
    for (const [dr, dc] of DIRS) {
      ...
      for (const [nr, nc] of DIRS) {
        const er = r + nr, ec = c + nc
        if (er >= 0 && er < BOARD_ROWS && ec >= 0 && ec < cols && !board[er][ec]) {
...
function fitShape(board: boolean[][], cols: number, shape: Shape, oRow: number, oCol: number): Cell[] | null {
  const cells: Cell[] = []
  for (const [sr, sc] of shape) {
    const r = oRow + sr, c = oCol + sc
    if (r < 0 || r >= BOARD_ROWS || c < 0 || c >= cols || board[r][c]) return null
    cells.push([r, c])
  }
  return cells
}
...
function canSolveAhead(board: boolean[][], cols: number, depth: number): boolean {
  ...
  for (const piece of PIECES) {
    for (const rot of piece.rotations) {
      for (const [dr, dc] of rot) {
        ...
        const ok = !hasNewIsolatedCell(board, cols, cells)
...
export function generateTilingPlan(
  ...
  while (true) {
    ...
    const sorted = shuffleArray([...PIECES], rng)
    ...
    for (const piece of sorted) {
      ...
      const rotOrder = shuffleArray(
        Array.from({ length: piece.rotations.length }, (_, i) => i), rng,
      )
    ...
    if (!placed) {
      ...
      throw new Error(`Unexpected: no placement found at [${target[0]},${target[1]}] on ${cols}-col board`)
    }
  }
...
function tryPlace(
  board: boolean[][], cols: number,
  piece: PieceDefinition, rotOrder: number[], target: Cell,
): Placement | null {
  ...
  for (const ri of rotOrder) {
    ...
    for (const [dr, dc] of shape) {
      ...
      for (const [r, c] of cells) board[r][c] = true
      const valid = !hasNewIsolatedCell(board, cols, cells)
