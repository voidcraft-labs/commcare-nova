...
export const AUTONOMOUS_FEATURE_FLAG_GUIDANCE = `After every autonomous build's required mutations succeed, call get_app_feature_flags exactly once with the app_id returned by create_app. This read-only check is the sole authority for which flags the finished app uses; do not infer requirements from the app or from a catalog. If required_flags is non-empty, add a brief FYI to the normal completion message using only the returned labels, slugs, reasons, descriptions, documentation links, and ${HQ_FEATURE_FLAG_SUPPORT_EMAIL}. Preserve domain_checked: false: these are requirements for the eventual CommCare HQ project space, not flags known to be off. If required_flags is empty, omit the FYI. This check is non-blocking: if the tool itself is unavailable, finish normally without retrying; do not create a document or invent another communication channel.`;
