...
returned batch once, under the Project lookup context the caller hands it
(`ConnectSwitchGate`). `useSwitchConnectMode` binds the builder's live context
from `useLookupCommitState`, so a doc that carries a lookup-backed select can
still switch modes; the store deliberately has no default for it, because the
gate is absolute and an unavailable context refuses every switch on such a
doc. A null target clears the app mode and every form block;
