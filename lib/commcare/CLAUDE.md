...
The routing rule is `(mode, via)`-shaped: `exact` self-walk and `range` self-walk ride on the bare prompt slot alone (CCHQ's runtime default does the exact comparison; the `daterange` widget handles the two-bound semantic internally for the current case). Every other combination — non-`exact` modes on any via, and `exact` mode with a non-self via — routes through `_xpath_query`. The validator rule `searchInputViaModeCompatibility` rejects two `(mode, via)` shapes no faithful wire form covers: `range` on a non-self via (the two-value semantic can't ride on a single prompt binding when the property lives on a related case) and `multi-select-contains` on every simple-arm input (CCHQ's runtime defaults to full-string exact match, so token containment via the bare prompt silently mismatches; authors who need it author `selected(prop, input(name))` on the advanced arm). The validator rule `caseSearchConfigRequiresCaseType` rejects a `caseSearchConfig` without a `caseType` — CCHQ's `<remote-request>` carries a mandatory `case_type` slot.
