...
Location owners add two exact leaves to that vocabulary. `fixed-location` emits its app-scoped place UUID as a literal. `owner-location-at-level` emits `instance('locations')/locations/location[@type='<destination-level-code>'][@<nearest-case-owning-ancestor-code>_id = <owner-case-expression>]/@id` and pulls both `locations` and `casedb`. Either leaf must occupy the complete owner expression; the validator refuses name, rename, and nested carriers and proves the destination level owns cases before this boundary prints it.
