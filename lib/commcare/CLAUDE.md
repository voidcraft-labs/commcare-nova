...
The routing rule is `(mode, via, name vs property)`-shaped: only `exact` (or `range`) on self-walk / absent `via` AND `name === property` rides on the bare prompt slot alone (CCHQ's runtime auto-match against the prompt key IS the authored comparison; the `daterange` widget handles the two-bound semantic internally for the current case). Every other combination — non-`exact` modes on any via, `exact` mode with a non-self via, OR `exact` mode with `name !== property` — routes through `_xpath_query` and stamps `exclude="true()"` on the prompt. The validator rule `searchInputViaModeCompatibility` rejects three shapes no faithful wire form covers: `range` on a non-self via (the two-value semantic can't ride on a single prompt binding when the property lives on a related case), `range` with `name !== property` on self-walk (CCHQ's auto-match keys on the prompt key, and the `_xpath_query` route has no range arm to fall back to), and `multi-select-contains` on every simple-arm input (CCHQ's runtime defaults to full-string exact match, so token containment via the bare prompt silently mismatches; authors who need it author `selected(prop, input(name))` on the advanced arm). The validator rule `caseSearchConfigRequiresCaseType` rejects a `caseSearchConfig` without a `caseType` — CCHQ's `<remote-request>` carries a mandatory `case_type` slot.
