...
The one terminal state no in-process callback can reach — a hard process kill — is settled by the stale-`generating` reaper (`reapStaleGenerating`), which refunds the stranded build reservation off the durable `reservation` marker on the app doc. Edits stay `complete` (never `generating`), so a hard-killed edit's 5-credit hold is an accepted residual. `draft` rows (MCP builds) are exempt by construction — the reaper and the list failure inference key on `"generating"`, and an external agent runs on no liveness timer, so a draft can sit untouched indefinitely without being inferred dead. A RETRY of a failed build re-enters the reaper's coverage: the chat route flips `error → generating` (`markAppGenerating`, awaited, fresh `updated_at`) before its concurrency check — the same write-then-check ordering `createApp` uses — so a hard-killed retry's reservation refunds exactly like a first build's. The flip is a transaction that moves ONLY `error → generating`: same-app retry contenders share one row (which `hasActiveGeneration` excludes as the contender's own), so the compare-and-flip is what arbitrates them — the loser throws `GenerationRetryConflictError` and the route 429s it without touching the winner's live lock.
