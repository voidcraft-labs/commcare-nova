/**
 * `mutationTargetsInvalid` — the target-admission guard the guarded commit runs
...
const USER_PROPERTY = testUuid("55555555-5555-4555-8555-555555555555");
const USER_TYPE = testUuid("66666666-6666-4666-8666-666666666666");
const PERSONA = testUuid("77777777-7777-4777-8777-777777777777");
...
describe("mutationTargetsInvalid — entity kinds", () => {
...
describe("mutationTargetsInvalid — user collection identities", () => {
	function withUsers(): BlueprintDoc {
		const { doc } = fixture();
		doc.userProperties = {
			[USER_PROPERTY]: {
				uuid: USER_PROPERTY,
		...
		doc.personas = {
			[PERSONA]: {
				uuid: PERSONA,
				name: "Priya",
				userTypeUuid: USER_TYPE,
				values: { [USER_PROPERTY]: "south" },
			},
		};
		return doc;
	}

	it("rejects missing item targets and unresolved value/role references", () => {
		const doc = withUsers();
		expect(
			mutationTargetsInvalid(doc, [
				{ kind: "updateUserProperty", uuid: MISSING, patch: { label: "X" } },
			]),
		).toBe(true);
		expect(
			mutationTargetsInvalid(doc, [
			...
			mutationTargetsInvalid(doc, [
				{
					kind: "addPersona",
					persona: {
						uuid: testUuid("missing-role-persona"),
						name: "Missing role",
						userTypeUuid: MISSING,
					},
				},
			]),
		).toBe(true);
	...
	it("requires explicit same-batch cleanup before referenced entities are removed", () => {
		const doc = withUsers();
		expect(
			mutationTargetsInvalid(doc, [
				{ kind: "removeUserProperty", uuid: USER_PROPERTY },
			]),
		).toBe(true);
		expect(
			mutationTargetsInvalid(doc, [
			...
			mutationTargetsInvalid(doc, [
				{
					...
					patch: {},
					valuePatch: {
						userPropertyUuid: USER_PROPERTY,
						value: null,
					},
				},
				{
...
describe("mutationTargetsInvalid — granular catalog kinds", () => {
	...
	it("rejects duplicate catalog birth actions before reduction", () => {
		const { doc } = fixture();
		expect(
			mutationTargetsInvalid(doc, [
			...
			mutationTargetsInvalid(doc, [
				{
					kind: "addCaseProperty",
					caseType: "patient",
					property: { name: "case_name", label: proseText("Duplicate") },
				},
			]),
		).toBe(true);
	});
...
describe("mutationTargetsInvalid — granular collection kinds (item uuid)", () => {
...
describe("mutationTargetsInvalid — case-operation logical identities", () => {
	...
	it("rejects a scalar/move/write/link edit after a peer removed its target", () => {
		...
		for (const mutation of operationEdits) {
			expect(mutationTargetsInvalid(withoutOperation, [mutation])).toBe(true);
		}
	...
	it("requires a move anchor to exist earlier in the same batch", () => {
		...
		expect(mutationTargetsInvalid(doc, [move(null), move(null)])).toBe(false);
	});
...
describe("mutationTargetsInvalid — app-level scalars", () => {
