...
describe('applyDefaults', () => {
  it('fills in label from case type for sparse question', () => {
    const result = applyDefaults({ id: 'case_name', type: 'text', case_property_on: 'patient' }, [testCaseType])
    expect(result.label).toBe('Full Name')
  })

  it('preserves explicit label when provided', () => {
    const result = applyDefaults({ id: 'case_name', type: 'text', label: 'Custom Label', case_property_on: 'patient' }, [testCaseType])
  ...
  it('fills in options for select properties', () => {
    const result = applyDefaults({ id: 'gender', type: 'single_select', case_property_on: 'patient' }, [testCaseType])
    expect(result.options).toEqual([{ value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }])
  })

  it('fills in hint from case type', () => {
    const result = applyDefaults({ id: 'phone', type: 'text', case_property_on: 'patient' }, [testCaseType])
    expect(result.hint).toBe('Include country code')
  })

  it('derives type from case type data_type', () => {
    const result = applyDefaults({ id: 'age', case_property_on: 'patient' }, [testCaseType])
  ...
  it('returns question unchanged when case types is null', () => {
    const result = applyDefaults({ id: 'case_name', type: 'text', case_property_on: 'patient' }, null)
    expect(result.label).toBeUndefined()
  })

  it('returns question unchanged when property not found in case type', () => {
    const result = applyDefaults({ id: 'nonexistent', type: 'text', case_property_on: 'patient' }, [testCaseType])
    expect(result.label).toBeUndefined()
  })
