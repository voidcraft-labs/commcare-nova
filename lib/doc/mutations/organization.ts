...
export function applyOrganizationMutation(
	...
	switch (mut.kind) {
		case "addOrganizationLevel":
			draft.organizationLevels = recordWithValue(
				draft.organizationLevels,
				mut.level.uuid,
				structuredClone(mut.level),
			);
			draft.organizationLevelOrder = spliceAfter(
				draft.organizationLevelOrder,
				mut.level.uuid,
				mut.after,
			);
			return;
		case "updateOrganizationLevel":
			applyPatch(ownRecordValue(draft.organizationLevels, mut.uuid), mut.patch);
			return;
		case "removeOrganizationLevel":
			dropEntry(draft, "organizationLevels", mut.uuid);
			return;
		case "addLocationProperty":
			draft.locationProperties = recordWithValue(
				draft.locationProperties,
				mut.property.uuid,
				structuredClone(mut.property),
			);
			draft.locationPropertyOrder = spliceAfter(
				draft.locationPropertyOrder,
				mut.property.uuid,
				mut.after,
			);
			return;
		case "updateLocationProperty":
			applyPatch(ownRecordValue(draft.locationProperties, mut.uuid), mut.patch);
			return;
...
function dropEntry(
	...
	const orderSlot =
		slot === "organizationLevels"
			? "organizationLevelOrder"
			: "locationPropertyOrder";
	const sequence = draft[orderSlot];
	if (sequence !== undefined) {
		const remaining = sequence.filter((entry) => entry !== uuid);
		if (remaining.length === 0) delete draft[orderSlot];
		else draft[orderSlot] = remaining;
	}
	...
	if (Object.keys(remaining).length === 0) delete collections[slot];
	else collections[slot] = remaining;
}
