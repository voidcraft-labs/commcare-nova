...
**Clearing an optional slot must not ride a generic `update*` patch.** The SA streams mutations to the client as JSON over SSE; `JSON.stringify` DROPS keys whose value is `undefined`. So a clear encoded as an `updateField`/`updateModule`/`updateForm` patch of `{ key: undefined }` arrives at `applyMany` as `{}` and the reducer's `Object.assign` no-ops — the slot never clears, and the client auto-saves the stale value back. Any slot that must be clearable across the wire needs a dedicated mutation kind carrying an explicit `null` (which survives JSON), mapped to `undefined` INSIDE the reducer — the `setAppLogo` / `setFieldMedia` / `setModuleMedia` / `setFormMedia` pattern. Do NOT add a generic "null-means-clear" rule to the `update*` reducers: `setConnectType`'s slot is genuinely `.nullable()` and stores `null` as a real value. (A concrete array survives JSON whole, so a wholesale `{ options: [...] }` patch can clear a nested key by omitting it from the rebuilt element — that path stays on `updateField`.)
