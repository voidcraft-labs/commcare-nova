...
Pure functions that read or produce a `BlueprintDoc` (or a subtree) colocate in this package. The doc stores fields by uuid while mutations and the agent tools speak path strings — the path↔uuid resolution helpers here are the only translation layer. Connect-config doc helpers (validate-time defaults, empty-sub-config stripping, restore-time id dedup) live here too; wire-emit Connect defaults live separately at `lib/commcare/connectDefaults.ts`.
