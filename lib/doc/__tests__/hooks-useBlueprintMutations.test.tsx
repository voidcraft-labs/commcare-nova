...
describe("useBlueprintMutations", () => {
	...
	it("markCasePropertyExternal marks (with note), then clears, touching nothing else", () => {
		const { result } = renderHook(() => useMutationsAndFirstFormChildren(), {
			wrapper,
		});
		...
		act(() => {
			result.current.mutations.markCasePropertyExternal(
				"medication_order",
				"order_status",
				{ note: "set by the pharmacy app" },
			);
		});
		let record = result.current.store
			?.getState()
			.caseTypes?.find((ct) => ct.name === "medication_order");
		expect(
			record?.properties.find((p) => p.name === "order_status")?.external,
		).toEqual({ note: "set by the pharmacy app" });
		expect(record?.properties.find((p) => p.name === "ordered_on")).toEqual({
			name: "ordered_on",
			label: "Ordered on",
			data_type: "date",
		});
		...
		record = result.current.store
			?.getState()
			.caseTypes?.find((ct) => ct.name === "medication_order");
		const cleared = record?.properties.find((p) => p.name === "order_status");
		expect(cleared?.external).toBeUndefined();
		expect(cleared?.label).toBe("Order status");
	});

	it("markCasePropertyExternal declares an unlisted property in the bare shape plus the marking", () => {
		const { result } = renderHook(() => useMutationsAndFirstFormChildren(), {
			wrapper,
		});
		...
		act(() => {
			result.current.mutations.setCaseTypes([
				{ name: "medication_order", properties: [] },
			]);
		});
		...
		const record = result.current.store
			?.getState()
			.caseTypes?.find((ct) => ct.name === "medication_order");
		expect(record?.properties).toEqual([
			{ name: "priority_flag", label: "priority_flag", external: {} },
		]);
	});
